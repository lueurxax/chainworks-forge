...
async fn run_daemon() -> Result<()> {
    ...
    match engine::shutdown_service::dispatch_planned_shutdown_signals(&pool).await {
        Ok(dispatched) => {
            if dispatched > 0 {
                info!(dispatched, "P083 shutdown signal dispatch on startup complete");
            }
        }
        Err(err) => {
    ...
    insert_durable_monotonic_clock_baseline(&pool).await
    ...
    match mode {
        ...
        _ => {
            ...
            let principals_reload_secs: u64 = std::env::var("CHAINWORKS_PRINCIPALS_RELOAD_SECS")
                ...
                .unwrap_or(2)
                .max(1); // Clamp to at least 1s to prevent a busy reload loop.
