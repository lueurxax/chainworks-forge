...
use engine::command_handler::{validate_caller_request_id as validate_caller_request_id_engine, ApprovalResolutionConflict, CommandHandler};
...
async fn enrich_run_with_artifact_contracts(
    ...
    gql.p083_rollout_contract_readback = if let Some(check) = rollout_check.as_ref() {
        ...
        let mut readback_obj = match crate::types::p083::GqlP083RolloutContractReadback::from_readback_json(
            &readback,
            queue_rank,
        ) {
            Ok(obj) => obj,
...
impl QueryRoot {
    ...
    /// P083: Cancel late-output overflow latches for a run.
    /// Per post_cancel_late_output_contract_v1.readback_fields: surfaces all overflow latch rows
    /// for the run so operators can confirm projection_mutation_blocked=true and see quarantine info.
    async fn p083_cancel_late_output_overflow(
        &self,
        ctx: &Context<'_>,
        run_id: ID,
    ) -> Result<Vec<crate::types::p083::GqlP083CancelLateOutputOverflowRow>> {
        require_operator_read(ctx).await?;
        let pool = ctx.data::<SqlitePool>()?;
        let rows = db::repos::cancel_late_output_overflow::find_by_run_id(pool, run_id.as_str())
            .await
            .map_err(|e| Error::new(format!("P083_QUERY_ERROR: {e}")))?;
        Ok(rows
            .into_iter()
            .map(crate::types::p083::GqlP083CancelLateOutputOverflowRow::from)
            .collect())
    }

    /// P083: Identity-ambiguous provider sessions for a run requiring manual identity verification.
    /// Per manual_process_identity_check_ui_v1: drives the ManualProcessIdentityCheckBanner in
    /// the run detail overview, stage detail, and recovery inbox surfaces.
    async fn p083_identity_hold_sessions(
        ...
        let pool = ctx.data::<SqlitePool>()?;
        let rows =
            db::repos::provider_sessions::find_identity_ambiguous_for_run(pool, run_id.as_str())
                .await
                .map_err(|e| Error::new(format!("P083_QUERY_ERROR: {e}")))?;
        let rid = run_id.as_str();
        Ok(rows
            .into_iter()
            .map(|s| GqlP083IdentityHoldSession {
                provider_session_id: s.provider_session_id,
                provider_name: s.provider,
                cancellation_epoch: s.cancellation_epoch,
                last_seen_pid: s.process_id,
                // SEC-P083-MED-001: derive a run-scoped non-reversible reference; never
                // return the raw process_start_identity value to callers.
                process_start_identity_hash: s.process_start_identity.as_deref().map(|raw| {
                    derive_p083_process_identity_ref(rid, raw)
                }),
                latest_receipt_id: s.latest_receipt_id,
                reason_detail: s.held_reason.as_deref().map(|r| {
                    format!("Process identity could not be confirmed. Reason: {r}")
                }),
            })
            .collect())
    }
...
fn validate_caller_request_id_graphql(request_id: &str) -> Result<()> {
    validate_caller_request_id_engine(request_id).map_err(|e| {
        Error::new(format!("P083_INVALID_ARG: {e}"))
    })
}
...
fn p083_command_error(mutation_name: &str, message: &str) -> Error {
    ...
    let code: &str = if message.contains("PROVIDER_SESSION_NOT_FOUND") {
    ...
    } else if message.contains("identity_ambiguous") || message.contains("IDENTITY_AMBIGUOUS_NOT_FOUND") {
        "identity_ambiguous"
    ...
    } else if message.contains("APPROVAL_NOT_ACTIONABLE") || message.contains("approval_not_actionable") {
        "approval_not_actionable"
    } else if message.contains("cannot be retried") || message.contains("cannot be targeted-retried") || message.contains("STAGE_NOT_RETRYABLE") {
        "stage_not_retryable"
    } else if message.contains("requires_effect_reconciliation") || message.contains("SIDE_EFFECT_NOT_RECONCILABLE") {
        ...
        "run_not_found"
    } else if message.contains("p083_operator_required") || message.contains("P083_OPERATOR_REQUIRED") {
        "p083_operator_required"
    } else if message.contains("operator_required") || message.contains("OPERATOR_REQUIRED") {
        "operator_required"
    } else {
        tracing::error!(mutation = mutation_name, error = message, "P083 command error");
        "internal"
    };
...
impl MutationRoot {
        ...
        match result {
            ...
            Err(e) => {
                    ...
                    approval_resolution_conflict_code(&e)
                {
                    ...
                    gql_err = gql_err.extend_with(|_, ext| ext.set("requestId", request_id.clone()));
                    Err(gql_err)
                }
            }
        }
    ...
    async fn p083_rollback_execution(
        ...
        if !matches!(rollback_mode.as_str(), "permissive" | "disabled") {
            return Err(Error::new("P083_INVALID_ARG: rollbackMode must be 'permissive' or 'disabled'"));
        }
    ...
    async fn p083_set_enforcement_mode(
        ...
        if !matches!(enforcement_mode.as_str(), "disabled" | "permissive" | "enforce") {
            return Err(Error::new("P083_INVALID_ARG: enforcementMode must be 'disabled', 'permissive', or 'enforce'"));
        }
    ...
    async fn runs_retry(
        ...
        let caller = graphql_caller(&ctx, &principal, "runsRetry")
            .with_request_id(&request_id);
    ...
    async fn p083_mark_provider_session_process_absent(
        ...
        mutation_allowed(ctx, &principal, MutationName::P083MarkProviderSessionProcessAbsent)
            .await?;
