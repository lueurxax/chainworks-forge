...
mod tests {
    ...
    async fn p080_auth_ok_for_subscription_operator_passes() {
        ...
        let fingerprint =
            auth::principal_token_fingerprint_by_id(&table, "test-operator").unwrap();
    ...
    async fn p080_auth_ok_for_subscription_read_only_operator_passes() {
        ...
        let handle = P046LivePrincipalHandle::new(table.clone());
        let fp = auth::principal_token_fingerprint_by_id(
            &table,
            "test-p080-read-only-operator",
        )
        .unwrap();
        let cred = P046LiveCredential {
        ...
        assert!(
            !handle
                .auth_ok_for_p080_subscription(&cred, None)
                .await,
