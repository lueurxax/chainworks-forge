...
use async_graphql::{InputValueError, InputValueResult, Json, Scalar, ScalarType, SimpleObject, Value};
...
pub fn validate_rollback_disposition_v1(
    ...
    let dlr_val = obj.get("data_loss_risk").expect("checked in REQUIRED_FIELDS loop above");
    ...
    for key in obj.keys() {
        if !ALLOWED_FIELDS.contains(&key.as_str()) {
            return Err(RollbackDispositionValidationError::ExtraProperty(key.clone()));
        }
    }
...
pub fn wrap_rollback_disposition_for_readback(
...
impl RollbackDispositionJSON {
    ...
    pub fn validated(
        value: serde_json::Value,
    ) -> Result<Self, RollbackDispositionValidationError> {
...
impl GqlP083RolloutContractReadback {
    ...
    pub fn from_readback_json(
        ...
        Ok(GqlP083RolloutContractReadback {
            ...
            rollout_contract_status: str_field("status").unwrap_or_else(|| "not_applicable".to_string()),
            rollout_contract_decision: str_field("backendDecision").unwrap_or_else(|| "not_applicable".to_string()),
            rollout_contract_failure_reasons: str_arr("failureReasons"),
            rollout_contract_waiver_state: str_field("waiverState").unwrap_or_else(|| "none".to_string()),
            rollout_contract_waiver_expires_at: str_field("waiverExpiresAt"),
            rollout_contract_enforcement_mode: str_field("enforcementMode").unwrap_or_else(|| "disabled".to_string()),
            rollout_contract_enforcement_mode_reason: str_field("enforcementModeReason").unwrap_or_else(|| "not_applicable".to_string()),
            rollout_contract_hold_conditions: str_arr("holdConditions"),
            rollout_contract_rollback_disposition: validated_disposition.clone(),
            rollout_contract_source_lane: str_field("sourceLane").unwrap_or_else(|| "not_applicable".to_string()),
            rollout_contract_enabled_state: str_field("enabledState").unwrap_or_else(|| "disabled".to_string()),
            rollout_contract_disabled_reason_code: str_field("disabledReasonCode"),
            rollout_contract_action_id: str_field("actionId"),
            rollout_contract_operator_message: str_field("operatorMessage").unwrap_or_else(|| "not_applicable".to_string()),
            rollout_contract_projection_integrity: str_field("projectionIntegrity").unwrap_or_else(|| "not_applicable".to_string()),
            rollout_contract_cutover_policy_revision: str_field("cutoverPolicyRevision").unwrap_or_else(|| "not_applicable".to_string()),
            rollout_contract_diagnostic_redaction: str_field("diagnosticRedaction").unwrap_or_else(|| "not_applicable".to_string()),
            rollout_contract_next_steps: str_arr("nextSteps"),
            rollout_contract_shutdown_deadline_config_state: shutdown_deadline_config.unwrap_or_else(|| "not_configured".to_string()),
            rollout_contract_command_lease_ttl_config_state: command_lease_ttl_config.unwrap_or_else(|| "not_configured".to_string()),
...
mod tests {
    ...
    fn missing_schema_version_rejected() {
        ...
        assert!(matches!(err, RollbackDispositionValidationError::MissingField("schema_version")));
    }
    ...
    fn wrong_schema_version_rejected() {
        ...
        let err = validate_rollback_disposition_v1(&v).unwrap_err();
        assert!(matches!(err, RollbackDispositionValidationError::InvalidSchemaVersion));
    }
    ...
    fn non_string_mode_rejected() {
        ...
        let err = validate_rollback_disposition_v1(&v).unwrap_err();
        assert!(matches!(err, RollbackDispositionValidationError::InvalidModeType));
    }
    ...
    fn non_array_steps_rejected() {
        ...
        let err = validate_rollback_disposition_v1(&v).unwrap_err();
        assert!(matches!(err, RollbackDispositionValidationError::InvalidStepsType));
    }
    ...
    fn missing_steps_rejected() {
        ...
        let err = validate_rollback_disposition_v1(&v).unwrap_err();
        assert!(matches!(err, RollbackDispositionValidationError::MissingField("steps")));
    }
    ...
    fn invalid_data_loss_risk_rejected() {
        ...
        let err = validate_rollback_disposition_v1(&v).unwrap_err();
        assert!(matches!(err, RollbackDispositionValidationError::InvalidDataLossRisk(_)));
    }
    ...
    fn numeric_data_loss_risk_rejected() {
        ...
        let err = validate_rollback_disposition_v1(&v).unwrap_err();
        assert!(
            matches!(err, RollbackDispositionValidationError::InvalidDataLossRiskType),
    ...
    fn from_readback_json_succeeds_with_no_disposition_field() {
        ...
        let result = GqlP083RolloutContractReadback::from_readback_json(&readback, None);
        assert!(result.is_ok(), "from_readback_json must succeed when rollback_disposition is absent");
        let obj = result.unwrap();
        assert_eq!(
            obj.rollout_contract_rollback_disposition.inner().get("mode").and_then(|v| v.as_str()),
            Some("not_applicable"),
    ...
    fn wrap_strips_extra_properties_before_validation() {
        ...
        let wrapped = wrap_rollback_disposition_for_readback(&inline);
        assert!(wrapped.get("extra_field").is_none(), "extra_field must be stripped by wrap");
