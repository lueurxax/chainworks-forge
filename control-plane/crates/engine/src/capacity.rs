...
pub fn invoke_agent_capacity_config_from_lookup(
    ...
    let global_active_agent_executions =
        lookup_usize(&lookup, GLOBAL_CAP_ENV).unwrap_or(defaults.global_active_agent_executions);
    let per_run_active_agent_executions =
        lookup_usize(&lookup, PER_RUN_CAP_ENV).unwrap_or(defaults.per_run_active_agent_executions);

    let mut provider_caps = BTreeMap::new();
    for family in ProviderFamily::ALL {
        let env_key = provider_cap_env_key(family);
        let cap = lookup_usize(&lookup, &env_key).unwrap_or_else(|| defaults.provider_cap(family));
