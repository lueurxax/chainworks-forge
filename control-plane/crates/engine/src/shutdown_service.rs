...
use db::metrics::{
    record_p083_provider_cancellation_intent, record_p083_shutdown_duplicate_signal_suppressed,
...
pub async fn recover_shutdown_state(pool: &SqlitePool) -> Result<ShutdownRecoveryReport> {
    ...
    for intent in &requested_intents {
        let (should_hold, reason) =
            should_hold_requested_null_epoch_as_ambiguous(pool, &intent.provider_session_id).await;
    ...
    for session in &active_sessions {
        ...
        record_p083_provider_cancellation_intent(&session.provider, "held", "shutdown_recovery");
...
pub async fn suppress_duplicate_signal(
    ...
    if suppressed {
        ...
        warn!(
            provider_session_id,
            shutdown_epoch, signal_kind, generation, "P083 duplicate shutdown signal suppressed"
        );
    }
...
pub async fn dispatch_planned_shutdown_signals_scoped(
    ...
    let planned =
        load_planned_for_dispatch_scoped(pool, provider_session_id, shutdown_epoch).await?;
    if planned.is_empty() {
        return Ok(0);
    }
...
pub async fn dispatch_planned_shutdown_signals(pool: &SqlitePool) -> Result<usize> {
    ...
    if dispatched > 0 {
        info!(
            dispatched,
            "P083 dispatch: completed signal dispatch on startup"
        );
    }
...
/// logged as warnings.
async fn dispatch_signal_batch(
    pool: &SqlitePool,
    planned: &[PlannedSignalDetail],
) -> Result<usize> {
    ...
    for signal in planned {
        ...
        if signal.signal_kind == "kill" {
            ...
            match has_unobserved_graceful_signal(
                pool,
            ...
            IdentityVerificationResult::Match => {
                ...
                if send_ret == 0 {
                    ...
                    if !still_alive {
                        ...
                        graceful_just_issued
                            .insert((signal.provider_session_id.clone(), signal.shutdown_epoch));
                    }
                } else {
...
async fn resolve_provider_for_session(
    ...
    sqlx::query_scalar("SELECT provider FROM provider_sessions WHERE provider_session_id = ?1")
        .bind(provider_session_id)
        .fetch_optional(pool)
        .await
        .ok()
...
async fn mark_session_identity_ambiguous(pool: &SqlitePool, provider_session_id: &str) {
    ...
    if let Err(e) =
        provider_sessions::update_process_fate(pool, provider_session_id, "identity_ambiguous")
            .await
    {
        warn!(
...
async fn should_hold_requested_null_epoch_as_ambiguous(
    ...
    match (pid_opt, psi_opt) {
            ...
            if !alive {
                ...
                match verify_process_identity(pid, stored_identity) {
                    IdentityVerificationResult::Match => {
                        ...
                        (
                            false,
                            "process alive with matching identity; no hold needed",
                        )
                    }
                    IdentityVerificationResult::Mismatch => {
        ...
        _ => {
            ...
            (
                true,
                "process_start_identity empty or missing; unverifiable",
            )
        }
    }
}
...
async fn resolve_process_fate_for_session(
    ...
    sqlx::query_scalar("SELECT process_fate FROM provider_sessions WHERE provider_session_id = ?1")
        .bind(provider_session_id)
        .fetch_optional(pool)
        .await
        .ok()
...
mod p083_shutdown_service_tests {
    ...
    fn p083_shutdown_recovery_action_all_variants_constructible() {
        ...
        assert_eq!(
            actions.len(),
            6,
            "expected 6 distinct ShutdownRecoveryAction variants"
        );
    }
