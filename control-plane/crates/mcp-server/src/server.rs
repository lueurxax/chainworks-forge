        ...
        loop {
            ...
            if let Some(dup_key) = crate::http::find_duplicate_json_object_key(trimmed) {
                if dup_key == "__budget_exceeded__" {
                    ...
                    db::metrics::increment_counter_with_label("p080_mcp_parser_rejected_total", "canonicalization_budget_exceeded");
                    let resp = JsonRpcResponse {
                        jsonrpc: "2.0".to_string(),
                        id: None,
                        result: Some(serde_json::json!({
                    ...
                    db::metrics::increment_counter_with_label("p080_mcp_parser_rejected_total", "duplicate_key");
                    let resp = JsonRpcResponse {
                        jsonrpc: "2.0".to_string(),
                        id: None,
                        result: Some(serde_json::json!({
