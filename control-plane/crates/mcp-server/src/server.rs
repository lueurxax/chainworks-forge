...
impl McpServer {
    ...
    async fn dispatch_tool(
        ...
        } else if tool_name.starts_with("provider_session.")
            || tool_name.starts_with("p083.")
            || tool_name.starts_with("side_effects.")
        {
            tools::runs::execute(tool_name, params, pool, cmd, principal).await
        } else {
...
fn is_state_changing_tool(tool_name: &str) -> bool {
    matches!(
        ...
        // runs.cancel, stages.retry, approvals.resolve use is_p083_command_idempotency_tool.
    )
}
...
mod tests {
    ...
    async fn test_mcp_tools_call_response_includes_journal_id_in_content_text() {
        ...
        let payload = call_tool_and_parse(
            ...
            serde_json::json!({
                "run_id": run_id.to_string(),
                // P083 command_idempotency_contract_v1: runs.cancel requires a
                // UUIDv4 request_id (CallerRequestId), not the legacy P081
                // UUIDv7 idempotency_key.
