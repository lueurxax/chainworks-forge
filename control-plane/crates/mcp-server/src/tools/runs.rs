...
use db::repos::{
    artifact_contracts, cancel_late_output_overflow as cancel_late_output_overflow_repo, closeout,
    code_writer_completion_receipts, escalation as escalation_repo, legacy_discovery_overrides,
...
pub async fn execute(
    ...
    match tool_name {
        ...
        "runs.cancel" => {
            ...
            let caller = mcp_caller(&principal, "runs.cancel").with_request_id(request_id.clone());
            let cmd = Command::CancelRun(CancelRunCmd {
                run_id,
        ...
        "side_effects.force_reconcile" => {
            ...
            // SEC-P083-MED-003: stamp the validated CallerRequestId into CallerContext so
            // command_journal.request_id records the lifecycle CallerRequestId, not ambient HTTP.
            let caller = mcp_caller(&principal, "side_effects.force_reconcile")
                .with_request_id(request_id.clone());
            let commanded = cmd_handler
                .handle(
            ...
            let commanded = cmd_handler
                .handle(
                    Command::MarkProviderSessionProcessAbsent(
                        MarkProviderSessionProcessAbsentCmd {
                            provider_session_id: provider_session_id.clone(),
                            cancellation_epoch,
                            request_id: request_id.clone(),
                        },
...
async fn attach_implementation_self_assessment_summary(
    ...
    if let Some(object) = value.as_object_mut() {
        ...
        if is_operator {
            ...
            object.insert("rollout_contract_readback".to_string(), enriched_rollout);
            object.insert(
...
async fn enrich_rollout_contract_readback_p083(
    ...
    let rollout_contract_rollback_disposition = if let Some(inline) =
        obj.get("rollback_disposition").cloned()
    {
        let wrapped = graphql_server::types::p083::wrap_rollback_disposition_for_readback(&inline);
        graphql_server::types::p083::validate_rollback_disposition_v1(&wrapped).map_err(|e| {
            anyhow::anyhow!(
                "rollout_contract_rollback_disposition validation failed: {}",
                e
            )
        })?;
        wrapped
...
async fn load_latest_shutdown_queue_rank(pool: &SqlitePool, run_id: RunId) -> serde_json::Value {
