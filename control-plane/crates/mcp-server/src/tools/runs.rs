...
use db::repos::{
    artifact_contracts, cancel_late_output_overflow as cancel_late_output_overflow_repo,
    closeout, code_writer_completion_receipts, escalation as escalation_repo,
    legacy_discovery_overrides, projections, rollout_contract_checks, runs, side_effects,
};
use domain::commands::{
    CancelRunCmd, Command, ForceReconcileSideEffectCmd, KnowledgeCapsuleIgnoreCmd,
    MarkProviderSessionProcessAbsentCmd, MainSyncMode, MainSyncRecordRecoveryDecisionCmd,
    MainSyncRecoveryDecision, MainSyncRepairStateCmd, MainSyncRequestCmd, MainSyncRetryCmd,
    MainSyncSetRunOverrideCmd, MainSyncTriggerReason, P083RollbackExecutionCmd,
    P083SetEnforcementModeCmd, ProposalGateSettlementAction, RetryRunCmd, SettleProposalGateCmd,
...
pub fn tool_specs() -> Vec<McpTool> {
    vec![
        ...
        McpTool {
            ...
            description: "Cancel a run. Requires a CallerRequestId (lowercase UUIDv4) for \
                command_idempotency_contract_v1 replay safety (TTL=120s)."
                .to_string(),
            input_schema: serde_json::json!({
        ...
        McpTool {
            name: "provider_session.shutdown".to_string(),
            description: "P083: Initiate graceful shutdown of a provider session. \
                Requires operator principal and a CallerRequestId (lowercase UUIDv4) \
                for command_idempotency_contract_v1 replay safety."
                .to_string(),
            input_schema: serde_json::json!({
                "type": "object",
                "$schema": "https://json-schema.org/draft/2020-12/schema",
        ...
        McpTool {
            ...
            description: "P083: Execute rollback to permissive or disabled enforcement mode. \
                Requires operator principal and a CallerRequestId (lowercase UUIDv4) \
                for command_idempotency_contract_v1 replay safety."
                .to_string(),
            input_schema: serde_json::json!({
        ...
        McpTool {
            ...
            description: "P083: Set the execution-truth enforcement mode (disabled/permissive/enforce). \
                Requires operator principal and a CallerRequestId (lowercase UUIDv4) \
                for command_idempotency_contract_v1 replay safety."
                .to_string(),
            input_schema: serde_json::json!({
        ...
        McpTool {
            ...
            description: "P083: Force-reconcile a side effect to reconciled status with operator decision. \
                Requires operator principal and a CallerRequestId (lowercase UUIDv4) for \
                command_idempotency_contract_v1 replay safety (TTL=300s). \
                decision_json must be a valid JSON object with schema_version='side_effect_decision_v1'."
                .to_string(),
            input_schema: serde_json::json!({
                "type": "object",
                "$schema": "https://json-schema.org/draft/2020-12/schema",
                ...
                "properties": {
                    ...
                    "decision_json": {
                        "type": "string",
                        "maxLength": 65536,
                        "description": "JSON string with schema_version='side_effect_decision_v1' describing the operator reconciliation decision"
                    }
                }
            }),
            output_schema: Some(serde_json::json!({
        ...
        McpTool {
            ...
            description: "P083: Re-queue an AdvanceRun work item for a run that has failed or stalled. \
                Requires operator principal and a CallerRequestId (lowercase UUIDv4) \
                for command_idempotency_contract_v1 replay safety. \
                Rejects runs in terminal states (completed, cancelled, cancelling)."
                .to_string(),
            input_schema: serde_json::json!({
                "type": "object",
                "$schema": "https://json-schema.org/draft/2020-12/schema",
        ...
        McpTool {
            ...
            description: "P083: Confirm that the provider process for an identity-ambiguous hold \
                is absent. Atomically sets process_fate to absent_verified and re-opens the held \
                cancellation intent to requested. Requires operator principal and a CallerRequestId \
                (lowercase UUIDv4) for command_idempotency_contract_v1 replay safety."
                .to_string(),
            input_schema: serde_json::json!({
                "type": "object",
                "$schema": "https://json-schema.org/draft/2020-12/schema",
...
pub async fn execute(
    ...
    match tool_name {
        ...
        "runs.cancel" => {
            ...
            let cmd = Command::CancelRun(CancelRunCmd { run_id, request_id: Some(request_id.clone()) });
            let commanded = cmd_handler.handle(cmd, caller).await?;
            Ok(serde_json::json!({
                "cancelled": true,
        ...
        "provider_session.shutdown" => {
            ...
            // SEC-P083-MED-003: stamp the validated CallerRequestId into CallerContext so
            // command_journal.request_id is the P083 request_id, not the ambient HTTP X-Request-ID.
            let caller = mcp_caller(&principal, "provider_session.shutdown")
                .with_request_id(request_id.clone());
            let commanded = cmd_handler
                .handle(
            ...
            match commanded.result {
                ...
                _ => Err(anyhow::anyhow!("Unexpected result from ShutdownProviderSession")),
            }
        }
        ...
        "p083.rollback_execution" => {
            ...
            match commanded.result {
                ...
                _ => Err(anyhow::anyhow!("Unexpected result from P083RollbackExecution")),
            }
        }
        ...
        "p083.set_enforcement_mode" => {
            ...
            let enforcement_mode = params["enforcement_mode"]
                ...
                .to_string();
            if !matches!(enforcement_mode.as_str(), "disabled" | "permissive" | "enforce") {
            ...
            match commanded.result {
                ...
                _ => Err(anyhow::anyhow!("Unexpected result from P083SetEnforcementMode")),
            }
        }
        ...
        "runs.retry" => {
            ...
            let run_id = parse_run_id(&params)?;
            // SEC-P083-MED-003: stamp the validated CallerRequestId into CallerContext so
            // command_journal.request_id records the lifecycle CallerRequestId, not ambient HTTP.
            let caller = mcp_caller(&principal, "runs.retry").with_request_id(request_id.clone());
            let commanded = cmd_handler
                .handle(
        ...
        "side_effects.force_reconcile" => {
            ...
            let decision_json = params["decision_json"]
                .as_str()
                .ok_or_else(|| anyhow::anyhow!("Missing 'decision_json'"))?
                .to_string();
            // SEC-P083-MED-003: stamp the validated CallerRequestId into CallerContext so
            // command_journal.request_id records the lifecycle CallerRequestId, not ambient HTTP.
            let caller = mcp_caller(&principal, "side_effects.force_reconcile").with_request_id(request_id.clone());
            let commanded = cmd_handler
                .handle(
            ...
            match commanded.result {
                ...
                _ => Err(anyhow::anyhow!("Unexpected result from ForceReconcileSideEffect")),
            }
        }
        ...
        "provider_session.mark_process_absent" => {
            ...
            // SEC-P083-MED-003: stamp the validated CallerRequestId into CallerContext so
            // command_journal.request_id is the P083 request_id, not the ambient HTTP X-Request-ID.
            let caller = mcp_caller(&principal, "provider_session.mark_process_absent")
                .with_request_id(request_id.clone());
            let commanded = cmd_handler
                .handle(
                    Command::MarkProviderSessionProcessAbsent(MarkProviderSessionProcessAbsentCmd {
                        provider_session_id: provider_session_id.clone(),
                        cancellation_epoch,
                        request_id: request_id.clone(),
                    }),
            ...
            match commanded.result {
                ...
                _ => Err(anyhow::anyhow!("Unexpected result from MarkProviderSessionProcessAbsent")),
            }
        }
...
async fn attach_implementation_self_assessment_summary(
    ...
    if let Some(object) = value.as_object_mut() {
        ...
        if is_operator {
            ...
            let mut enriched_rollout = rollout_contract_readback.unwrap_or(serde_json::Value::Null);
            if let Some(run_id) = run_id {
                enrich_rollout_contract_readback_p083(pool, &mut enriched_rollout, run_id).await?;
            }
            object.insert(
...
/// P083 HARDEN-002 + rollout_readback_api_parity_v1: Enrich the MCP rollout_contract_readback
/// with all required rollout_contract_* prefixed fields and p083_shutdown_queue_rank.
///
/// Adds the full rollout_readback_api_parity_v1 key set by aliasing existing legacy fields
/// with the required rollout_contract_* prefix, plus:
/// - `rollout_contract_rollback_disposition`: wrapped+validated rollback_disposition_v1 object.
/// - `p083_shutdown_queue_rank`: [integer, null] — latest shutdown queue_rank per mcp_schema_queue_rank_rule.
///
/// Mutates the readback JSON in-place; no-op if rollout_contract_readback is null.
async fn enrich_rollout_contract_readback_p083(
    pool: &SqlitePool,
    readback: &mut serde_json::Value,
    run_id: RunId,
) -> anyhow::Result<()> {
    let Some(obj) = readback.as_object_mut() else {
        return Ok(());
    };

    // Add rollout_contract_* prefixed aliases for rollout_readback_api_parity_v1 compliance.
    // The DB readback emits legacy unprefixed keys (status, enforcement_mode, etc.);
    // the parity contract requires rollout_contract_* keys in all lanes.
    ...
    // Wrap rollback_disposition → rollout_contract_rollback_disposition with schema_version.
    // Per mcp_schema_rule: the value is a rollback_disposition_v1 data object (schema_version,
    // mode, data_loss_risk, steps). JSON Schema metadata ($schema, additionalProperties) must
    // NOT be embedded in the data value — those belong in the MCP tool's input/output schema.
    // Per output_validation_rule: when no authoritative rollback_disposition exists the field
    // is null — do NOT fabricate a synthetic not_applicable object.
    // Per rollout_readback_api_parity_v1: fail closed on invalid disposition — do not null-coalesce.
    // additionalProperties=false: use wrap_rollback_disposition_for_readback which strictly
    // whitelists only mode, data_loss_risk, steps, and sets schema_version — all extra keys stripped.
    let rollout_contract_rollback_disposition = if let Some(inline) = obj.get("rollback_disposition").cloned() {
        let wrapped = graphql_server::types::p083::wrap_rollback_disposition_for_readback(&inline);
    ...
    // p083_shutdown_queue_rank: load the latest queue_rank for unrecovered receipts for this run.
    let queue_rank: serde_json::Value = load_latest_shutdown_queue_rank(pool, run_id).await;
    obj.insert("p083_shutdown_queue_rank".to_string(), queue_rank);

    // MISSING-009: P083 migration readback parity.
    // Every MCP rollout readback lane must include applied_migrations with logical_id, filename,
    // sha256, dependencies, applied_at, schema_version, state, and verification_query_result.
    let migration_rows = rollout_contract_checks::p083_migration_readback(pool)
        .await
        .unwrap_or_default();
    ...
    obj.insert(
        ...
        serde_json::Value::Array(migration_json),
    );

    // MISSING-004 / post_cancel_late_output_contract_v1.readback_fields: include
    // p083_cancel_late_output_overflow in MCP rollout readback lane per rollout_readback_api_parity_v1.
    // Fields: scope, normalized_run_id, normalized_provider_session_id, cancellation_epoch,
    // overflow_kind, dropped_message_count, dropped_byte_count, quarantine_uri,
    // reservation_release_state, projection_mutation_blocked.
...
/// Load the latest queue_rank from shutdown_interrupted_receipts for any provider session
/// linked to this run. Returns null if no unrecovered queued_no_signal receipts exist.
async fn load_latest_shutdown_queue_rank(
    pool: &SqlitePool,
    run_id: RunId,
) -> serde_json::Value {
    ...
    match result {
        Some(rank) => serde_json::Value::Number(rank.into()),
...
/// p083_rollback_audit.reason. Reject oversized inputs (DB-bloat / OOM risk) and control
/// characters (log injection / credential-pattern risk). Returns a bounded, printable string.
///
/// `max_bytes` should match the maxLength declared in the MCP tool input_schema.
fn validate_p083_reason(reason: &str, max_bytes: usize) -> anyhow::Result<()> {
    if reason.len() > max_bytes {
        anyhow::bail!(
            "reason exceeds maximum length of {} bytes (got {})",
            max_bytes,
            reason.len()
        );
    }
    // Reject C0/C1 control characters (except common whitespace: space, \t, \n, \r).
    for (i, ch) in reason.char_indices() {
        if ch.is_control() && !matches!(ch, ' ' | '\t' | '\n' | '\r') {
            anyhow::bail!(
                "reason contains a disallowed control character at byte offset {i} (U+{:04X})",
                ch as u32
            );
        }
    }
