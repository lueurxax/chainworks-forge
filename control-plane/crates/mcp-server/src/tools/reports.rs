...
use db::repos::{
    ...
    agent_execution_runtime_receipts, agent_executions, artifact_contracts, artifacts,
    cancel_late_output_overflow as cancel_late_output_overflow_repo, closeout,
...
pub async fn execute_with_readback_config(
    ...
    if let Some(run_id) = run_id {
        let fields =
            super::run_continuations::compact_fields(pool, run_id, principal, config).await?;
        let fields = fields.as_object().expect("compact object");
        if let Some(reports) = value["reports"].as_array_mut() {
...
pub(crate) async fn rollout_contract_readback_json(
    ...
    let rollout_contract_rollback_disposition = if let Some(inline) =
        merged.get("rollback_disposition").cloned()
    {
    ...
    let applied_migrations = rollout_contract_checks::p083_migration_readback(pool)
        .await
        .map(|rows| serde_json::to_value(rows).unwrap_or(serde_json::Value::Array(vec![])))
        .unwrap_or(serde_json::Value::Array(vec![]));
