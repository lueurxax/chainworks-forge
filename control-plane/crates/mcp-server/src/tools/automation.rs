...
fn readback_envelope(
    ...
    let (ledger_path, budget_state_path, known_issue_catalog_path,
         generated_markdown_catalog_path, lock_path, rollup_report_path) = if is_operator {
        (
            Value::String(paths.ledger_path.to_string_lossy().into_owned()),
            Value::String(paths.budget_state_path.to_string_lossy().into_owned()),
            Value::String(paths.known_issue_catalog_path.to_string_lossy().into_owned()),
            Value::String(paths.generated_markdown_catalog_path.to_string_lossy().into_owned()),
...
mod tests {
    ...
    fn p076_auto_retry_latest_observer_paths_redacted() {
        ...
        assert_eq!(payload["observations"][0]["next_systemic_action"], Value::Null);
        assert_eq!(payload["observations"][0]["observation_path"], Value::Null);
        // Non-sensitive aggregate fields are still present.
        assert_eq!(payload["observations"][0]["run_id"], "run-1");
        assert_eq!(payload["observations"][0]["policy_decision"], "observe_only");
    }
