...
mod tests {
    ...
    fn proposal_017_lint_accepts_common_complementary_transition_pairs() {
        ...
        assert!(!conditions_can_match_simultaneously(
            "implementation_review_summary.status == vars.implementation_target_status and implementation_review_summary.status != 'release_evidence_blocked'",
            "implementation_review_summary.status != vars.implementation_target_status or implementation_review_summary.status == 'release_evidence_blocked'"
        ));
    }
}
