...
fn p017_phase_b_dogfood_exit_record_has_operator_approved_flag_gated_evidence() {
    let record = read_json("docs/proposals/017-evidence/phase-b-dogfood-exit-record.json");
...
fn p017_zero_bundled_simultaneous_findings_pass_without_known_issues_records() {
    let known_issues =
        read_json("docs/proposals/017-evidence/phase-a-known-issues-migration-records.json");
