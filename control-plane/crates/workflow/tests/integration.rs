...
fn test_parse_full_mvp_live_workflow() {
    ...
    assert_eq!(wf.states.len(), 22, "full-mvp-live has 22 states");
    ...
    assert!(
        transitions.iter().any(|transition| {
            transition.to == "state_9_external_evidence_blocked"
                ...
                && transition
                    .when
                    .contains("has_no_release_blocking_external_blockers == false")
                && transition
                    .when
    ...
    assert!(
        transitions.iter().any(|transition| {
            transition.to == "state_11_manual_release"
                && transition
                    .when
                    .contains("has_release_blocking_external_blockers == false")
                && transition
                    .when
...
#[test]
fn implementation_agents_consume_control_plane_git_evidence_without_direct_git_tools() {
    let cat_path = format!("{}/agents/agents.yaml", fixtures_dir());
    let cat = workflow::catalog::load(&cat_path).expect("should parse agent catalog YAML");
    let agents = cat.agents.as_ref().expect("has agents");

    let code_writer = agents
        .iter()
        .find(|agent| agent.id == "code_writer")
        .expect("code_writer agent");
    assert!(
        !code_writer.outputs.as_ref().is_some_and(|outputs| outputs
            .iter()
            .any(|output| output == "changed_files_manifest")),
        "changed_files_manifest is generated by the control plane, not the code writer"
    );
    ...
    for agent_id in [
        ...
        let agent = agents
            .iter()
            .find(|agent| agent.id == agent_id)
            .unwrap_or_else(|| panic!("missing {agent_id} agent"));
        let direct_git_tools = agent
            .required_tools
            .as_deref()
            .unwrap_or_default()
            .iter()
            .filter(|tool| tool.trim_start().starts_with("git "))
            .collect::<Vec<_>>();
        assert!(
            direct_git_tools.is_empty(),
            "{agent_id} must consume control-plane Git evidence instead of direct Git tools: {direct_git_tools:?}"
        );
    }
    ...
    for agent_id in [
        ...
        assert!(
            agent
                .inputs
                .as_ref()
                .is_some_and(|inputs| inputs.iter().any(|input| input == "changed_files_manifest")),
            "{agent_id} must consume the canonical changed_files_manifest"
        );
        let prompt = agent.prompt.as_deref().unwrap_or_default();
        assert!(
            prompt.contains("Do not run `git status`, `git diff`, or `git rev-parse`")
                && prompt.contains("do not read `.git`"),
            "{agent_id} prompt must prohibit direct Git access"
        );
    }

    let code_writer_prompt = code_writer.prompt.as_deref().unwrap_or_default();
    for forbidden_command in ["git status", "git diff", "git rev-parse"] {
        assert!(
            code_writer_prompt.contains(forbidden_command),
            "code_writer prompt must explicitly prohibit {forbidden_command}"
        );
    }
    assert!(
        code_writer_prompt.contains("Git evidence is generated by the control plane"),
        "code_writer prompt must name the control plane as Git evidence owner"
    );

    let raw_catalog: serde_yaml::Value =
        serde_yaml::from_str(&fs::read_to_string(&cat_path).unwrap()).unwrap();
    for profile_id in ["CODE_WRITE", "RO_VERIFY", "RO_PREPUSH_VERIFY"] {
        let profile = &raw_catalog["permission_profiles"][profile_id];
        for capability in ["status", "diff", "checkout"] {
            assert_eq!(
                profile["git"][capability].as_bool(),
                Some(false),
                "{profile_id}.git.{capability} must be disabled"
            );
        }
        let direct_git_shell_entries = profile["shell"]["allow"]
            .as_sequence()
            .into_iter()
            .flatten()
            .filter_map(serde_yaml::Value::as_str)
            .filter(|entry| entry.trim_start().starts_with("git "))
            .collect::<Vec<_>>();
        assert!(
            direct_git_shell_entries.is_empty(),
            "{profile_id} shell allowlist must not expose direct Git: {direct_git_shell_entries:?}"
        );
        let denied_paths = profile["filesystem"]["deny"]
            .as_sequence()
            .into_iter()
            .flatten()
            .filter_map(serde_yaml::Value::as_str)
            .collect::<Vec<_>>();
        assert!(
            denied_paths.iter().any(|path| path.ends_with("/.git")),
            "{profile_id} must deny the linked-worktree .git file itself"
        );
    }

    let workflow_path = format!("{}/workflows/full-mvp-live.yaml", fixtures_dir());
    let plan = compiler::compile(&workflow_path, &cat_path)
        .expect("full production workflow should compile");
    let code_writer_tasks = plan
        .states
        .values()
        .flat_map(|state| state.tasks.iter())
        .filter(|task| task.agent.agent_id == "code_writer")
        .collect::<Vec<_>>();
    assert!(
        !code_writer_tasks.is_empty(),
        "full workflow has code_writer tasks"
    );
    assert!(
        code_writer_tasks.iter().all(|task| task
            .outputs
            .iter()
            .any(|output| output == "changed_files_manifest")),
        "workflow must retain changed_files_manifest as an engine-owned declared output"
    );
}
...
fn test_compile_full_mvp_live_plan() {
    ...
    assert_eq!(
        ...
        "state_4 owner=lead_orchestrator uses the canonical Codex orchestration profile"
    );
    assert_eq!(
        s4.system_task
            .as_ref()
...
fn implementation_review_transitions_use_aggregate_review_summary_not_self_assessment() {
    for workflow_name in ["workflow.yaml", "full-mvp-live.yaml"] {
        ...
        let refine_transition = review_state
            .transitions
            .iter()
            .find(|transition| transition.condition.contains("return_to_code_refine"))
...
#[test]
fn compile_from_snapshot_json_rejects_policy_snapshot_without_format_version() {
    let workflow = r#"
workflow:
  id: steward-workflow
  family: mvp_live
...
"#,
    ...
    let workflow_path = write_temp_fixture("snapshot-policy-workflow.yaml", workflow);
    let catalog_path = write_temp_fixture("snapshot-policy-catalog.yaml", &catalog);
    let plan = compiler::compile(&workflow_path, &catalog_path).expect("plan compiles");

    let mut catalog_snapshot: serde_json::Value =
        serde_json::from_str(&plan.catalog_snapshot_json).expect("snapshot JSON parses");
    catalog_snapshot
        .as_object_mut()
        .expect("catalog snapshot is an object")
        .remove("catalog_snapshot_format_version");
    let catalog_snapshot_json =
        serde_json::to_string(&catalog_snapshot).expect("snapshot JSON serializes");
    ...
    let err = result.expect_err("snapshot without format version should fail");
    assert!(
        err.to_string()
            .contains("frozen_snapshot_contract_incompatible"),
        "error should identify as frozen_snapshot_contract_incompatible: {err}"
    );
}
