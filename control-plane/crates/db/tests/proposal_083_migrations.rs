...
async fn artifact_lineage_non_report_row_accepts_null_report_kind() {
    ...
    assert!(
        result.is_ok(),
        "non-report row with null report_kind should succeed: {result:?}"
    );
}
...
async fn artifact_lineage_active_report_with_valid_report_kind_accepted() {
    ...
    assert!(
        result.is_ok(),
        "active report with valid report_kind should succeed: {result:?}"
    );
}
...
async fn artifact_lineage_active_report_with_null_report_kind_rejected() {
    ...
    assert!(
        result.is_err(),
        "active report with NULL report_kind must be rejected"
    );
}
...
async fn artifact_lineage_active_report_with_unknown_report_kind_rejected() {
    ...
    assert!(
        result.is_err(),
        "active report with unbounded report_kind must be rejected"
    );
}
...
async fn artifact_lineage_inactive_report_accepts_null_report_kind() {
    ...
    assert!(
        result.is_ok(),
        "inactive report with NULL report_kind should succeed: {result:?}"
    );
}
...
async fn artifact_lineage_active_report_kind_unique_per_run() {
    ...
    assert!(
        result.is_err(),
        "duplicate active report_kind per run must be rejected by unique index"
    );
}
...
async fn command_idempotency_unique_active_per_request() {
    ...
    assert!(
        result.is_err(),
        "duplicate active lease for same (principal, request) must be rejected"
    );
}
...
async fn shutdown_receipts_queue_rank_null_for_non_queued() {
    ...
    assert!(
        result.is_err(),
        "non-queued receipt with non-null queue_rank must be rejected"
    );
}
...
async fn shutdown_receipts_valid_queued_no_signal_with_queue_rank() {
    ...
    assert!(
        result.is_ok(),
        "queued_no_signal with non-null queue_rank should succeed: {result:?}"
    );
}
...
async fn shutdown_receipts_valid_grace_deadline_expired_null_queue_rank() {
    ...
    assert!(
        result.is_ok(),
        "grace_deadline_expired with NULL queue_rank should succeed: {result:?}"
    );
}
...
async fn shutdown_signal_side_effects_unique_generation_per_session_epoch_kind() {
    ...
    assert!(
        result.is_err(),
        "duplicate (session, epoch, kind, generation) must be rejected"
    );
}
...
async fn cancel_late_output_overflow_unique_latch_key() {
    ...
    assert!(
        result.is_err(),
        "duplicate latch key must be rejected by unique index"
    );
}
...
async fn cancel_late_output_overflow_normalized_columns_are_generated() {
    ...
    assert_eq!(
        norm_run, "run-upper",
        "normalized_run_id should be lowercased and trimmed"
    );
    assert_eq!(
...
async fn p083_enforcement_transition_journal_transitioning_no_commit_marker() {
    ...
    assert!(
        result.is_ok(),
        "valid transitioning row should be accepted: {result:?}"
    );
}
...
async fn v1_verification_query_returns_zero_rows() {
    ...
    assert_eq!(
        count, 0,
        "V1 verification query must return zero rows on fresh DB"
    );
}
...
async fn v2_verification_query_returns_zero_rows() {
    ...
    assert_eq!(
        count, 0,
        "V2 verification query must return zero rows on fresh DB"
    );
}
...
async fn v3_verification_query_returns_zero_rows() {
    ...
    assert_eq!(
        count, 0,
        "V3 verification query must return zero rows on fresh DB"
    );
}
...
async fn v4_verification_query_returns_zero_rows() {
    ...
    assert_eq!(
        count, 0,
        "V4 verification query must return zero rows on fresh DB"
    );
}
...
async fn v5_verification_query_returns_zero_rows() {
    ...
    assert_eq!(
        count, 0,
        "V5 verification query must return zero rows on fresh DB"
    );
}
...
async fn v7_verification_query_returns_zero_rows() {
    ...
    assert_eq!(
        count, 0,
        "V7 verification query must return zero rows on fresh DB"
    );
}
...
async fn command_idempotency_acquire_and_commit_round_trip() {
    ...
    let committed =
        command_idempotency::commit(&pool, "principal-1", "req-1", 1, r#"{"result":"ok"}"#)
            .await
            .unwrap();
...
async fn command_idempotency_fail_and_abandon() {
    ...
    let failed = command_idempotency::fail_lease(&pool, "principal-2", "req-2", 1, "CONFLICT")
        .await
        .unwrap();
    assert!(failed);
    ...
    assert!(
        active.is_none(),
        "abandoned lease must not appear as active"
    );
}
...
async fn provider_sessions_update_process_fate_identity_ambiguous() {
    ...
    assert!(ambiguous
        .iter()
        .any(|s| s.provider_session_id == "psess-repo2"));
}
...
async fn shutdown_signal_side_effects_planned_to_issued_to_observed() {
    ...
    let claimed = shutdown_receipts::mark_signal_dispatching(&pool, "sess-sg1", 1, "graceful", 1)
        .await
        .unwrap();
    assert!(claimed, "first claim must succeed");

    // Duplicate claim must fail (concurrent-dispatcher suppression).
    let dup_claim = shutdown_receipts::mark_signal_dispatching(&pool, "sess-sg1", 1, "graceful", 1)
        .await
        .unwrap();
...
async fn shutdown_signal_dispatching_skipped_by_recovery() {
    ...
    let claimed = shutdown_receipts::mark_signal_dispatching(&pool, "sess-rec1", 5, "graceful", 1)
        .await
        .unwrap();
    assert!(claimed);

    // Row must still appear as active (it is in progress, not terminal).
    let still_active = shutdown_receipts::find_active_signal(&pool, "sess-rec1", 5, "graceful", 1)
        .await
        .unwrap();
    assert!(
...
async fn shutdown_signal_identity_mismatch_transition() {
    ...
    let ok = shutdown_receipts::mark_signal_identity_mismatch(
        &pool,
        "sess-sg2",
        1,
        "kill",
...
fn p083_required_metric_names_are_declared_and_recordable() {
    use db::metrics::{
        self, record_p083_artifact_lineage_projection_integrity,
        record_p083_cancel_late_output_dropped, record_p083_cancel_late_output_overflow,
        record_p083_command_idempotency_lease_acquire, record_p083_command_idempotency_replay,
        record_p083_enforcement_mode_transition, record_p083_provider_cancellation_intent,
        record_p083_provider_session_lifecycle, record_p083_rollback_execution,
        record_p083_rollout_contract_lint, record_p083_rollout_contract_run_start_block,
        record_p083_shutdown_duplicate_signal_suppressed, record_p083_shutdown_interrupted_receipt,
        P083_REQUIRED_METRICS,
    };
...
fn p083_metric_domain_conformance_valid_labels_do_not_collapse() {
    use db::metrics::{
        get_counter, record_p083_artifact_lineage_projection_integrity,
        record_p083_cancel_late_output_dropped, record_p083_cancel_late_output_overflow,
        record_p083_command_idempotency_lease_acquire, record_p083_command_idempotency_replay,
        record_p083_enforcement_mode_transition, record_p083_provider_cancellation_intent,
        record_p083_provider_session_lifecycle, record_p083_rollback_execution,
    ...
    record_p083_artifact_lineage_projection_integrity("graphql", "fresh");
    assert!(
        get_counter(key) > before,
        "surface=graphql,state=fresh must not collapse to unknown"
    );
    ...
    assert!(
        get_counter(key) > before,
        "lifecycle_state=registered must not collapse to unknown"
    );
    ...
    assert!(
        get_counter(key) > before,
        "provider=claude must not collapse to unknown"
    );
    ...
    assert!(
        get_counter(key) > before,
        "outcome=acquired must not collapse to unknown"
    );
    ...
    assert!(
        get_counter(key) > before,
        "outcome=replayed must not collapse to unknown"
    );
    ...
    assert!(
        get_counter(key) > before,
        "interrupted_state=grace_deadline_expired must not collapse"
    );
    ...
    assert!(
        get_counter(key) > before,
        "overflow_kind=message_count must not collapse to unknown"
    );

    let key =
        "cancel_late_output_dropped_total:provider=gemini,scope=run,overflow_kind=session_bytes";
    let before = get_counter(key);
    record_p083_cancel_late_output_dropped("gemini", "run", "session_bytes");
    assert!(
        get_counter(key) > before,
        "overflow_kind=session_bytes must not collapse to unknown"
    );
    ...
    assert!(
        get_counter(key) > before,
        "proposal_id=P083,status=pass must not collapse to unknown"
    );
    ...
    assert!(
        get_counter(key) > before,
        "reason=hold_condition_present must not collapse to unknown"
    );
    ...
    assert!(
        get_counter(key) > before,
        "transition=disabled_to_permissive must not collapse to unknown"
    );
    ...
    assert!(
        get_counter(key) > before,
        "transition=disabled_to_enforce_denied must not collapse to unknown"
    );

    // Approved domain: status=pass/fail/..., reason from hold-condition set.
    let key =
        "p083_rollback_execution_total:action=rollback_disable,status=pass,reason=gate_failed";
    let before = get_counter(key);
    record_p083_rollback_execution("rollback_disable", "pass", "gate_failed");
    assert!(
        get_counter(key) > before,
        "action=rollback_disable must not collapse to unknown"
    );
    ...
    assert!(
        get_counter(key) > before,
        "intent_state=requested must not collapse to unknown"
    );
    ...
    assert!(
        get_counter(key) > before,
        "intent_state=shutdown_started must not collapse to unknown"
    );
    ...
    assert!(
        get_counter(key) > before,
        "intent_state=held must not collapse to unknown"
    );
}
