...
pub async fn seed_rollout_control_if_absent(pool: &SqlitePool) -> Result<usize> {
    ...
    let mut tx = pool
        .begin()
        .await
        .context("p080 seed_rollout_control begin tx")?;
    ...
    tx.commit()
        .await
        .context("p080 seed_rollout_control commit")?;
    Ok(seeded)
}
...
pub async fn p080_release_receipt_section(pool: &SqlitePool, run_id: &str) -> serde_json::Value {
    ...
    for (side_effect_status, hold_reason, readback_json, _integrity) in
        all_rows.iter().take(RECEIPT_ROW_CAP)
    {
    ...
    for (key, val) in obj {
        ...
        if let serde_json::Value::String(s) = val {
            match key.as_str() {
                ...
                "side_effect_status" => {
                    const ALLOWED: &[&str] = &["not_applicable", "retry_safe", "unsafe", "unknown"];
                    if !ALLOWED.contains(&s.as_str()) {
                        return Err(anyhow::anyhow!(
