...
impl StoredRolloutContractCheck {
    ...
    pub fn operator_readback_json_for_lane(&self, source_lane: &str) -> serde_json::Value {
        ...
        let safe_rollback_disposition = filter_rollback_disposition_readback(&self.rollback_disposition);
...
pub async fn p083_migration_readback(pool: &SqlitePool) -> Result<Vec<P083MigrationReadbackRow>> {
    ...
    for desc in P083_MIGRATIONS {
        ...
        let (state, applied_at, verification_result) = if let Some(installed_on) =
            applied_map.get(&desc.version)
        {
            ...
            let vq_result = if is_count_query {
                match sqlx::query(desc.verification_query)
                    .fetch_one(pool)
                    .await
                {
                    Ok(row) => {
