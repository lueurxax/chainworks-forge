...
const CANCELLATION_REASON_DOMAIN: &[&str] =
    &["operator_cancel", "backpressure_cutoff", "shutdown_recovery"];
...
const IDEMPOTENCY_OUTCOME_DOMAIN: &[&str] =
    &["acquired", "replayed", "denied", "committed", "failed", "abandoned", "expired_reacquired"];
...
const SURFACE_DOMAIN: &[&str] =
    &["graphql", "mcp", "run_report", "release_receipt", "swift_ui"];
...
const OVERFLOW_KIND_DOMAIN: &[&str] =
    &["message_count", "session_bytes", "elapsed_time", "run_bytes", "global_bytes"];
...
fn bounded_label<'a>(value: &'a str, domain: &[&str], metric: &str, label_name: &str) -> Option<&'a str> {
...
pub fn record_p083_artifact_lineage_projection_integrity(surface: &str, state: &str) {
    let Some(s) = bounded_label(surface, SURFACE_DOMAIN, "artifact_lineage_projection_integrity_total", "surface") else { return; };
    let Some(st) = bounded_label(state, PROJECTION_STATE_DOMAIN, "artifact_lineage_projection_integrity_total", "state") else { return; };
...
pub fn record_p083_provider_session_lifecycle(provider: &str, lifecycle_state: &str) {
    let Some(p) = bounded_label(provider, PROVIDER_DOMAIN, "provider_session_lifecycle_total", "provider") else { return; };
    let Some(ls) = bounded_label(lifecycle_state, LIFECYCLE_STATE_DOMAIN, "provider_session_lifecycle_total", "lifecycle_state") else { return; };
...
pub fn record_p083_command_idempotency_lease_acquire(command: &str, outcome: &str) {
    let Some(c) = bounded_label(command, COMMAND_DOMAIN, "command_idempotency_lease_acquire_total", "command") else { return; };
    let Some(o) = bounded_label(outcome, IDEMPOTENCY_OUTCOME_DOMAIN, "command_idempotency_lease_acquire_total", "outcome") else { return; };
...
pub fn record_p083_command_idempotency_replay(command: &str, outcome: &str) {
    let Some(c) = bounded_label(command, COMMAND_DOMAIN, "command_idempotency_replay_total", "command") else { return; };
    let Some(o) = bounded_label(outcome, IDEMPOTENCY_OUTCOME_DOMAIN, "command_idempotency_replay_total", "outcome") else { return; };
...
pub fn record_p083_shutdown_interrupted_receipt(provider: &str, interrupted_state: &str) {
    let Some(p) = bounded_label(provider, PROVIDER_DOMAIN, "shutdown_interrupted_receipt_total", "provider") else { return; };
    let Some(state) = bounded_label(interrupted_state, INTERRUPTED_STATE_DOMAIN, "shutdown_interrupted_receipt_total", "interrupted_state") else { return; };
...
pub fn record_p083_shutdown_duplicate_signal_suppressed(provider: &str) {
    let Some(p) = bounded_label(provider, PROVIDER_DOMAIN, "shutdown_duplicate_signal_suppressed_total", "provider") else { return; };
...
pub fn record_p083_cancel_late_output_overflow(
    provider: &str,
    scope: &str,
    overflow_kind: &str,
) {
    let Some(p) = bounded_label(provider, PROVIDER_DOMAIN, "cancel_late_output_overflow_total", "provider") else { return; };
    let Some(sc) = bounded_label(scope, SCOPE_DOMAIN, "cancel_late_output_overflow_total", "scope") else { return; };
    let Some(ok) = bounded_label(overflow_kind, OVERFLOW_KIND_DOMAIN, "cancel_late_output_overflow_total", "overflow_kind") else { return; };
...
pub fn record_p083_cancel_late_output_dropped(
    provider: &str,
    scope: &str,
    overflow_kind: &str,
) {
    let Some(p) = bounded_label(provider, PROVIDER_DOMAIN, "cancel_late_output_dropped_total", "provider") else { return; };
    let Some(sc) = bounded_label(scope, SCOPE_DOMAIN, "cancel_late_output_dropped_total", "scope") else { return; };
    let Some(ok) = bounded_label(overflow_kind, OVERFLOW_KIND_DOMAIN, "cancel_late_output_dropped_total", "overflow_kind") else { return; };
...
pub fn record_p083_rollout_contract_lint(
    ...
    let Some(pid) = bounded_label(proposal_id, PROPOSAL_ID_DOMAIN, "rollout_contract_lint_total", "proposal_id") else { return; };
    let Some(st) = bounded_label(status, ROLLOUT_STATUS_DOMAIN, "rollout_contract_lint_total", "status") else { return; };
    ...
    match failure_reason {
        Some(fr_raw) => {
            let Some(fr) = bounded_label(fr_raw, FAILURE_REASON_DOMAIN, "rollout_contract_lint_total", "failure_reason") else { return; };
            increment_counter_with_label(
                "rollout_contract_lint_total",
...
pub fn record_p083_rollout_contract_run_start_block(
    ...
    let Some(pid) = bounded_label(proposal_id, PROPOSAL_ID_DOMAIN, "rollout_contract_run_start_block_total", "proposal_id") else { return; };
    let Some(r) = bounded_label(reason, BLOCK_REASON_DOMAIN, "rollout_contract_run_start_block_total", "reason") else { return; };
    let Some(em) = bounded_label(enforcement_mode, ENFORCEMENT_MODE_DOMAIN, "rollout_contract_run_start_block_total", "enforcement_mode") else { return; };
...
pub fn record_p083_enforcement_mode_transition(transition: &str, enforcement_mode: &str) {
    let Some(t) = bounded_label(transition, TRANSITION_DOMAIN, "p083_enforcement_mode_transition_total", "transition") else { return; };
    let Some(em) = bounded_label(enforcement_mode, ENFORCEMENT_MODE_DOMAIN, "p083_enforcement_mode_transition_total", "enforcement_mode") else { return; };
...
pub fn record_p083_rollback_execution(action: &str, status: &str, reason: &str) {
    let Some(a) = bounded_label(action, ROLLBACK_ACTION_DOMAIN, "p083_rollback_execution_total", "action") else { return; };
    let Some(s) = bounded_label(status, ROLLBACK_STATUS_DOMAIN, "p083_rollback_execution_total", "status") else { return; };
    let Some(r) = bounded_label(reason, ROLLBACK_REASON_DOMAIN, "p083_rollback_execution_total", "reason") else { return; };
...
pub fn record_p083_provider_cancellation_intent(
    ...
    let Some(p) = bounded_label(provider, PROVIDER_DOMAIN, "provider_cancellation_intent_total", "provider") else { return; };
    let Some(is) = bounded_label(intent_state, INTENT_STATE_DOMAIN, "provider_cancellation_intent_total", "intent_state") else { return; };
    let Some(reason) = bounded_label(cancellation_reason, CANCELLATION_REASON_DOMAIN, "provider_cancellation_intent_total", "cancellation_reason") else { return; };
