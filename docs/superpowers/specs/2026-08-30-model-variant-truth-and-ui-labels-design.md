...
The last sentence applies only to snapshots that contain
`provider_fallback_policy_v1`. A pre-v2 snapshot with no policy uses the
separately frozen `LegacyFallbackSelectorV0`; it is never passed to the V1
selector and is not upgraded in place. V0 reads only backend profiles present in
that run's frozen catalog and the following exact ordered candidate table,
which is generated byte-for-byte from the retained legacy implementation:

| Eligible legacy lane/source | Ordered candidate profile IDs |
|---|---|
| Junie code writer requiring implementation output | `claude_builder_high` |
| lead orchestrator producing `proposal_review_summary` | `codex_writer_high`, `codex_architect_high` |
| proposal writer from Codex | `claude_writer_high`, `claude_product_high` |
| proposal writer from another provider | `codex_writer_high`, `codex_architect_high` |
| design proposal-review from Gemini | `claude_design_medium`, `claude_product_high`, `codex_architect_high` |
| other proposal-review from Gemini | `claude_product_high`, `codex_architect_high` |
| design proposal-review from Codex | `claude_design_medium`, `claude_product_high` |
| other proposal-review from Codex | `claude_product_high`, `claude_design_medium` |
| proposal-review from another provider | `codex_architect_high`, `codex_writer_high` |
| docs guardian from Gemini | `claude_docs_medium`, `claude_design_medium`, `codex_architect_high` |
| docs guardian from another provider | `gemini_docs_flash`, `claude_docs_medium`, `codex_architect_high` |
| security checker from Claude | `codex_architect_high`, `codex_audit_high`, `codex_writer_high` |
| security checker from another provider | `claude_security_high`, `claude_product_high` |
| pre-push reviewer from Claude | `codex_architect_high`, `codex_writer_high` |
| pre-push reviewer from another provider | `claude_prepush_medium`, `claude_product_high` |

The only V0 reasons are `junie_code_writer_unavailable` for the first row and
`prior_run_local_provider_output_failure` for every other row. The latter
requires one authoritative failed execution of the same agent/source provider
whose frozen failure classifier is provider-health eligible. Selection walks
the listed profiles in order, skips an absent profile, the same provider family,
or a target under the existing live run-local quota wait, and chooses the first
remaining profile. It may apply once per
`(run_id, failed_execution_id-or-forced-junie-marker, agent_id,
source_backend_profile_id)`; replay returns the same target and does not consume
another retry. No candidate leaves the source binding unchanged, records
`legacy_fallback_no_route`, and consumes no fallback budget. Current catalog
contents, map order, new profile names, and V1 routes are invisible to V0.
Known-answer fixtures freeze every row, source-family branch, missing profile,
quota skip/next-candidate, all-blocked no-route, and restart replay.
...
Claude `session/new.params.resumeSessionId` contract. That member is enabled
only for an exact executable/adapter closure present in a checked-in
`ProviderAttachConformanceManifestV1`; immutable executable digest alone is not
semantic proof. The Claude manifest creates a private session, terminates its
child, restarts the adapter, resumes only the exact stored session with tagged
`session/new`, proves correlated `result.sessionId`, cwd/root/MCP preservation,
prompt-after-attach success, mismatched-session rejection, and zero prompt on
every malformed or unsupported branch. Codex, Gemini, Auggie,
...
`session/resume` shape into authority. Adding any member or executable version
requires a later schema-version/manifest bump plus a checked-in
`ProviderAttachConformanceManifestV1`
...
class-specific readiness response (a complete option catalog only for an exact-
pair contract), reject a mismatched session, and prove zero prompt on
...
fingerprint, exact `provider_contract_class`, ordered
`ProviderRootAuthorityV1` set for `cwd` and
`additionalDirectories`, an immutable complete
`source_mcp_inventory_v1` plus digest, and a separately immutable
`effective_resurrection_mcp_inventory_v1` plus digest. Source inventory entries
contain name, exact transport, required-versus-resurrection-optional policy,
verified executable/endpoint identity, and broker-owned secret references,
never raw tokens, expanded environment secrets, or a northbound provider-
session ID. Effective inventory contains only admitted stdio entries and records
the source-entry digest for each. The resume-context digest binds both complete
inventories and the closed filtering decision; neither may be recomputed from
the current catalog. The source
...
MCP admission is exhaustive. Empty source inventory yields empty effective
inventory. Required HTTP, SSE, Streamable HTTP, WebSocket, Xcode/IDE broker, or
external/local socket entries reject zero-send
`resume_mcp_containment_incompatible`; an entry explicitly frozen as
`resurrection_optional` is omitted with its omission reason in the effective
digest. A stdio entry is admitted only when its complete executable closure is
verified, transferred by role FD, and launched inside the same final provider
sandbox; otherwise it rejects. Mixed inventory succeeds only when every
required entry is admitted and every omitted entry was explicitly optional.
No entry is silently removed after context persistence, and no MCP credential
is resolved until this matrix and both digests pass.

Every `cwd` and additional root in that context is a
`ProviderRootAuthorityV1`, not an unverified path string. Admission walks from a
trusted workspace/root directory FD with no symlink traversal, opens the
directory, and records canonical display path plus `st_dev`, `st_ino`,
`st_gen`, `st_birthtime_ns`, `fstatfs.f_fsid`, mount flags, content-tree digest,
and required read/write mode. The descriptor proves source identity but is not
misrepresented as a dynamic Seatbelt vnode grant.

`P086FilesystemContainmentV1` materializes every admitted source into a fresh
daemon-owned mode-0700 provider root beneath the private runtime home. A
read-only root is a verified copy/reflink whose files have no provider write
bits. A read-write root is a private copy-on-write staging tree; the provider
may mutate only that tree, and post-terminal publication into the real run
worktree uses the existing trusted worktree mutation coordinator after a
bounded no-symlink diff, source-identity recheck, and operator/workflow policy
check. No original workspace descriptor or path is passed to provider code.
`ProviderMaterializedRootV1` binds source authority digest, private root
device/inode/generation, ordered content digest, mode, and eventual publication
policy. The complete source and materialized inventories/digests are durable in
the resume context and attach receipt.

At launch the runtime reopens each source identity and materialized root,
requires both records byte-equal, then transfers only materialized-root FDs in
the authenticated `SCM_RIGHTS` release. The gate sets cwd with `fchdir`; ACP cwd
and `additionalDirectories` name the corresponding canonical daemon-private
paths whose parents are not writable by the provider. The final path-based
Seatbelt profile can therefore grant those immutable private path prefixes
without claiming unsupported descriptor-bound policy. It denies write, rename,
unlink, link, clone, create, truncate, xattr, chmod/chown, and executable
creation by default; read-only materializations grant read/search, and
read-write materializations grant only the frozen mutation set. Roots absent
from the materialized inventory are inaccessible. If materialization identity,
profile compilation, or enforcement probe fails, admission is zero-send
`resume_filesystem_containment_unavailable`.

Same-path source replacement, symlink retarget, mount/fsid change, inode
generation change, content drift, materialized-root replacement, descriptor
loss, or reordered/duplicated roots rejects zero-send. Race fixtures replace
each source/private directory before materialization, before transfer, and
during launch and prove no attach, prompt, or widened access. Syscall fixtures
execute `openat(O_WRONLY)`, rename, hard/symbolic link, clone, truncate, chmod,
...
frozen adapter capability fingerprint must match the mandatory conformance
manifest and declare the exact
...
materialized `cwd`, complete ordered materialized additional roots, and the
complete effective resurrection MCP inventory after broker-side secret
resolution.
`ResolvedProviderSecretsV1` is scoped to the exact
`claude_session_new_resume_session_id_v1` serializer and request ID; it cannot be
constructed for generic `session/resume`, `session/load`, or an untagged
`session/new` branch.
...
The correlated tagged attach response must arrive for that request and requires
non-empty `result.sessionId` resolving to the same private provider-session
identity. Claude remains `provider_neutral`; its attached generation never
creates accepted model/effort, a `GenerationOptionSnapshotV1`, an exact-pair
receipt, or `acceptance_source = attached_session_reverification`.
`configOptions`, when present, are bounded diagnostic transport data only and
cannot become configuration authority. A pre-response update, malformed attach
response, response/session correlation mismatch, duplicate/regressed transport
sequence, or another attach method is
`ACP_P086_RESUME_CONFIGURATION_UNAVAILABLE` and closes the generation with
identity-safe reap and zero prompt bytes.

After correlated attach, the manager loads the continuation's already-reserved
active configuration attempt/generation/process intent and invokes
`provider_configuration.settle_readiness`. It creates one immutable
`ProviderGenerationReadinessV1` bound to the exact Claude generation, process,
provider-session ref, attach response, conformance-manifest digest, owner,
attempt, work item, and prompt turn. Requested model/effort remain planned
frozen identity; accepted model/effort and every exact-pair field remain null.
The attach receipt, readiness binding, active attempt, generation, process, and
continuation turn form the sole authority tuple before a prompt permit. Source
generation readiness is never transferred by ID or digest. Fake ACP fixtures
cover missing/mismatched context, manifest/executable mismatch,
launch/initialize failure, tagged attach error, session mismatch, forbidden
secret scope, pre-response update, optional `configOptions`, readiness
settlement crash/replay, and successful provider-neutral prompt. Every negative
asserts identity-safe reap, no prompt, no `session/load|session/resume`, and no
`session/new` except the one tagged Claude attach request.
...
an `AF_UNIX/SOCK_SEQPACKET` control socket, and a write-only exec-error pipe
whose write end is `FD_CLOEXEC`. It verifies the suspended helper's code
...
part of the daemon trust boundary and loads no plugin/provider code. It has no
credential, workspace, runtime-home, provider-MCP, or backend input and blocks
on the control socket before opening or executing the provider image. It does
not install an irreversible deny-network bootstrap profile that would also deny
the later provider backend. EOF,
malformed data, timeout, or a release nonce/digest mismatch exits without
provider `execve`.

The helper is a real Xcode target, `ChainworksProviderLaunchGate`, embedded at
`Chainworks Forge.app/Contents/Library/LaunchServices/chainworks-provider-launch-gate`.
It is built for every app architecture, has hardened runtime and library
validation, no plug-in loading, JIT, debugger, automation, or credential
entitlement, and shares the app team identifier. The daemon pins the helper's
designated requirement, Team ID, bundle-relative path, Mach-O architecture,
entitlements digest, and code-directory hash in
`ProviderLaunchGatePackagingManifestV1`. Development, archive, notarized,
stapled, and release-host gates verify that manifest with `SecStaticCode` and
`codesign`; ad-hoc, copied, re-signed, wrong-architecture, missing-helper, or
entitlement-drift builds disable provider launch before credential resolution.
The final provider profile and outbound network authority come from the
verified release message, not from a broad helper entitlement.
...
consuming the permit. It then sends exactly one authenticated release message
over the sequenced-packet socket. The message carries the bounded role manifest
and secret bytes; all post-spawn launch-closure descriptors are transferred in
the same `sendmsg` with `SCM_RIGHTS`. Roles are closed to `provider_entrypoint`,
`provider_interpreter`, `provider_script`, `provider_runtime_home`,
`provider_cwd`, `provider_additional_root`, `stdio_mcp`, and `secret_file`.
Each role has a fixed destination FD range. Duplicate/missing/extra FDs, role
count/order drift, `MSG_CTRUNC`, non-`SCM_RIGHTS` control data, or a descriptor
whose stat/digest/access mode differs from the manifest aborts before sandbox or
provider code. The gate uses `fchdir` for cwd, `dup2` only into the frozen role
range, sets `FD_CLOEXEC` on every control/status/unconsumed FD, and closes the
control socket before provider `execve`.

The gate independently reopens and hashes the target (and the separately
declared interpreter plus script when applicable), validates the release
envelope and allowlisted environment, installs the final provider-specific
...
nothing and closes the pipe by CLOEXEC. EOF is only a candidate success: the
daemon must additionally observe the same live PID/start identity and the exact
post-exec image/manifest before admitting initialize. Exit or unverifiable image
after EOF is failure, never success. A
...
provider code, prompt bytes, tool access, or secret access. After candidate
success on the exec-error pipe, the daemon verifies `proc_pidpath`, PID/start
...
passed to the gate in the single `SCM_RIGHTS` release message as already-open
...
| `runtime_timeline.create_snapshot_lease` | `critical_barrier`; run ID + run-local upper sequence + snapshot digest + expiry | admit terminal settlement |
| `runtime_timeline.ensure_empty_anchor` | `critical_barrier`; run ID + upper sequence + snapshot digest | admit terminal settlement |
| `runtime_timeline.prune_snapshot_leases` | `critical_barrier`; expiry bound + ordered lease/optional-anchor-set digest | admit bounded maintenance settlement |
...
BEGIN
  ...
  SELECT CASE WHEN NEW.membership_sha256 <> COALESCE((
    SELECT chainworks_membership_sha256(
      member_kind, natural_owner_id, member_ordinal, member_sha256
    ) FROM (
      SELECT member_kind, natural_owner_id, member_ordinal, member_sha256
...
vectors are checked in. The Class A section of
`Migration100FinalSchemaManifestV1` includes normalized
...
natural_owner_id, member_sha256)`, inserts one child membership row per owner,
and stores count plus SHA-256 over the common-codec ordered tuples
`(member_ordinal, member_kind, natural_owner_id, member_sha256)` in the parent
result.
The registered deterministic SQLite aggregate
`chainworks_membership_sha256(kind, id, ordinal, member_sha256)` is available on every writer
...
| Timeline event persist | `RuntimeTimelineEventPersistResultV1` | exact immutable event/lane/cursor row; run-local event and lane allocator lower bounds use `closed_successor(runtime_timeline_v1)` and publication occurs only after this result commits |
| Timeline snapshot lease | `RuntimeTimelineSnapshotLeaseResultV1` | exact immutable run-local upper bound, snapshot digest, expiry, and cursor lease; same-key replay is byte-identical and no page is exposed before commit |
...
| Timeline snapshot-lease prune | `RuntimeTimelineLeasePruneResultV1` | exact expired lease/optional-anchor membership and delete tombstone digest are immutable; unexpired or unlisted rows are forbidden |
...
8 KiB duplicate-key-rejected canonical `member_json`, and its SHA-256. The
parent full-request digest is over the common-codec tuple
`(scalar_request_sha256, request_member_count,
request_membership_sha256)`, where the membership digest covers ordered
`(ordinal, kind, member_sha256)` tuples. P079 places each of its at most 200
output descriptors in one child row, so every legal request remains bounded
without a 16-KiB aggregate assumption.
...
parent seal, after seal, before domain transaction, and after result commit.
They prove incomplete unsealed intents are discarded before operation start,
sealed intents replay byte-identically, and no request is truncated or accepted
with a missing/reordered/mutated descriptor.
...
The guarded Rust finalizer for migration 100 renames the lease and fallback-link
tables created by immutable migration 095 to private `*_v1_source` names,
creates the following canonical tables, copies and verifies every source row,
then drops the source tables only in the final tracked-schema swap. This is the
complete final v2 target DDL, not the SQL executed by the staging migration;
omitted indexes are only non-semantic lookup indexes over the shown columns:

Every journal-owned exception below uses two connection-local DbWriter
functions, `chainworks_current_class_a_operation_name()` and
`chainworks_current_class_a_owner_id()`. The DbWriter sets their non-cloneable
values only while executing the matching sealed
`ClassAOperationPermitV1` transaction and clears them on commit/rollback/drop.
Migration, pool, read, and direct-SQL test connections return empty values and
cannot set them. A trigger never trusts an SQL column alone as proof of active
journal ownership.
...
BEGIN
  ...
  SELECT CASE WHEN NEW.terminal_result IN (
    ...
    AND NOT (
      ...
      AND chainworks_current_class_a_operation_name() =
        'p079_repair.complete_no_candidate'
      AND chainworks_current_class_a_owner_id() = NEW.operation_id
      AND EXISTS (
        SELECT 1 FROM p079_artifact_settlement_sets_v1 s
         WHERE s.operation_id = NEW.operation_id
           AND s.outcome = NEW.terminal_result
           AND s.member_count = 0
           AND s.absence_witness_id IS NOT NULL
           AND s.state = 'completed'
      )
    ) THEN RAISE(ABORT, 'p079_cancel_without_candidate_witness') END;
  SELECT CASE WHEN NEW.terminal_result IN (
    'unavailable','failed_transport'
  ) AND NOT (
    NEW.operation_state = 'settled'
    AND chainworks_current_class_a_operation_name() =
      'p079_repair.complete_no_candidate'
  ...
  ) THEN RAISE(ABORT, 'p079_no_candidate_branch_not_authorized') END;
...
BEGIN
  SELECT CASE WHEN NOT (
    (
      NEW.operation_state = 'settled'
      AND NEW.terminal_result = OLD.terminal_result
      AND EXISTS (
        SELECT 1 FROM p079_artifact_settlement_sets_v1 s
         WHERE s.operation_id = NEW.operation_id
      ...
      NEW.operation_state = 'settled'
      AND NEW.terminal_result = 'canonical_activation_conflict'
      AND chainworks_current_class_a_operation_name() =
        'p079_repair.settle_activation_conflict'
      AND chainworks_current_class_a_owner_id() = NEW.operation_id
      AND EXISTS (
        SELECT 1
          FROM p079_artifact_settlement_sets_v1 s
          JOIN p079_canonical_activation_conflicts_v1 c
            ON c.artifact_set_id = s.artifact_set_id
         WHERE s.operation_id = NEW.operation_id
           AND s.state = 'conflict_settled'
           AND NOT EXISTS (
             SELECT 1 FROM p079_artifact_settlement_members_v1 m
              WHERE m.artifact_set_id = s.artifact_set_id
                AND m.state <> 'conflict_settled'
           )
      )
    )
...
The generated set/member guards expose the same two exceptions explicitly:
`prepared -> completed` with zero members is legal only under
`p079_repair.complete_no_candidate`, matching operation owner, closed reason,
absence witness, empty digest, and one settlement timestamp;
`history_committed -> conflict_settled` is legal only under
`p079_repair.settle_activation_conflict`, matching operation owner, conflict
row, winning activation, restored canonical digest, and all members moving in
ordinal order before the set. Every other direct skip remains aborting. Positive
direct-SQL harness transactions bind the exact sealed connection-local owner
and prove both complete branches across crash/replay. The same statements on an
unbound, wrong-operation, wrong-owner, wrong-reason, missing-witness,
partial-member, or mutated-digest connection abort before any row changes.
...
Migration 100 additionally ships two disjoint generated schema files:
`docs/reference/schemas/p079-migration-100-staging-sqlite-master-v1.json` as
`Migration100StagingSchemaManifestV1` and
`docs/reference/schemas/p079-migration-100-final-sqlite-master-v1.json` as
`Migration100FinalSchemaManifestV1`. Each contains the normalized
`sqlite_master` type/name/table-name/SQL tuple and SHA-256 for every relation
legal in that phase. The tracked raw SQLx migration must produce the byte-equal
staging manifest from both an empty database and migration-095 fixture. Only
the guarded Rust finalizer may replace staging relations and, after all copy,
...
a final database claiming staging phase, or any unlisted/missing relation is
failed-serve `provider_truth_schema_mismatch`. Phase-by-phase fixtures
byte-compare both manifests and prove only the declared phase transition changes
the installed tuple set. Direct mutation fixtures drop/add/rewrite each relation
in turn, while a clean second restart after finalization proves no schema rewrite
or duplicate seed row.
...
state root capped at 4,096 files and 128 MiB, materializes every allowed input
root read-only under the same daemon-private containment parent, prepares the
verified provider launch image above, and has the trusted launch gate install
the final `p079_macos_seatbelt_staging_v1` profile immediately before provider
`execve`. The generated profile is inherited by every descendant and denies all
...
the verified daemon-private input materializations, launch closure, required
system runtime files, and immutable provider configuration. It allows outbound
network for the pinned provider backend contract while denying inbound/listen,
ambient local sockets, and every MCP transport below; backend egress is not MCP
authority. Canonical/history paths and the ordinary workspace are
...
digest. Before consumers open, each startup attempt processes at most 16 sets,
256 members, an 8-MiB soft byte budget, and a 12-MiB hard byte budget, ordered
strictly by `(settlement_sequence, member_ordinal, phase)`. The 10-second
continuous-clock value is a scheduling cutoff, not an impossible hard bound on
a blocking filesystem call: after it expires no new chunk starts.

Member copy/hash work is crash-safe and chunked rather than indivisible.
...
is terminal invalid evidence. The parent checks the scheduling cutoff and hard
byte cap before every chunk and phase transaction, checkpoints after each
durable chunk/member phase, and advances `last_completed_sequence` only after
exact final completion. Reaching any cutoff keeps failed-serve and the next
restart resumes at the stored chunk cursor; a new set always has a greater
sequence and cannot fall below the checkpoint. The reducer resumes only the exact missing history-member,
...
0/1/256/257 members, every 1-MiB chunk boundary, 8-MiB soft and 12-MiB hard byte
caps, scheduling cutoff before/after a chunk, exact 10-MiB single-member restart
convergence, an over-limit member, lower lexical IDs with higher sequences,
...
This boundary protects against untrusted provider processes and descendants
launched by Chainworks, accidental duplicate daemons, path replacement races,
and stale/PID-reused owners. It does not claim that POSIX mode `0600/0700` or
`flock` excludes an arbitrary already-compromised process running as the same
macOS login user; such a process has operator-equivalent account authority and
is outside this proposal's threat model. The authenticated challenge prevents
forged duplicate-owner evidence by contained providers because they cannot
reach the control root, endpoint key, or designated-requirement Keychain item.
A future requirement to survive arbitrary same-UID compromise must move lock
and bearer authority behind a separately privileged designated-requirement XPC
service and is not inferred from this design.
...
Schema truth is phase-qualified. `Migration100StagingSchemaManifestV1` contains
the exact normalized SQL/checksums immediately after SQLx migration 100 and may
contain only staging tables/views and nullable backfill columns.
`Migration100PhaseSchemaManifestV1` has one complete manifest for each closed
Rust phase `source_copied | derived_backfilled | constraints_verified |
final_swap_committed`; every manifest names the tables, columns, indexes,
triggers, foreign keys, function-version digests, and authorized next phase.
Only `final_swap_committed` is byte-equal to
`Migration100FinalSchemaManifestV1` and may be called canonical/final schema or
authorize migration 101+. Preflight reads the durable phase first and compares
only with that phase's manifest; matching a staging manifest can never satisfy a
final-schema gate. Generated manifest inventory is bidirectional and rejects a
table/trigger present in two incompatible phases, a skipped phase, or a phase
advance without byte-equal source and destination manifests.
...
The GraphQL SDL additions are additive, but the production `P031RunDetail`
document is a versioned client-protocol cutover because the old shipped document
selects the unbounded `stages.executions` array and the new document must not.
The explicit compatibility handshake below prevents silent mixed-version
execution. Existing scalar `model` fields remain compatibility aliases for
requested/planned model and must not be used as actual truth by the updated UI.
...
is an explicitly abbreviated SDL delta, not a complete SDL document. The
...
type FrozenRunDefinition {
  schemaVersion: String!
  runId: ID!
  ideaId: ID!
  frozenRunSnapshotId: ID!
  frozenRunSnapshotDigest: String!
  workflowSnapshotId: ID!
  workflowSha256: String!
  workflowYamlUtf8: String!
  catalogSnapshotId: ID!
  catalogSha256: String!
  catalogYamlUtf8: String!
  runPlanSnapshotId: ID!
  runPlanSha256: String!
  runPlanJsonUtf8: String!
  sourceProposalRevision: String
}
input CreateReplacementRunFromFrozenSnapshotInput {
  callerRequestId: ID!
  originalRunId: ID!
  expectedFrozenRunSnapshotId: ID!
  expectedFrozenRunSnapshotDigest: String!
  replacementWorkflowYamlUtf8: String!
  replacementWorkflowSha256: String!
  recoveryOperationId: ID!
  recoveryTargetId: ID!
}
type FrozenRunReplacementSuccess {
  schemaVersion: String!
  originalRunId: ID!
  replacementRunId: ID!
  commandJournalId: ID!
...
type FrozenRunReplacementDenied {
  schemaVersion: String!
  code: FrozenRunReplacementDenialCode!
  nextAction: String!
}
union FrozenRunReplacementResult =
  FrozenRunReplacementSuccess | FrozenRunReplacementDenied
enum FrozenRunReplacementDenialCode {
  replacement_not_available caller_request_conflict snapshot_mismatch
  replacement_workflow_invalid stale_recovery_target
}
extend type QueryRoot {
  providerExecutionTruthSchemaVersion: Int!
  runDetailProtocolVersion: Int!
  frozenRunDefinition(runId: ID!): FrozenRunDefinition!
}
extend type MutationRoot {
  createReplacementRunFromFrozenSnapshot(
    input: CreateReplacementRunFromFrozenSnapshotInput!
  ): FrozenRunReplacementResult!
}
...
type OccurrenceExecutionAttemptPage {
  ...
  topologySnapshotDigest: String!
  presentationRowId: ID!
  attemptSnapshotCursor: String!
  attemptSnapshotDigest: String!
  pageState: CursorPageState!
...
extend type QueryRoot {
  ...
  occurrenceExecutionAttemptPage(
    runId: ID!, presentationRowId: ID!, topologySnapshotCursor: String!,
    attemptSnapshotCursor: String, attemptAfterCursor: String, first: Int!
  ): OccurrenceExecutionAttemptPage!
}
# Final backward-compatible SDL argument list on the existing SubscriptionRoot field:
...
Timeline operations are normalized before `async-graphql` parses, validates, or
coerces them. `TimelineGraphQLRequestNormalizerV1` is installed in the Axum HTTP
route and the GraphQL-over-WebSocket subscribe-message path. Its generated
inventory contains the exact accepted document bytes/hash, operation name,
transport kind, and exact variable key/type/nullability grammar for
`P031RuntimeTimelineSnapshot`, `P031RuntimeStatusChanged`, and the frozen legacy
live-only `runtimeStatusChanged(runId:, replayCursor:)` document; an alias,
fragment/document mutation, anonymous operation, duplicate JSON key, extra or
missing variable, JSON number in place of a string, null for a non-null value,
or unknown envelope key is rejected by the normalizer and never delegated to
the schema. The HTTP body is exactly one JSON object with keys `query`,
`operationName`, and `variables`. A WebSocket subscribe payload has those same
three keys and a non-empty outer operation ID; a malformed outer protocol frame
without a usable ID follows the existing graphql-transport-ws `4400` close and
cannot be represented as a Timeline operation error.

Validation precedence is exact and shared: transport envelope/duplicate keys;
boundary authentication; operation name plus document hash; variable key set,
JSON types, and canonical scalar grammar; strict `RunId`; cursor mode; cursor
syntax and run binding; page size; run authorization; then snapshot lease and
retention. The first failure wins. Accepted documents therefore cannot produce
an untyped GraphQL parse, unknown-field, unknown-argument, extra-variable, or
coercion error. Any such error returned after normalization is an internal
contract violation, closes the request/stream, increments
`timeline_normalizer_escape_total`, and is never shown as empty Timeline data.
Raw HTTP and WebSocket vectors mutate every envelope, document byte, variable,
type, and precedence pair and assert zero resolver, database, replay, or receiver
registration before the designated layer.
Boundary-authentication failure retains the existing shared HTTP/WebSocket auth
envelope and is not relabeled as a Timeline domain code; fixtures still prove it
wins before document/variable/run disclosure.
...
domain code set is `TIMELINE_TRANSPORT_ENVELOPE_INVALID`,
`TIMELINE_OPERATION_DOCUMENT_INVALID`, `TIMELINE_VARIABLES_INVALID`,
...
exactly one listed code after the three normalizer codes. Query and WebSocket fixtures cover every code and prove
...
atomically advances `runtime_timeline_run_sequences_v1(run_id PRIMARY KEY,
last_event_sequence)` by checked successor and allocates that run's positive
signed-64-bit `event_sequence`, beginning at `1`; value `0` is reserved only
for the empty-snapshot bound. A row at the signed maximum is exhausted and the
next allocation fails before mutation. The event table is unique on
`(run_id, event_sequence)` and on
`(run_id, runtime_event_id)`. The same transaction allocates
the lane ordinal above, inserts the complete bounded `GqlRuntimeEvent` source
fields plus run/authorization owner, non-null
`canonical_private_event_sha256`, non-null `canonical_wire_event_sha256`, and a non-null unique
...
`[run_id, canonical_run_event_sequence, runtime_event_id]`; it is never a
process-local broadcast counter or a cross-run sequence.

`event_sequence` is a SQLite signed 64-bit integer constrained to
`1...9223372036854775807`; attempting a successor after the upper bound fails
closed before an insert. The wire projects it as canonical unsigned ASCII decimal with no sign
...
hex public wire digest as `canonicalEventDigest`.
`canonical_private_event_sha256` uses domain
`chainworks.runtime_timeline_event.private.v1` and covers every authoritative
stored field, including authorization owner/scope. It is verified only inside
Rust and is never exposed. `canonical_wire_event_sha256` uses domain
`chainworks.runtime_timeline_event.wire.v1` and covers duplicate-key-rejected
RFC 8785 JSON of exactly the fields selected in the complete
`GqlRuntimeEvent` fragment, excluding only `projectionGeneration`,
`gapDetected`, `requiresFullRefetch`, and the digest itself. It includes the
run-local sequence, cursor, complete public identity/lane tuple, timestamp,
bounded detail metadata, and legacy compatibility fields, but no private
authorization owner or unselected storage field. The private digest binds the
wire digest. Rust persistence/GraphQL and Swift independently recompute the
wire vectors; Rust separately verifies private vectors. A row with an
unparsable/overflowing sequence, changed field under the
...
Swift uses one concrete `TimelineEventSequenceV1` value type backed by `UInt64`
but accepting only canonical ASCII decimal wire bytes. Event values are exactly
`1...9223372036854775807`; `0` is accepted only by the separately tagged
`TimelineSnapshotUpperBoundV1.empty` constructor and can never construct an
event sequence. A sign, whitespace, non-ASCII digit, leading zero, empty input,
value above signed-64-bit maximum, or checked-successor overflow is
`response_schema_mismatch` and starts a Timeline-only full refetch. Snapshot
upper bounds, page cursors, durable cursors, and event rows all use this parser;
no `Int`, `Double`, locale formatter, or lexicographic comparison is legal.
Known-answer tests cover `0`, `1`, `9`, `10`, the signed maximum, maximum plus
one, and every rejected spelling.
...
are immutable. Every initial snapshot, empty or non-empty, first submits
registered Class A `runtime_timeline.create_snapshot_lease` with the authorized
run, exact current run-local upper sequence, ordered event-set digest, and
server-issued expiry. The committed/replayed
`RuntimeTimelineSnapshotLeaseResultV1` is required before reading/exposing page
bytes. An append after lease creation has a larger run-local sequence and cannot
change the frozen set; same-key replay returns the same cursor/expiry, while a
different digest is conflict. There is no unowned resolver-side lease insert.

A run with no durable event has the exact upper bound `0`, ordered empty-set
digest `e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855`,
and no event row with sequence zero. At that bound, `TimelineEmptyAnchorWriterV1`
derives the cursor from `(run_id, "0", "empty_snapshot_anchor_v1")`; all three
bytes are normative. At any snapshot bound with no
retained event for the run, `TimelineEmptyAnchorWriterV1` derives the cursor with
the same codec using that run, the frozen upper sequence, and literal
`empty_snapshot_anchor_v1` as `runtime_event_id`; it uses registered Class A
`runtime_timeline.ensure_empty_anchor` bound to the already-committed lease and
commits or replays the anchor before the snapshot response
...
anchor has a greater run-local sequence and is replayed exactly once after process
...
both snapshot and handoff cursors, and an anchor copies the same value.
At most 64 unexpired snapshot leases per run and 4096 globally may exist;
empty anchors are a subset and add no independent quota. Creation first
runs bounded expiry pruning and fails with
`TIMELINE_SNAPSHOT_ANCHOR_PERSISTENCE_FAILED` rather than evicting an unexpired
lease. Registered Class A `runtime_timeline.prune_snapshot_leases` is the sole
delete authority. It deletes at most 64 expired leases and their optional
anchors or 1 MiB per transaction in `(expires_at, snapshot_cursor)` order,
first inserts one append-only
`runtime_timeline_lease_prune_tombstones_v1` row containing the complete lease,
optional anchor identity, and prune operation for each member, records the
...
snapshot. Fixtures cover 0/1/64/65 per-run and 4096/4097 global leases with and
without anchors,
...
256. On the first page, null `snapshotCursor` and `afterCursor` first read the
run-local upper sequence (or reserved `0`), count, and ordered wire digest, then
commit/replay `runtime_timeline.create_snapshot_lease`. A second read transaction
requires the same rows at or below that immutable bound to match the committed
digest before returning the newest page plus the separate resolvable
`timeline_cursor_v1` `handoffCursor` for that upper bound. The response presents that page in ascending
run-local-sequence order and returns `nextPageCursor` only when an older page
...
The successor check is run-local: persisted interleaving `runA:1, runB:1,
runA:2` delivers `runA:1 -> runA:2` without a gap, while `runA:1 -> runA:3`
does produce one. Migration, snapshot, replay, live-buffer, and Swift tests use
that exact vector so a different run's inserts can never manufacture a gap.
...
`timeline_transport_envelope_invalid`, `timeline_operation_document_invalid`,
`timeline_variables_invalid`, `timeline_run_id_invalid`, `timeline_cursor_mode_invalid`,
`timeline_snapshot_cursor_invalid`, `timeline_page_cursor_invalid`,
`timeline_durable_cursor_invalid`, `timeline_cursor_run_mismatch`,
...
or mixing durable and legacy cursors is rejected. A retained schema-only
legacy fixture bypasses protocol middleware and executes the old
`runtimeStatusChanged(runId:, replayCursor:)` document unchanged to prove the
SDL field itself remains source-compatible. Separate transport fixtures prove
that an old application without the exact v2 capability is rejected before a
resolver or database read; the schema-only fixture is not mixed-version runtime
...
`TIMELINE_VARIABLES_INVALID -> timeline_variables_invalid`,
`TIMELINE_SNAPSHOT_ANCHOR_PERSISTENCE_FAILED ->
timeline_snapshot_anchor_persistence_failed`; generated exhaustiveness covers
all thirteen pairs. The HTTP decoder requires schema/code/requestId and the
...
`P031RunDetail` uses the closed client protocol
`p031_topology_paging_v2`. The repository generates
`P031RunDetailDocumentInventoryV1` from the exact shipped pre-proposal and v2
documents. Each row contains operation name, canonical source bytes, SHA-256,
complete variable schema, selected root fields, and whether
`stages.executions` or `executionAttempts` is selected. The pre-proposal row is
frozen from the current checked-in `P031GraphQLDocumentSet.runDetail` bytes; a
handwritten summary or field-presence scan cannot replace it. The v2 row selects
neither legacy array and is the sole document accepted from the updated app.

Before any run-detail/topology/attempt resolver executes, the HTTP client sends
header `X-Chainworks-P031-Protocol: p031_topology_paging_v2`; GraphQL-over-
WebSocket sends the same value as `connection_init.payload.p031Protocol`. The
updated app first executes exact operation `P031RunDetailProtocolProbe` and
accepts only non-null integer `runDetailProtocolVersion = 2`. An old daemon's
unknown-field response is locally reduced to `daemon_upgrade_required`, and no
v2 run-detail/topology/attempt document is sent. The new daemon's pre-execution
middleware rejects the frozen old document, an absent/wrong capability, and an
unknown document hash as exact `p031_client_protocol_error_v1` codes
`P031_CLIENT_UPGRADE_REQUIRED`, `P031_PROTOCOL_CAPABILITY_INVALID`, and
`P031_DOCUMENT_UNSUPPORTED`, respectively, before a resolver or database read.
HTTP errors carry the existing non-empty P042 request ID; WebSocket errors bind
the non-empty outer operation ID. The updated UI reduces all three to the fixed
incompatible-client/daemon state and never retries them as transport failures.

This is a deliberate lockstep app/daemon boundary, not a claim that the old
document remains response-compatible with the new daemon. An old app receives
the typed upgrade-required failure instead of a truncated or unbounded result;
a new app does not execute against an old daemon. Schema, middleware, document,
transport-spy, and mixed-version fixtures cover old-app/new-daemon,
new-app/old-daemon, missing/wrong capability, altered old/new bytes, and the
accepted v2 pair. Only after the capability and exact document hash pass may
normal GraphQL validation and the bounded resolvers run.

The shipped `P031GraphQLDocumentSet` has proposal-owned properties
`providerExecutionTruthSchemaProbe`, `runDetailProtocolProbe`, `runDetail`, `runStageTopologyPage`,
...
query P031OccurrenceExecutionAttemptPage(
  ...
  occurrenceExecutionAttemptPage(
    runId: $runId, presentationRowId: $presentationRowId,
    topologySnapshotCursor: $topologySnapshotCursor,
    attemptSnapshotCursor: $attemptSnapshotCursor,
    attemptAfterCursor: $attemptAfterCursor, first: $first
  ) {
    schemaVersion topologySnapshotCursor topologySnapshotDigest presentationRowId
    attemptSnapshotCursor attemptSnapshotDigest pageState nextAttemptCursor
    attempts {
...
exist. These SDL fields preserve bounded non-P031 callers; the frozen old
`P031RunDetail` document never reaches them on a new daemon because the protocol
middleware returns `P031_CLIENT_UPGRADE_REQUIRED`. The updated app never
selects either field and uses `P031OccurrenceExecutionAttemptPage` instead.
...
response cap, the same topology snapshot/digest, and total order
`started_at DESC, id DESC`. The request identifies the frozen topology row by
`presentationRowId`, never by nullable `taskOccurrenceId`. Initial mode requires
both `attemptSnapshotCursor` and `attemptAfterCursor` null. The resolver first
finds that exact row inside the supplied topology snapshot and freezes an ordered
membership of at most 128 immutable execution IDs into `attempt_snapshot_v1`;
its duplicate-key-rejected binary payload is
MAC-bound to run ID, topology snapshot cursor/digest, presentation-row ID,
ordered ID-membership digest, count, and topology expiry and is capped at 8 KiB.
The response repeats the resulting cursor and digest. Continue mode requires the
same non-null attempt snapshot plus the prior non-null page cursor and reads only
the frozen membership; attempts inserted after initial mode cannot appear in a
later page. Same membership with another digest, missing/reordered members,
cross-run/row/topology cursors, expiry, or continue after exhausted is a typed
failure before partial publication.
Execution status/evidence fields may advance only through their existing legal
state transitions while the snapshot is open; they are read by frozen ID on
each page and do not alter membership/order/digest. Swift merges such an update
only into the same execution ID and never inserts a non-member.

For `matched_v2`, membership contains only executions whose immutable
`task_occurrence_id` equals the frozen row. For `legacy_unique`, it contains the
one execution ID already frozen as that row's unique legacy association even
though `taskOccurrenceId` is null. `not_started` freezes the empty set and
returns `exhausted`; `legacy_ambiguous` is not queryable and returns exact
`topology_graphql_error_v1` code
`TOPOLOGY_ATTEMPT_ASSOCIATION_AMBIGUOUS`. A 129th member or an 8-KiB cursor
overflow returns
`TOPOLOGY_ATTEMPT_PROJECTION_SIZE_LIMIT_EXCEEDED`; the UI keeps the occurrence
summary and exposes no attempt retry. Swift retains at most 128 attempts and
1 MiB per selected presentation row. Page eviction may discard decoded rows but
retains the immutable attempt snapshot and page cursor for explicit reload; it
never refreezes membership, truncates the visible page, or folds attempts into
aggregate execution truth. For non-P031 legacy documents admitted by their
existing boundary, the monolithic `runStageTopology` selected-field bytes remain
unchanged only within 1,024 occurrences, 4,096 transitions, 256 stages, and
8 MiB; this is not old-P031 compatibility. The 32-attempt/256-KiB
per-occurrence cap is additionally evaluated
...
The remaining attempt request errors are exact `topology_graphql_error_v1`
codes `TOPOLOGY_ATTEMPT_SNAPSHOT_INVALID`,
`TOPOLOGY_ATTEMPT_PAGE_CURSOR_INVALID`, and
`TOPOLOGY_SNAPSHOT_EXPIRED`; the generated Swift mapping is respectively
`attempt_snapshot_invalid`, `attempt_page_cursor_invalid`, and
`topology_snapshot_expired`. Association ambiguity and projection-size codes map
to their same-word lowercase failure values. Unknown/extra codes are response
schema mismatches and never empty attempt history.
...
- `P031RunStageTopologyOccurrenceReadModel` adds every exact topology field,
  ...
  paged `P031OccurrenceExecutionAttemptPageReadModel`, which owns the immutable
  presentation-row-bound attempt snapshot cursor/digest and page cursor, rather
  than an unbounded nested array;
...
- `P031RuntimeTimelineSnapshotReadModel`,
  ...
  concrete `TimelineEventSequenceV1`/`TimelineSnapshotUpperBoundV1`, canonical
  digest, projection generation,
...
schema probes, old/v2 `P031RunDetail` inventory,
`P031RunStageTopologyPage`, `P031OccurrenceExecutionAttemptPage`,
`P031RuntimeTimelineSnapshot`, `P031RuntimeStatusChanged`, and
`P031TimelineRawDetail` operation strings from
...
- for `codex_exact_pair`, `configuring`, `failed_before_prompt`,
  `cancelled_before_configuration`, and `legacy_unverified` require all
  accepted/source/verified fields null; these state literals are not required
  or synthesized for another contract class;
- non-Codex execution requires configuration state, accepted wire pair,
  ...
  `not_applicable`;
- `failure_available` requires a null receipt/cancellation pointer, a matching
  immutable configuration-failure row/code, and a `not_started` prompt turn;
  configuration state is exactly `failed_before_prompt` for
  `codex_exact_pair` and exactly null for `provider_neutral|legacy_best_effort`;
- `cancellation_available` requires null accepted values and receipt/failure
  pointers, a matching immutable pre-acceptance cancellation row/code, and a
  `not_started` prompt turn; configuration state is exactly
  `cancelled_before_configuration` for `codex_exact_pair` and exactly null for
  `provider_neutral|legacy_best_effort`;
...
The two probe SDL fields are `runDetailProtocolVersion: Int!` and
`providerExecutionTruthSchemaVersion: Int!` on `QueryRoot`. Their only documents
are shipped properties with exact bytes
`query P031RunDetailProtocolProbe { runDetailProtocolVersion }` and
`query ProviderExecutionTruthSchemaProbe { providerExecutionTruthSchemaVersion }`.
`P031GraphQLRunReadClient` sends both through
`P031URLSessionGraphQLReadTransport` under the v2 capability header, decodes
presence-aware read models, and accepts only protocol `2` plus truth schema `1`
with no GraphQL errors. Protocol probe runs first; a truth-schema probe or
run-detail request is forbidden until it succeeds. Document snapshots,
transport spies, decoder fixtures, and daemon-replacement tests all use these
production properties; a recursive source scan rejects another probe literal or
test-only transport. Handling is frozen:

| Probe result | App action |
|---|---|
| HTTP/network/auth failure | Surface existing daemon/auth error; do not replace for schema |
| Either probe returns GraphQL unknown-field or typed upgrade-required | Replace bundled daemon once, await readiness, restart the two-probe sequence once |
| Protocol missing/null/non-integer/not 2, truth schema missing/null/non-integer/not 1, or data plus errors | Same one replacement/retry, then typed schema mismatch |
| Malformed response JSON | Typed daemon protocol error; no reduced query |
| Protocol 2 then truth schema 1 | Issue only the v2 bounded `P031RunDetail` document and paged topology/attempt operations |

After one replacement attempt, every non-success renders
`Daemon schema mismatch` with retry/restart action; the app never loops
replacement, sends the old document, or falls back to planned values as runtime
truth.

The Swift DTO fields remain nullable for historical database rows and
pre-configuration executions returned by a daemon that advertises protocol 2
and truth schema 1.
...
|---|---|
| planned | `Planned: Codex<legacyModelSegment><legacyEffortSegment> - Unverified` |
| starting | `Starting: Codex<legacyModelSegment><legacyEffortSegment> - Unverified` |
| running after prompt | `Running: Codex<legacyModelSegment><legacyEffortSegment> - Unverified` |
| completed after prompt | `Completed: Codex<legacyModelSegment><legacyEffortSegment> - Unverified` |
| failed after prompt | `Failed: Codex<legacyModelSegment><legacyEffortSegment> - Unverified` |
| cancelled after prompt | `Cancelled: Codex<legacyModelSegment><legacyEffortSegment> - Unverified` |
| delivery unknown | `Prompt delivery unknown: Codex<legacyModelSegment><legacyEffortSegment> - Do not retry automatically - Unverified` |

`<legacyModelSegment>` is derived only from the frozen requested model. It is
` - ` plus the exact display mapping below for any non-empty value, including
older/custom models, and is empty only when that historical field is absent.
It never substitutes generic `gpt-5.6` for a different or missing pre-marker
model. `<legacyEffortSegment>` is derived only from the frozen requested effort. It is
...
state, clears the source-qualified focus registration, and leaves focus nil.
`rowsChanged` may independently reduce semantic selection from its retained
snapshot, but background removal never restores focus to that selection, a
surviving row, or a stage heading; only a later explicit user navigation
activation may consume the remembered heading candidate. Pure tests invoke every
...
loads the authorized frozen definition and opens the proposal-owned
`FrozenRunRepairV1` workspace and command;
...
The proposal adds a production Runs-window minimum and governs all three
non-detail columns rather than counting only the inner Runs sidebar. The
scene/window coordinator sets `NSWindow.contentMinSize` to exactly `920 x 620`
points before publishing the first Runs surface, and the SwiftUI root uses the
same minimum so restoration cannot reopen a smaller content area.
`RunWorkspaceColumnPolicyV1` owns exact widths: outer `P036MainShell`
navigation is `compact(92) | expanded(260)`, the Runs
`NavigationSplitView` sidebar is `collapsed(0) | expanded(280)`, Run Inspector
is `closed(0) | open(360)`, and each visible boundary consumes one additional
point. The primary run detail is pinned and requires at least 460 points before
its own horizontal scroll viewport.

Every user/restoration request first computes
`detailWidth = contentWidth - outerWidth - runsWidth - inspectorWidth -
visibleBoundaryCount`. If it is below 460, the triggering user-expanded column
is protected for that reduction and unprotected columns collapse in exact order
outer navigation, Runs sidebar, then Inspector until the bound is met. A request
to open Inspector at 920 therefore produces
`compact outer + collapsed Runs + open Inspector`; a request to expand Runs in
that state preserves Runs and closes Inspector if collapsing the outer column
is insufficient. Restoration has no protected column and uses the same listed
order. At 1440 all three expanded/open columns leave at least 537 points and are
preserved. Auto-collapse updates visible column state but not semantic run,
occurrence, Timeline, popover, or focus selection; reopening a column is an
explicit navigation activation through the focus reducer.

The responsive proof matrix is exact: hosted snapshots run at logical content
sizes `920 x 620` and `1440 x 900`, display scale `2.0`, cross all eight
requested outer-navigation x Runs-sidebar x Inspector combinations and SwiftUI
dynamic type `.large | .accessibility3`, for 32 cases. Each fixture asserts the
normalized column state, exact widths/boundaries, detail width at least 460,
and target reachability. At `920 x 620` the topology canvas may scroll horizontally and vertically, but
...
| Inspector attempt `attempt_row` | `Execution attempt <attempt ordinal> for <task name>, <occurrenceDiscriminator>. <accessibilityIdentity>. Status: <status>.` |
| Inspector attempt `attempt_info` | `Show full runtime identity for execution attempt <attempt ordinal>, <task name>, <occurrenceDiscriminator>` |
| Inspector attempt `attempt_copy` | `Copy full runtime identity for execution attempt <attempt ordinal>, <task name>, <occurrenceDiscriminator>` |
| Inspector attempt `attempt_popover` | `Runtime identity details for execution attempt <attempt ordinal>, <task name>, <occurrenceDiscriminator>` |
| Inspector attempt `attempt_close` | `Close execution attempt runtime identity details` |
...
| Inspector attempt page `loading_attempts_initial` | `Loading execution attempts` |
| Inspector attempt page `attempts_initial_failure` | `Execution attempts unavailable` |
| Inspector attempt page `retry_attempts_initial` | `Retry loading execution attempts` |
...
`AccessibilityContractV1` is the sole generated source of the complete
`(label, value, hint, role, enabled)` tuple. Role tokens map exactly to AppKit
`button = AXButton`, `group = AXGroup`, `static_text = AXStaticText`, and
`heading = AXHeading`. Empty means the zero-length string, not null or an
omitted modifier. The label column is the table above; all remaining fields are
closed here:

| Control kinds | Role | Exact value | Exact hint | Enabled |
|---|---|---|---|---:|
| occurrence `row` | `button` | `<accessibilityIdentity>. Status: <status>. Attempts: <count>.` | `Select this task occurrence.` | true |
| occurrence `info` | `button` | empty | `Show runtime identity details.` | true |
| occurrence `copy` | `button` | empty | `Copy the complete visible runtime identity.` | true |
| occurrence `popover` | `group` | `<accessibilityIdentity>` | empty | true |
| occurrence `close` | `button` | empty | `Close identity details and return to the task occurrence.` | true |
| attempt `attempt_row` | `button` | `<accessibilityIdentity>. Status: <status>.` | `Select this execution attempt.` | true |
| attempt `attempt_info` | `button` | empty | `Show runtime identity details for this execution attempt.` | true |
| attempt `attempt_copy` | `button` | empty | `Copy the complete visible runtime identity for this execution attempt.` | true |
| attempt `attempt_popover` | `group` | `<accessibilityIdentity>` | empty | true |
| attempt `attempt_close` | `button` | empty | `Close identity details and return to the execution attempt.` | true |
| event `event_row` | `button` | `Collapsed` or `Expanded` from exact state | `Select this timeline event.` | true |
| event `event_expand` | `button` | empty | `Show event details.` | true |
| event `event_collapse` | `button` | empty | `Hide event details.` | true |
| event `event_copy_id` | `button` | empty | `Copy the complete event identifier.` | true |
| event `event_copy_raw` full | `button` | `Full raw content` | `Copy the authorized full raw event content.` | true |
| event `event_copy_raw` retained | `button` | `Retained raw content` | `Copy the authorized retained raw event content.` | true |
| `load_older`, `retry_load_older`, `retry_timeline_initial` | `button` | empty | `Load the requested timeline activity.` | true |
| `loading_older`, `loading_timeline_initial` | `static_text` | empty | empty | false |
| `timeline_initial_failure`, `legacy_gap_row` | `static_text` | empty | empty | true |
| `retry_attempts_initial`, `load_attempts_older`, `retry_attempts_older` | `button` | empty | `Load the requested execution attempts.` | true |
| `loading_attempts_initial`, `loading_attempts_older` | `static_text` | empty | empty | false |
| `attempts_initial_failure` | `static_text` | empty | empty | true |
| `stage_heading` | `heading` | empty | empty | true |
| `run_events` | `button` | empty | `Select run events.` | true |
| `loading_summary` | `static_text` | empty | empty | false |
| `failed_summary`, `topology_unavailable_summary`, `empty_summary` | `static_text` | empty | empty | true |
| `retry_load` | `button` | empty | `Retry loading the selected run.` | true |
| `restart_daemon_and_retry_topology` | `button` | empty | `Restart the daemon and reload topology in a new generation.` | true |
| `open_frozen_input_repair` | `button` | empty | `Open the frozen workflow in a replacement-run repair workspace.` | true |

For an occurrence or attempt whose delivery is unknown, the base row hint is
followed by one ASCII space and exact sentence `Automatic retry is blocked.`;
for legacy ambiguity it is followed by one ASCII space and exact sentence
`Multiple legacy executions; runtime identity is unavailable.` Both in that
order when both apply. No other control inherits those hints. A legal target is
enabled exactly as shown; an inapplicable target is absent rather than present
disabled, except the four explicit loading controls. Hosted AppKit tests assert
AXRole, AXLabel, AXValue, AXHelp, AXEnabled, child order, and focusability for
every row and reject omitted, null, localized, or extra values.
...
Unknown-delivery and legacy-ambiguity hints are composed only by the exact rule
above and never alter the row value. Tests assert labels, values, hints, child
...
| `inspector_attempt` | `presentation_row_id`, `agent_execution_id`, `topology_snapshot_digest`, `attempt_snapshot_digest` |
| `inspector_attempt_page` | `presentation_row_id`, `topology_snapshot_digest`, `attempt_snapshot_digest_or_empty`, `attempt_page_generation`, `page_cursor_digest_or_empty` |
...
| `inspector_attempt` | `attempt_row`, `attempt_info`, `attempt_copy`, `attempt_popover`, `attempt_close` |
| `inspector_attempt_page` | `loading_attempts_initial`, `attempts_initial_failure`, `retry_attempts_initial`, `load_attempts_older`, `loading_attempts_older`, `retry_attempts_older` |
...
`not_loaded |
loading_initial(presentationRowID, topologySnapshotDigest,
attemptPageGeneration, requestDigest) |
failed_initial(presentationRowID, topologySnapshotDigest,
attemptPageGeneration, failureCode, requestDigest) |
ready(presentationRowID, topologySnapshotDigest,
attemptSnapshotCursorDigest, attemptSnapshotDigest, attemptPageGeneration,
nextAttemptCursor?) |
loading_older(presentationRowID, topologySnapshotDigest,
attemptSnapshotCursorDigest, attemptSnapshotDigest, attemptPageGeneration,
pageCursorDigest, requestDigest) |
failed_retaining_attempts(presentationRowID, topologySnapshotDigest,
attemptSnapshotCursorDigest, attemptSnapshotDigest, attemptPageGeneration,
pageCursorDigest, failureCode, requestDigest)`.
`InspectorAttemptPageFailureCodeV1` is closed to `transport_unavailable`,
`response_schema_mismatch`, `topology_snapshot_expired`,
`attempt_snapshot_invalid`, `attempt_page_cursor_invalid`,
`attempt_association_ambiguous`, `attempt_projection_size_limit_exceeded`, and
`page_byte_limit_exceeded`; only
...
surface and only for the exact selected occurrence, snapshot digest, and page
generation. Initial loading/failure uses fixed lowercase `empty` for both
snapshot/page digest payloads. A ready/older state instead requires the immutable
attempt-snapshot digest and SHA-256 digest of the current opaque next-page
cursor.
`loading_attempts_initial` requires the exact in-flight initial request;
`attempts_initial_failure` requires exact failed initial state;
`retry_attempts_initial` exists only for its `transport_unavailable` failure.
...
Each retained attempt publishes one distinct `inspector_attempt` subject on the
Run Inspector surface. It is legal only when the current selected presentation
row, topology digest, and immutable attempt-snapshot digest match and that exact
agent-execution ID is a member at the displayed zero-based snapshot position.
The spoken `<attempt ordinal>` is that position plus one through the canonical
integer codec; it is not a retry counter or database ID. Row/info/copy/popover/
close controls mirror the occurrence identity actions but carry the attempt's
own frozen provider truth. A removed or evicted page withdraws its complete
target inventory; stale focus, popover, copy, or callback is rejected and
cannot fall back to the occurrence summary or another attempt. Known-answer
tests use two attempts from the same agent and occurrence and require distinct
machine targets, labels, clipboard bytes, focus registrations, and popover
state.
...
frozenRunSnapshotID, frozenRunSnapshotDigest, topologyFailureCode,
...
authorized frozen-run readback, never reconstructed from a selected row.

This proposal owns a real replacement boundary; it does not refer to an
existing start/clone UI. Operator-only query `frozenRunDefinition(runId: ID!)`
returns exact `frozen_run_definition_v1` with run/idea IDs, frozen workflow,
catalog, and run-plan snapshot IDs, duplicate-key-rejected UTF-8 bytes, per-
member SHA-256, aggregate snapshot digest, and source proposal revision. Each
member is capped at 4 MiB and the response at 8 MiB. Agent and Observer callers
receive the existing indistinguishable unauthorized/not-found contract, and no
snapshot bytes are logged, cached globally, or exposed by Timeline/report
readback. The route is accepted only when the returned snapshot ID/digest equal
the recovery registration.

The Definitions destination mounts a new `FrozenRunRepairV1` workspace from
those exact bytes. It is read-only until the operator explicitly chooses
`Create replacement`; the only editable member is replacement workflow YAML,
while catalog and original run-plan bytes remain immutable evidence. Operator-
only mutation `createReplacementRunFromFrozenSnapshot(input:)` requires
`callerRequestId`, original run ID, expected frozen snapshot ID/digest,
replacement workflow UTF-8 bytes/digest, and recovery operation/target IDs. The
command journal intent hash covers every field. In one transaction it rechecks
the original frozen bytes, compiles the replacement workflow against the frozen
catalog, creates exactly one new run linked by `replaces_run_id`, stores the new
frozen snapshot and source digest, and returns
`frozen_run_replacement_result_v1`. Same request and intent replay returns the
same replacement run; same request with another intent is
`caller_request_conflict`; snapshot drift, invalid workflow, stale recovery
target, or unauthorized caller mutates nothing. The original run and all of its
artifacts, attempts, approvals, ledger, and snapshot remain byte-identical.

`AppShellNavigationCoordinatorV1` accepts the typed route only for the same
window routing ID. `FrozenInputRepairRouteV1` is a distinct
`AppShellRouteV1.frozenInputRepair` member, not an invented
`RunWorkbenchDestinationV1`; the Runs router emits it through its typed
cross-shell port and cannot select Definitions directly. The app-shell
coordinator journals local navigation acceptance, retrieves the frozen
definition through the production authorized client, and opens that explicit
repair workspace. Replay of the same complete route returns the same local
operation and does not open a second destination or workspace; a field mismatch
is stale. It records no daemon-restart operation and cannot submit the mutation
without a separate user activation. Query/mutation transport spies, command-
journal crash/replay tests, unauthorized principals, stale digest/route tests,
and two-window hosted fixtures prove exact snapshot bytes, zero mutation on
failure, one replacement on replay, and byte-identical background/original run
state. Direct Run Inspector activation follows the same route. Run Inspector
and every primary destination consume the same window registration and never
retain an occurrence summary while topology is unavailable. Restart callbacks
across every stale router/target/operation/load permutation prove frozen-input
activation cannot enter a restart loop.
...
`RunInspectorCompositionV1` preserves the current Inspector rather than
replacing it with occurrence diagnostics. Its un-nested section order is exact:
existing `runSnapshotSection`; new `selectionContextSection`; new
`attemptHistorySection` when legal; existing `approvalGateSection` when
present; existing `outputContractRepairSection` when present; and existing
`recoverySection`. Approval actions continue to call only the current blessed
settlement path, output-repair controls retain their current authorization/path
rules, and Recovery retains its current closeout route. Selection, Timeline,
attempt paging, or topology failure cannot hide, reorder, duplicate, or disable
those three governed existing sections.

The selection-context matrix is complete:

| Semantic selection/state | Selection-context section | Attempt-history section |
|---|---|---|
| `occurrence` | exact selected occurrence summary | retained immutable attempt snapshot plus legal page control |
| `run_events` | selected Run events lane summary | absent |
| `loading` | generation-qualified loading summary | absent |
| `failed` | generation-qualified failed summary | absent |
| `topology_unavailable` | exact topology-unavailable summary | absent |
| `empty` | loaded-empty summary | absent |

Generated composition tests cross those six rows with approval absent/present,
output repair absent/present, and existing recovery visual state
`healthy|attention|blocked` (72 cells). Every cell byte-compares section IDs and
order and invokes each still-visible existing action through its production
route. Hosted fixtures additionally cover a pending approval and repair evidence
while selection changes through every row, proving the new context never turns
the Inspector into a selection-only panel.
...
loaded-empty content inside `selectionContextSection`; governed existing
Inspector sections remain composed as above. Initial/default occurrence choice
...
| P086 pre-response option update or response/request/session correlation invalid | `ACP_P086_RESUME_CONFIGURATION_UNAVAILABLE`; identity-safe reap | 0 |
...
| P031 protocol 2 / truth schema 1 probe, capability, or exact-document contract fails | typed app/daemon upgrade or schema mismatch; no run-detail resolver/reduced query | unchanged |
| Attempt snapshot is ambiguous, oversized, expired, or cursor/digest mismatched | typed Inspector attempt failure; retain occurrence summary and publish no stale attempts | unchanged |
| Frozen-definition readback or replacement command is unauthorized/stale/invalid | typed no-oracle denial; original run and snapshot remain byte-identical | unchanged |
...
9. P086 admission binds the immutable secret-safe resume-context ID/digest,
   ...
   proving the mandatory pinned Claude conformance manifest, verifies complete
   source/effective MCP inventories against the required/optional transport
   matrix, reopens byte-equal source authorities, exposes only daemon-private
   materialized roots, sets cwd by transferred materialized descriptor, and
   sends only fixed-role inherited descriptors plus the exact effective MCP
   inputs. It rejects every pre-response authority update without reordering,
   requires the request-correlated non-empty response session ID, creates only
   provider-neutral readiness with null accepted pair, treats optional bounded
   `configOptions` as diagnostics, and orders later updates. It identity-safely reaps every post-launch
   ...
   it and no raw-ID field ever contains `psref_*`; before helper spawn, launch copies
   the descriptor-opened bounded native image or interpreter/script closure into
   the private immutable launch directory and reopens/byte-verifies its complete
   identity. Root/secret descriptors are assembled only after the inert helper
   has a durable process binding and are transferred in the authenticated release;
   post-spawn
...
16. every registered Class A operation crosses committed, busy/shutdown rejected,
    ...
    the generated registry manifest has zero missing/extra/drifting rows across
    operation, lane, request/result codec, replay rule, natural-owner selector,
    completion/proved-no-start callback, and shutdown policy; no request-scoped
    task owns final reconciliation and no mutation is resubmitted;
    scalar parent plus sealed ordered request-member rows replay a legal P079
    request larger than 16 KiB and all 200 descriptors; parent/member
    update/delete/late-insert negatives cover sealed
...
21. launch-barrier/process-binding crashes at every boundary cover original,
    ...
    identity-matched processes are reaped. The signed bundled LaunchServices
    helper remains inert before authenticated release; package/designated-
    requirement/architecture/entitlement drift disables launch. Exact
    SOCK_SEQPACKET/SCM_RIGHTS fixed-role inventory, `MSG_CTRUNC`, order/count,
    `fchdir`, `dup2`, CLOEXEC, EOF, and nonce/digest mutations prove zero secret
    or provider code before durable authority. Final provider profiles permit
    only pinned backend egress plus declared private materialized roots and stdio
    MCP, while inbound/listen, ambient sockets, source/canonical roots, and every
    undeclared filesystem syscall fail on all supported macOS releases;
...
29. migration 095 remains byte-identical and checksum-equal; migration 100
    ...
    checked-in normalized staging and final migration-100 manifests at their
    exact phases, while removal or rewriting of
...
30. direct-SQL P079 negatives reject active zero selected budget, contradictory
    ...
    use an empty set for accepted/rejected or create history/activation fails.
    Only the exact connection-local journal owner permits those terminal-guard
    exceptions and `settle_activation_conflict`; unset, stale, wrong-operation,
    or direct-SQL owner values abort, while crash/replay positives converge. Each
    ...
    phase/chunk cursors and proves the 10-second no-new-chunk scheduling cutoff,
    16-set/256-member limits, 8-MiB soft and 12-MiB hard budgets, 1-MiB chunks,
    and exact 10-MiB member convergence plus
...
32. fresh `AppSchema::sdl()` byte-matches the checked-in snapshot, every new enum
    ...
    continuation-generation-B turn truth, and the exact production protocol and
    truth schema probes, generated old/v2 P031 document inventory,
    `P031RunDetail`, `P031RunStageTopologyPage`,
    ...
    against real transports, resolvers, and decoders; both mixed-version P031
    directions fail at the capability/document middleware before resolver/DB
    work. The SDL freezes the exact additive `durableAfterCursor` subscription
    argument, snapshot schema literal, cursor-mode errors, strict malformed-
    `runId` rejection, pre-schema document/variable normalization, the complete
    thirteen-code HTTP/WebSocket `timeline_graphql_error_v1` matrix, and old-
    client live-only Timeline coexistence, while the closed
...
33. same-agent interleaved execution events preserve execution, occurrence,
    ...
    pages under one registered Class A lease, publishes the newest page, persists and resolves either the event
    ...
    only a new Timeline generation on gap/overflow. Run-local signed-64-bit
    sequence and separate private/wire digest vectors prove `A1,B1,A2` is
    contiguous for run A, `A1,A3` is a gap, and Swift verifies only the public
    wire fields; zero-event upper/sentinel, lease/anchor expiry, and pruning are
    exact. The 512-event/8-MiB aggregate
...
35. topology association and layout fixtures cover all source kinds, legacy
    ...
    immutable topology snapshot, 32-row occurrence-attempt pages under a second
    immutable presentation-row-bound membership snapshot (including null-ID
    `legacy_unique` and rejection of `legacy_ambiguous`), transition
    ...
    can never invoke restart or mutate the original snapshot. Operator-only
    frozen-definition readback plus command-journaled replacement mutation is
    idempotent, no-oracle for Agent/Observer, and leaves the original bytes
    unchanged across every denial/crash;
...
37. the single `RunOccurrenceSelectionReducer` covers run change, planned-to-
    ...
    initial/older attempt controls and each attempt row bind the exact selected
    row, topology snapshot, immutable attempt snapshot, attempt-page generation,
    member execution, and page cursor. A background row removal clears a disappearing focused registration
    ...
    `FrozenInputRepairRouteV1`, authorized snapshot client, and same-window
    app-shell coordinator, keeps
    ...
    The 72-cell Inspector composition matrix preserves Run snapshot, Approval,
    Output Contract Repair, and Recovery IDs/actions while selection context and
    attempt history vary;
38. accessibility fixtures exhaust every legal subject/control matrix cell and
    byte-compare exact label/value/hint/AXRole/AXEnabled tuples,
    ...
    Timeline legacy-gap, distinct Inspector attempt rows plus initial/older
    attempt-page loading/failure/retry, and
    ...
    and spoken output; the exact 32-case `920 x 620|1440 x 900` x all eight
    requested outer-navigation/Runs-sidebar/Inspector states x
    `.large|.accessibility3` matrix proves normalized column widths, the new
    production `920 x 620` content minimum, at least 460 pt detail reachability,
    and no
...
- [ ] P079 repair uses `OutputContractRepair`; its fallback child remains an
      ...
      candidate or source mutations outside the verified daemon-private
      materialized staging root fail at the OS boundary, while bounded adapter
      state is confined to a
      ...
      blocked. Startup settlement is cursor-addressed; after the 10-second
      continuous-time scheduling cutoff it starts no new chunk. Each attempt is
      bounded to 16 sets, 256 members, an 8-MiB soft and 12-MiB hard budget, and
      1-MiB durable chunks, resumes from the last durable member/phase/chunk,
      converges one legal 10-MiB member across restarts, and never rescans a
      completed sequence.
...
- [ ] P086 admission atomically commits command, continuation/context, one
      ...
      verifies the complete source/effective MCP inventory pair and closed
      required/optional transport matrix, reopens byte-equal source authorities,
      and exposes only their verified daemon-private materialized roots under the
      mandatory syscall-enforced profile. It uses only the manifest-pinned frozen supported
      ...
      pre-response option updates and requires a request-correlated non-empty
      response session ID plus provider-neutral readiness; optional bounded
      `configOptions` remain diagnostic and create no accepted pair. Every
      post-launch failure reaps by identity; every mode owns
...
- [ ] `AppSchema::sdl()` and both protocol/truth schema probes are exact. The
      production `P031RunDetailProtocolProbe`,
      ...
      selections. Generated old/v2 P031 document inventory and capability
      middleware reject both mixed-version directions before resolver/DB work.
      Run-detail requests bounded non-topology truth; the two paged topology
      operations request every occurrence/transition/attempt field under exact
      row/byte/snapshot caps, and attempt pages freeze membership by
      presentation-row-bound snapshot so nullable legacy occurrence IDs remain
      addressable. Shipped DTOs distinguish omission from null
      ...
      pre-async-graphql exact-document/variable normalizer, complete thirteen-
      code `timeline_graphql_error_v1` HTTP/WebSocket decoding,
...
- [ ] Operator-only `frozenRunDefinition` returns the exact bounded frozen
      workflow/catalog/run-plan bytes and aggregate digest used by
      `FrozenInputRepairRouteV1`; Agent/Observer and cross-run callers receive no
      existence or payload oracle. `createReplacementRunFromFrozenSnapshot`
      revalidates those bytes, compiles the replacement workflow against the
      frozen catalog, journals the complete intent, creates one linked
      replacement run, and leaves the original immutable. Same request/intent
      replays one result; request conflict, unauthorized caller, stale target,
      digest mismatch, invalid YAML, and crash at every commit boundary mutate
      neither run.
...
- [ ] Timeline and raw-detail identity preserve exact event, execution,
      ...
      Run-local signed-64-bit sequence and separate private/wire digests have
      Rust/Swift known-answer vectors; `A1,B1,A2` is contiguous for run A while
      `A1,A3` is a gap. Every snapshot has a registered Class A lease owner,
      exact zero-event upper bound/sentinel, bounded lease/anchor retention, and
      deterministic expiry/full-refetch behavior.
      ...
      target; retry allocates one new Timeline generation. Inspector initial and
      older-attempt loading/failure/retry plus every retained attempt row publish
      only their selected-row/topology/attempt-snapshot/page-generation/cursor-
      qualified targets and reject every stale callback.
...
- [ ] One `RunOccurrenceSelectionReducer` owned by
      ...
      load/loading/retry are separate exact targets. Inspector initial/older
      attempt paging and each retained attempt are separate selected-row/
      topology-snapshot/attempt-snapshot/page-generation/cursor-qualified states
      and targets. Run-load and topology recovery are separate window-owned
      ...
      the exact authorized immutable-snapshot replacement-run flow, and
      ...
      `AppShellNavigationCoordinatorV1`, retrieves the bounded Operator-only
      snapshot bytes, opens `FrozenRunRepairV1` in Definitions in that same
      window, and submits only the separately authorized, command-journaled
      idempotent replacement mutation. The original snapshot remains immutable.
      ...
      `RunInspectorCompositionV1` independently preserves run snapshot,
      Approval, Output Contract Repair, and Recovery sections while selection
      context/attempt sections change; the complete 72-cell composition matrix
      proves no governed action disappears or is rerouted.
- [ ] Accessibility implements the exhaustive subject/control matrix and exact
      generated `(label,value,hint,AXRole,AXEnabled)` tuple. Only
      ...
      generation-qualified run and Timeline initial loading/failed/retry,
      Timeline legacy gap, Inspector attempt rows and initial/older attempt-page
      ...
      prove every legal pair, reject illegal pairs, run the exact 32-case
      `920 x 620|1440 x 900` x all eight requested outer-navigation/Runs-sidebar/
      Inspector states x `.large|.accessibility3` matrix after enforcing the
      production `920 x 620` content minimum and 460-pt detail bound, without
      overlap/clipping. The production reducer accounts for exact outer
      `92|260`, Runs `0|280`, Inspector `0|360`, and visible-boundary widths and
      honors the triggering column while applying the frozen collapse order.
      Tests assert real `.focused`/
