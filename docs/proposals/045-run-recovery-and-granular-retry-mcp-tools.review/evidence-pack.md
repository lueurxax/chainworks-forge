...
| DOC-02 | `docs/proposals/041-server-parity-harness-golden-runs-and-behavioral-diff.md` | 2026-04-17 | Medium | Declared dependency is still Draft and focuses on parity harness/golden runs, not durable resume cursor or granular retry persistence. | Proposal could assume unavailable prerequisite behavior. | Dependency validation. |
...
| TEST-04 | Parity/golden harness | P041 dependency | Draft dependency exists; some golden harness tests are present in engine integration | Add recovery golden scenarios only if P041 becomes current proof owner | 2026-04-17 | Medium | Dependency is not enough by itself. |
