...
### Junie structured-output canary proof

Junie is covered by a retained structured-output proof lane under the
`proposal-089|p089` gate alias. The proof has two durable parts:

- native Junie CLI evidence that strict JSON and strict `CHAINWORKS_OUTPUT`
  can be returned without prose; and
- an ACP `code_writer` canary using the production `code_writer` catalog
  binding, `junie_code_editor_acp` backend profile, `JunieAdapter`, and the
  full production output set.

The ACP canary accepts only a narrow final `CHAINWORKS_OUTPUT` payload for the
agent-authored outputs. `implementation_progress`,
`implementation_self_assessment`, and `tests_result` are Junie-authored
provider-envelope outputs; `changed_files_manifest` is generated by the control
plane and does not count as Junie capability evidence. The canary feeds the
discovered artifacts through the same engine-owned
`generate_changed_files_manifest_if_declared` and
`settle_agent_outputs_from_discovery_decisions` settlement/materialization
functions used by the normal executor path.

This proof is scoped to provider capability plus the small ACP code-writer
handoff boundary. It does not replace post-generation contract validation and
does not claim that longer implementation attempts cannot fail.
...
- retained `proposal-089|p089` evidence validation for Junie native structured-output and ACP `code_writer` canary proof,
