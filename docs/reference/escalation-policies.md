...
- Durable SQLite tables created by migrations `063_p058_escalation_schema.sql` (`escalation_ledger`, `escalation_execution_metadata`, `escalation_events`) and `064_p058_escalation_redaction_version.sql` (`redaction_version` enforced at insert time), with repository-layer JSON validation that holds even without sqlite json1.
- Idempotency enforced at the persistence layer by migration `065_p058_escalation_idempotency.sql`: a unique index on `escalation_ledger(run_id, stage_id, agent_id, policy_id)` prevents duplicate chain creation, and a unique index on `escalation_execution_metadata(escalation_ledger_id, tier_id, tier_attempt_index)` enforces the proposal idempotency key (`run_id`, `stage_id`, `ledger_id`, `tier_id`, `tier_attempt_index`) at the tier-attempt grain.
...
The implementation-closeout path now enforces kill-switch pauses, chain wall-clock deadline pauses, launch-recycle storm pauses, provider force-detach pauses, late-frame event journaling, startup force-detach replay, and the default three-failure capacity-probe pause threshold before launching an escalation retry. Remote visual soak, long-run threshold trending, and live operator-restart drills remain release evidence items rather than missing runtime paths.
