...
This document details the architecture, configuration, and operational aspects of the Configurable Agent Escalation Chains feature. This system allows for defining auditable ordered escalation chains for agent-driven workflows, providing robust recovery paths and transparent operational controls.

## Implementation status

Current head implements the P058 escalation-policy foundation and the first scheduler-owned tier execution path:
...
The implementation-closeout path now enforces kill-switch pauses, chain wall-clock deadline pauses, launch-recycle storm pauses, provider force-detach pauses, late-frame event journaling, startup force-detach replay, and the default three-failure capacity-probe pause threshold before launching an escalation retry. The governed macOS read surface is adapter-owned and covered by focused P058 tests for status capsule field order/truncation/accessibility, pause-card countdown/metadata and ultra-narrow fallback, command disabled-reason parity, actual `DriftReviewSheet` structured tier/trigger/max-attempt inputs, compact banner co-occurrence summarization, non-collapsed lineage disclosures, SF Symbol availability, drift diff presentation, lineage retry collapse/duration/ref disclosure, MenuBarExtra overflow routing, all-run attention aggregation, retained inspector adapters, and atomic trace pasteboard copy. Remote visual soak, long-run threshold trending, and live operator-restart drills are tracked by P096 as release evidence rather than missing P058 implementation paths.
...
| `provider_session_force_detached`        | Inspect provider state and resume manually; do not retry through identity-ambiguous holds.    | `runtime` | `escalation/provider-session-force-detached`     |
| `escalation_recovery_inconsistent`       | Use v1 cancellation unstick; resolve provider identity before retrying held sessions.         | `runtime` | `escalation/recovery-inconsistent`               |
...
Chainworks currently has isolated conflict mediation and same-profile implementation retries, but operators cannot declare an auditable ordered escalation chain. Repeated blockers can burn time and quota without changing capability tier or producing durable lineage.
...
### Phase 3
- Scope: provider quota waits, dwell/timeouts, recovery invariants per trigger, graceful classifier contract hardening.
...
- **scheduler regression after Phase 2**
...
- **provider cancellation advances incorrectly**
  - Mitigation: force-detach pauses with `provider_session_force_detached`, no automatic advancement, replay idempotency, identity-ambiguous holds, and late-frame drop event.
- **planned restart trips storm detection**
...
- **Should reserved `operator_forced_escalation` become an authorized `command_journal`-backed MCP/GraphQL mutation later?**
...
- **Should server own humanized labels instead of the macOS frozen label catalog?**
  - Default: client owns labels in v1 while raw strings remain authoritative; revisit if other clients needs identical wording
- **Should policy-drift acknowledgement become a governed macOS action in a later proposal?**
...
## UI/UX Notes (Summarized)

The proposal includes extensive UI/UX notes for macOS, covering accessibility, authority boundaries, various components (DriftReviewSheet, EscalationBannerStack, EscalationCommandPresentation, EscalationLineageView, EscalationPauseCard, EscalationStatusCapsule, EscalationTraceTimeline, MenuBarExtra), cooccurrence resolution, density ladder, fixtures, notifications, presentation style ownership, read pipeline states, run detail header layout, and screen state matrix. These notes ensure a consistent and high-quality user experience for the escalation system within the macOS application.
...
## Rollout Contract v1 (Summarized)

The `rollout_contract_v1` defines the requirements and behavior for the P058 rollout. It includes:
- Applicability and gate aliases (`proposal-058`, `p058`).
- Commands for testing (`./scripts/test-gate.sh proposal-058`).
- Migration details: Escalation policy ledger, execution metadata, runtime facts, and readback/report projections are required.
- Operational and adoption metrics.
- Readback lanes and fields for operator visibility.
- Hold conditions that would block the rollout (e.g., missing schema compile/readback parity, non-durable ledger data).
- Rollback disposition: `disable_escalation_policy_behavior_keep_readback` with low data loss risk.
- Decision vocabulary and negative fixtures.
- Cutover policy details.
...
## Review Feedback Resolution (Summarized)

The proposal addresses numerous review feedback items, categorized by source (e.g., APPLE, REL, MAC, UI). Resolutions include:
- Removing governed macOS server mutations and making `DriftReviewSheet` read-only.
- Pinning `EscalationReadAdapter` ownership and `EscalationPresentationStyle` for UI consistency.
- Specifying shutdown drain handoff, SQLite `BUSY/LOCKED` retry logic, and recovery-cancelled marker commits.
- Adding specific metrics (`escalation_repeated_digest_no_progress_total`).
- Clarifying human-tier outage credit.
- Pinning SF Symbols resolution, `NSApp.requestUserAttention`, `DriftReviewSheet` dismissal policy, Dock badge derivation, `MenuBarExtra` compact item rendering, pause card behavior, full keyboard access tab order, and `NSPasteboard` writes.
- Defining `read_pipeline_states` matrix, `EscalationCommandPresentation` slots, `MenuBarExtra` layout, lineage column width policy, compact banner cooccurrence, shadow-row visuals, `same_backend_retry` color rules, `RunDetailHeader` vertical order, `EscalationTraceTimeline` placement, co-occurrence resolution for capsule state, and drift policy summary.
