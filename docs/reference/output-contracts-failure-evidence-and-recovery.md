...
## Code-Writer Completion Freshness and Repair (P088 retained alias)

`code_writer` work completion and required-output settlement are separate facts.
The executor captures `worktree_fingerprint_v1` before and after the original
prompt and classifies implementation-owned changes as `current_attempt_diff`,
`preexisting_dirty_work`, control-plane-only work, generated-evidence-only work,
or no work. `changed_files_manifest` is supporting evidence generated by the
control plane; it cannot establish current-attempt freshness by itself.

Only a current-attempt implementation change may enter the dedicated
`code_writer_completion_repair_v1` turn. The sole historical exception is an
explicit operator retry carrying the preserved evidence packet for the original
attempt. Stale files, inherited dirty work, empty manifests, and generated-only
changes fail closed. A usable final `CHAINWORKS_OUTPUT` always follows normal
declared-output settlement and does not consume completion repair.

The completion turn:

- reuses the current live session and the existing one-turn repair budget;
- asks only for missing declared outputs;
- accepts mutations only at declared output and engine-owned evidence paths;
- records original and repair prompt runtime receipts independently;
- fails closed when the generic repair turn already consumed the repair budget;
- never converts provider/runtime failure into successful output truth.

Every candidate persists a canonical `code_writer_completion_receipt_v1` linked
to its `agent_execution_id`, prompt-level completion-text captures, per-output
decisions, worktree fingerprint paths and hashes, and either transcript/runtime
evidence or typed absence reasons. Receipt writes are transactional and
idempotent; byte-different replay returns `completion_receipt_conflict`. A crash
after the receipt artifact write but before the DB transaction is recovered at
startup as `completion_receipt_partial_write`. Startup recovery is enabled by
default and scans at most 256 P088 directory entries per active run; operators
may lower that bound, and `0` explicitly disables the scan.

Bounded capture distinguishes `terminal_response_capture_truncated_before_output`
from `extraction_input_truncated`; neither is treated as a clean missing-output
result.

The P037 stale-active canary uses the same receipt path. A `code_writer` attempt
with real current-attempt changes, missing outputs, and an idle-terminalization
failure is terminalized with `activation_source=p037_idle_terminalization`; it
must not enter the generic active-prompt auto-requeue path. Explicit targeted
recovery uses `activation_source=operator_retry_completion_recovery` and keeps
the source execution and evidence packet in retry authority history.

Canonical readback is exposed as:

- GraphQL `implementationCompletion` by run and execution;
- MCP `runs.get`, `runs.list`, `reports.get`, and `report://{run_id}`;
- run-report `implementation_completion`;
- the read-only Swift run-row/timeline presentation.

Readback selects the receipt linked to the active artifact-contract generation,
not the most recently created historical row. Public enum projections preserve
unknown future values as `{ value: "unknown", raw: <original>, known: false }`.
The closed completion-turn result vocabulary is `not_attempted`, `succeeded`,
`failed_missing_outputs`, `failed_schema_validation`,
`failed_unexpected_worktree_mutation`,
`generic_repair_already_failed_completion_contract_required`,
`skipped_ineligible`, `skipped_no_live_session`,
`skipped_usable_final_output_settled`, and `unknown`. The same unknown wrapper
applies to completion status, ingestion-boundary failure, completion-boundary
subtype, and next-operator-action projections; their canonical values are owned
by `domain::code_writer_completion` and protected by the retained gate.

`proposal-088` and `p088` remain stable historical aliases for the deterministic
ACP/domain/DB/engine/GraphQL/MCP proof suite. Historical proposal and audit text
is not operational truth after reference promotion.
