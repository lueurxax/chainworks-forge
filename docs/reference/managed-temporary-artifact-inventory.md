...
| Implementation readiness | Implemented across the MCP, GraphQL, run report, release receipt, and packaged Swift Run Report lanes. Redaction-key startup initialization and contract-fixture reconciliation are complete (see [§6.1](#61-redaction-key) and [§4.4](#44-graphql-projection-specifics-implemented)). `operator_visible` promotion is still held on packaged remote UI/accessibility evidence and a refreshed operator-readback fixture; deletion, cleanup, durable persistence, and scheduled sweeps remain out of scope. |
| Implementation gate | `./scripts/test-gate.sh proposal-089-temp-inventory` / `p089-temp-inventory` |
| Author | Codex |
| Implementation evidence | Nine contract fixtures are checked in under [`docs/evidence/089/temp-inventory/contracts/`](../evidence/089/temp-inventory/contracts) (GraphQL SDL, MCP request/result schemas, run-report and release-receipt schemas, enum projection matrix, DateTime nullability parity, status-by-field matrix, over-2GB `ByteCountString`), reconciled with the shipped schema (see [§4.4](#44-graphql-projection-specifics-implemented)). Rust conformance tests in the domain/mcp-server/graphql-server crates diff the graphql-sdl, mcp-result-schema, enum-projection, datetime-nullability, and status-by-field-matrix fixtures against a live introspected schema, real emitted MCP payloads, and the shipped domain enums/classifier; the run-report and release-receipt schema fixtures remain presence-only. Operator readback: [`p089-temp-inventory-full-surface.fixture.json`](../evidence/rollout-contract/operator-readback/p089-temp-inventory-full-surface.fixture.json) — no lane records a captured operator observation yet: `run_report`, `release_receipt`, and `graphql` are `evidence_not_captured`, `mcp` is `implemented_smoke_slice`, and packaged smoke is `not_run` on every lane that declares it (`graphql` omits the field entirely). Negative fixtures: `docs/evidence/rollout-contract/negative/p089-temp-inventory-*.json`. |
...
What remains is packaged remote UI/accessibility smoke and a refreshed operator-readback fixture, not lane wiring or contract-fixture drift: redaction-key initialization now happens eagerly at daemon startup (see [§6.1](#61-redaction-key)), all nine contract fixtures are checked in and reconciled with the shipped schema, and Rust conformance tests diff five of those fixtures (graphql-sdl, mcp-result-schema, enum-projection, datetime-nullability, status-by-field-matrix) against a live introspected schema and real emitted payloads rather than only checking presence. Packaged remote UI/accessibility smoke is still outstanding, and the operator-readback fixture has not been refreshed with captured observations. `operator_visible` promotion therefore stays held. The focused gate unconditionally requires all nine contract fixture paths plus passing operator-readback lane status and packaged-smoke status; it remains red until the packaged-smoke and operator-readback evidence is completed and refreshed (see [`test-gates.md`](test-gates.md#proposal-089-temp-inventoryp089-temp-inventory)).
...
The shipped GraphQL projection in `control-plane/crates/graphql-server/src/types/temp_artifact_inventory.rs` is the authority for the GraphQL lane. All nine contract fixtures under [`docs/evidence/089/temp-inventory/contracts/`](../evidence/089/temp-inventory/contracts) are reconciled with the shipped schema, using the shipped type names (not renamed to match the fixtures):

- **Status matrix.** `status-by-field-matrix.fixture.json` is checked in, deriving its `enum_domains` from §5 and its `classification_dry_run_mapping`/`age_thresholds_seconds` from §9.
- **Enum coverage.** The seven proposal-era domains are GraphQL enums whose item values render in `SCREAMING_SNAKE_CASE` (`COMPLETE`, `RESOURCE_EXHAUSTED`, `WOULD_KEEP_ACTIVE`) via `rename_items = "SCREAMING_SNAKE_CASE"`: `InventoryStatus`, `InventoryEnabledState`, `MutationGuardStatus`, `RootKind`, `LifecycleClassification`, `DryRunRecommendation`, `InventoryErrorCode`. Their value mapping matches `enum-value-projection-matrix.fixture.json`, which now also uses the shipped GraphQL type name `InventoryEnabledState` (not `EnabledState`) and covers the refinement-added `mode` field and closed `InventoryMode` enum (`disabled` | `hidden_readback` | `operator_visible`, no `unknown` member).
- **Type names and fields.** The implemented objects are `TempArtifactInventoryLimitsApplied` and `TempArtifactInventorySummary`, matching the SDL fixture. `TempArtifactInventorySummary` carries `queueWaitMs`. `TempArtifactWorkspaceContextInput.workspaceRoot` is non-null in both the shipped input and fixture.
- **Dry run.** `TempArtifactDryRun` is `{ schemaVersion: String!, generatedAt: DateTime, recommendationCounts: JSON!, mutationGuard: TempArtifactDryRunMutationGuard! }`, matching the SDL fixture; `recommendationCounts` is an untyped JSON object keyed by `dry_run_recommendation` value, not a typed counts object.
- **Mutation guard.** `TempArtifactMutationGuard` is `{ status, checkedAt, noDelete, noPrune, noChmod, noPersist, noRetry }`. `TempArtifactDryRun.mutationGuard` is the object `TempArtifactDryRunMutationGuard { status, checkedAt }`, not a bare `MutationGuardStatus!` enum. Both match the SDL fixture.
- **MCP result schema fixture.** `mcp-result-schema.fixture.json` includes the canonical top-level `mode` field and `summary.queue_wait_ms`, and includes the always-emitted `phase` field on error items, so a real MCP result validates against it. The authoritative MCP output schema remains the one the tool itself declares in `tools/temp_artifacts.rs`; the fixture is a stricter, enum-constrained supplement.
- **DateTime nullability.** `datetime-nullability-parity.fixture.json` marks release-receipt top-level `generatedAt` non-null, since the release receipt carries the same canonical DTO as the run report and always supplies that field.

Fixture-vs-shipped-schema conformance for the GraphQL SDL, MCP result schema, enum projection matrix, DateTime nullability, and status-by-field matrix fixtures is proven by Rust tests (`p089_graphql_sdl_fixture_matches_shipped_schema`, `p089_enum_value_projection_matrix_fixture_matches_shipped_projection`, `p089_datetime_nullability_parity_fixture_matches_shipped_graphql_schema` in `graphql-server/src/schema.rs`; `p089_mcp_result_schema_fixture_validates_real_*` in `mcp-server/src/tools/temp_artifacts.rs`; `p089_temp_inventory_status_by_field_matrix_fixture_matches_shipped_enums` in `domain/src/temp_artifact_inventory.rs`; `p089_status_by_field_matrix_fixture_*` in `mcp-server/src/tools/scanner.rs`) rather than asserted only by fixture presence. `run-report-inventory-schema.fixture.json` and `release-receipt-inventory-schema.fixture.json` remain presence-only — no conformance test diffs them against a real report/receipt payload yet.
...
### 6.1 Redaction Key
- The implemented key is a 32-byte HMAC key assembled from two UUID v4 byte arrays. It is initialized eagerly at daemon startup — `daemon::main::run_daemon` calls `init_p089_redaction_key_at_startup`, which invokes `domain::temp_artifact_inventory::init_process_path_hash_key()` before the P089 GraphQL backend is installed or any other lane becomes reachable — and then retained for the daemon process lifetime.
- The key is retained purely in memory and is never serialized, logged, or shared.
- `compute_path_hash` still reads the same process-scoped `OnceLock`, so a call before startup init (e.g. in a unit test that never calls the daemon entry point) still lazily materializes it; the `OnceLock` only ever runs the generator once per process, so the key is identical whichever path first triggers materialization. Startup-lifecycle proof: `p089_temp_inventory_redaction_key_initialized_eagerly_and_stable` (domain) and `init_p089_redaction_key_at_startup_materializes_key` (daemon).
...
`operator_visible` promotion is still held pending packaged remote UI/accessibility smoke and a refreshed operator-readback fixture; redaction-key initialization and contract-fixture reconciliation are complete (see [§1.1](#11-implementation-status-by-lane-current-slice)).
