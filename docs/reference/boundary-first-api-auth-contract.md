...
| `agent_operator` | `mcp_tools_call` | `p081.agent_operator.mcp_tools_call.command` | `runs.*`, `ideas.*`, `approvals.*`, `stages.*`, `reports.*`, `artifacts.*`, `steward.*`, `storage.*`, `runtime.*`, `boundary.*`, `effects.*`, `resources.*`, `temp_artifacts.*` (allow.wildcard=true) | `command_journal` | JSON-RPC -32004 with CAPABILITY_OUT_OF_SCOPE or MATRIX_NO_ROW | `allowed_mcp_command`, `denied_graphql_mutation`, `denied_out_of_scope_tool`, `mcp_command_idempotency` |
...
| `automation` | `mcp_tools_call` | `p081.automation.mcp_tools_call.command` | `runs.*`, `ideas.*`, `approvals.*`, `stages.*`, `reports.*`, `artifacts.*`, `steward.*`, `storage.*`, `runtime.*`, `boundary.*`, `effects.*`, `resources.*`, `temp_artifacts.*` (allow.wildcard=true) | `command_journal` | JSON-RPC -32004 with CAPABILITY_OUT_OF_SCOPE | `token_scope_matrix`, `denial_side_effect_test`, `mcp_command_idempotency` |
...
A caller-class wildcard is necessary but not sufficient: the principal-class capability check runs independently and can be narrower. `temp_artifacts.*` (the read-only inventory preview) is boundary-scoped to `agent_operator` and `automation`, but `auth::tool_allowed_for_class` admits only the `Operator` principal class — `Observer`, `Agent`, and `ReadOnlyOperator` are denied. It is also a direct tool: it writes no `command_journal` row despite sitting in a `command_journal` matrix row. See [managed-temporary-artifact-inventory.md](managed-temporary-artifact-inventory.md).
