...
| `ProviderSessionShutdown` | `provider_session.shutdown` | yes |
| `ProviderSessionMarkProcessAbsent` | `provider_session.mark_process_absent` | yes |
| `P083RollbackExecution` | `p083.rollback_execution` | yes |
| `P083SetEnforcementMode` | `p083.set_enforcement_mode` | yes |
| `RetryRun` | `runs.retry` | yes |
| `SideEffectsForceReconcile` | `side_effects.force_reconcile` | yes |
...
| `caller_tool` | MCP tool name (e.g. `runs.start`) or GraphQL command mutation name (e.g. `approveApproval`, `p083RollbackExecution`) |
