...
- **MCP** — JSON-RPC over stdio and Streamable HTTP (`/mcp`). Serves tools and resources to agents, CLIs, and automations. `runs.get` includes execution-truth rollout readback such as `rollout_contract_rollback_disposition` (wrapped per `rollback_disposition_v1`) and `p083_shutdown_queue_rank`; the command tools (`provider_session.shutdown`, `provider_session.mark_process_absent`, `p083.rollback_execution`, `p083.set_enforcement_mode`, `runs.retry`, `side_effects.force_reconcile`) are registered for authorized operator callers, validate lowercase UUIDv4 `caller_request_id` values, require `decision_json` for `side_effects.force_reconcile`, and dispatch through durable command-idempotency handlers.
- **GraphQL** — HTTP POST `/graphql` and subscriptions over `/graphql/ws`. Serves read queries, approval-gate decisions, operator lifecycle mutations, and event streams to the macOS operator UI. Execution-truth readback includes `p083RolloutContractReadback` on `Run`, the `p083IdentityHoldSessions` read query, the `RollbackDispositionJSON` output scalar with resolver-side validation, and operator-only lifecycle mutations for provider shutdown, process-absent confirmation, rollback, enforcement-mode changes, and run retry.
...
- the GraphQL approval mutation boundary, execution-truth operator lifecycle mutation surface, and legacy non-UI compatibility boundary
...
The stale-execution reconciliation tools (`p080.diagnostics.get.v1`, `p080.reconcile.request.v1`, `p080.clear_permanent_hold.v1`) have typed `CapabilityToolId` variants (`P080DiagnosticsGet`, `P080ReconcileRequest`, `P080ClearPermanentHold`) so they participate in standard `tools/list` capability filtering. In the current slice `p080.diagnostics.get.v1` and `p080.reconcile.request.v1` are allowed for `Operator` and `ReadOnlyOperator`; `p080.clear_permanent_hold.v1` is `Operator`-only. The `p080.reconcile.request.v1` handler admits `ReadOnlyOperator` only for `requested_action=diagnose_only` and returns `unauthorized_missing_capability` for `repair_if_safe` or `hold`. `ArtifactEntity` and `ReportEntity` MCP resources are `Operator`-only; ReadOnlyOperator has no default MCP resource capabilities (SEC-P080-001). Per-run row scoping is enforced at the handler: `p080.diagnostics.get.v1` rejects ReadOnlyOperator (and Agent) callers unless `filter.run_id` is present and authorized by `run_scope`, surfacing `unauthorized_missing_capability` while the shared auth helper records whether the internal cause was `auth_scope_required` or `auth_scope_violation`. The tools are dispatched by prefix into a separate `tools::p080` namespace in `control-plane/crates/mcp-server/src/server.rs` and call repo functions directly without emitting `command_journal` rows. `p080.diagnostics.get.v1` requires the `detection_only` rollout-control row before reading existing projection rows. `repair_if_safe` is enabled for `acp_startup_stale` when that rollout row is enabled in Phase 2+, and for `scheduler_ownership_drift` when that rollout row is enabled in Phase 3+; it still refuses with typed rollout/class errors before mutation for disabled classes, unsupported classes, or phases that have not reached the class threshold. `hold` returns `action_disabled_in_phase`, and `p080.clear_permanent_hold.v1` returns `action_disabled_in_phase` until a later hold/clear proposal enables Phase 5 semantics. The live reconciliation loop records `diagnosed` / `diagnose_only` readback for unpromoted stale candidates and auto-repairs promoted `acp_startup_stale` and `scheduler_ownership_drift` candidates with `event_source=live_loop`. `acp_prompt_stale` is delegated to P037 prompt supervision with zero P080 mutations; helper reaping and side-effect-adjacent repair remain fail-closed until their owning safety contracts exist.
...
`class` is one of `operator`, `agent`, `observer`, or `read_only_operator`. The `read_only_operator` class was added with P080 to back diagnostics-only bearers: it is admitted to `p080.diagnostics.get.v1` and `p080.reconcile.request.v1` (only for `requested_action=diagnose_only`), carries no MCP resource capabilities, and is rejected from every mutating tool including `p080.clear_permanent_hold.v1`.
...
P080 added one optional authorization-scope field used by `check_p080_run_scope` for restricted classes (`agent`, `read_only_operator`):

- `run_scope` (array of run_id strings, optional): When present and non-empty, the principal may only access P080 diagnostics for `filter.run_id` values that appear in the list; any other run is rejected fail-closed. When present and empty, every run is rejected. When absent on a restricted principal, every P080 call is rejected fail-closed (SEC-P080-001). Operator principals are never restricted by `run_scope`. The shared auth helper classifies these internal causes as `auth_scope_violation` or `auth_scope_required`; northbound P080 handlers surface the failure as `unauthorized_missing_capability`. The field is ignored on non-P080 surfaces.
...
The daemon also exposes an operator-only GraphQL lifecycle surface for non-UI callers that are explicitly authorized by capability/surface policy. Each mutation validates a lowercase UUIDv4 `callerRequestId`, builds a `CallerContext::graphql(...)`, dispatches through `CommandHandler`, and returns a `journalId`/`requestId` pair:
...
| `p083RollbackExecution` | `P083RollbackExecution` | Moves enforcement mode to an explicit `targetEnforcementMode` of `permissive` or `disabled` and writes rollback audit/idempotency rows. |
| `p083SetEnforcementMode` | `P083SetEnforcementMode` | Changes enforcement mode with transition journal, audit, and idempotency rows. |
...
Implementation: `control-plane/crates/auth/src/lib.rs` (`filter_tools`, `filter_resources`, `match_resource_uri`, `tool_allowed_for_class`, `resource_allowed_for_class`), `control-plane/crates/mcp-server/src/server.rs` (`handle_request`, `visible_tool_specs`), `control-plane/crates/graphql-server/src/schema.rs` (approval mutation resolvers, execution-truth operator lifecycle mutation resolvers, and explicitly quarantined legacy compatibility resolvers).
...
- `CallerContext::graphql(principal_id, &principal_class, mutation_name)` — built inside GraphQL command mutation resolvers after the capability check. Execution-truth operator lifecycle mutations also require a lowercase UUIDv4 `callerRequestId` and dispatch through the same durable command-idempotency handlers as their MCP counterparts. Legacy non-approval GraphQL mutation rows may still appear only through explicit compatibility fixtures while those resolvers remain compiled.
...
| `caller_principal_class` | `"operator"`, `"agent"`, `"observer"`, or `"read_only_operator"` (added with P080) |
...
| `p083RollbackExecution` | `GqlP083RollbackExecutionPayload { committed: Boolean!, targetEnforcementMode: String!, journalId: String!, requestId: String! }` |
...
- the typed `DeliveryPreflight` object on blocked MCP `runs.start`,
- the UI action boundary: governed SwiftUI uses GraphQL read/subscription plus approval-only mutations; MCP owns non-approval operational commands for the governed UI while execution-truth operator GraphQL lifecycle mutations remain non-UI command surface,
