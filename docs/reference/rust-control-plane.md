...
> **Phases 1–3 implemented (Phases 4–8 deferred):** Phase 1 landed the DbWriter type, write classification types (`WriteClass`, `WriteLane`, `WriteOperation`, `WriteResult`), priority lane constants, write-bypass allowlist, and operation registry. Phase 2 wires the real bounded MPSC executor: per-lane channels at the documented capacities, priority drain (`CriticalBarrier`/`OperatorCommand` polled before lower lanes via biased `tokio::select!`), enqueue-to-commit deadline accounting (`WriteTimeout`), busy-error classification (`WriteBusyExhausted`), 1 Hz heartbeat with `is_alive()`, lane starvation watchdog incrementing `lane_starvation_total`, and graceful shutdown that rejects new B/C/D writes and drains Class A within `SHUTDOWN_CLASS_A_DRAIN_BUDGET_MS` followed by a Class B sub-budget flush. Phase 3 adds the Class B coalescing buffer (`CoalescingBuffer` keyed on `idempotency_key`, last-writer-wins, flushed every 500 ms via unconditional drain-all on each tick — `COALESCE_FLUSH_INTERVAL_MS`/`COALESCE_FLUSH_MAX_MERGES` — bounded at `COALESCE_MAX_KEYS=1024` distinct keys with `WriteRejected{reason="coalescing_map_saturated"}` and `coalesced_rejected_total` increment when full, force-drained on shutdown), per-lane oldest-enqueued tracking that populates `WriteRejected.oldest_queued_ms`, the populated `SHUTDOWN_ADMITTED_OPERATIONS` allowlist (terminal canonical operations such as `run_complete`, `stage_complete`, `agent_complete`, `approval_decision`, `operator_command_complete`, `projection_invalidation_terminal` are admitted during drain; all other Class A writes receive `WriteRejected{reason="shutdown_admission_denied"}`), structured-log fields per proposal §telemetry_metrics_and_thresholds.structured_logs (`write_id_hash`, `queue_wait_ms`, `tx_duration_ms`, `expected_rows`, `actual_rows`), and the evidence file spool module (`db::evidence_spool::write_spool_file` — temp write, SHA-256 checksum, `fsync(file)`, atomic rename, `fsync(parent_dir)` — paired with `verify_spool_file` for reader integrity). The full startup orphan sweep walk, telemetry rollup persistence, and `storageHealth` GraphQL/MCP diagnostics remain deferred to Phases 4–6. Behaviors not yet active are called out inline.
...
> **Phases 1 & 3 implemented (orphan sweep deferred):** The `evidence_spool_refs` and `storage_write_pressure_snapshots` schemas and metadata repository landed in Phase 1. The evidence file spool module (`db::evidence_spool::write_spool_file` / `verify_spool_file`: temp write, SHA-256 checksum, `fsync(file)`, atomic rename, `fsync(parent_dir)`) landed in Phase 3 alongside the high-volume "one metadata row per logical object" proof and security hardening (canonical layout enforcement, symlink-escape rejection, restrictive POSIX permissions — see below). The full startup orphan sweep walk (`startup_repairs` cross-check across active, completed, and abandoned runs) is the remaining Phase 3 deferral.
...
- **Run-Ownership Binding** (P075-SEC-001): `validate_spool_ref_fields` runs `validate_path_ownership` to require `relative_path` to start with `evidence/runs/{run_id}/`, so a producer cannot register evidence metadata under another run's subtree at insert time.
...
- **Verify Read Cap** (P075-SEC-004): `verify_spool_file` `stat`s the target before reading and rejects files larger than 512 MiB to prevent unbounded RAM allocation when the orphan sweep (Phase 4+) or another reader passes a large or attacker-influenced path. Streaming verification is reserved for larger artifacts.
