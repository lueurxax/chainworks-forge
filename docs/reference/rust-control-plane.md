...
> **Implemented closeout slice:** Phase 1 landed the DbWriter type, write classification types (`WriteClass`, `WriteLane`, `WriteOperation`, `WriteResult`), priority lane constants, write-bypass allowlist, and operation registry. Phase 2 wires the real bounded MPSC executor: per-lane channels at the documented capacities, priority drain (`CriticalBarrier`/`OperatorCommand` polled before lower lanes via biased `tokio::select!`), enqueue-to-commit deadline accounting (`WriteTimeout`), busy-error classification (`WriteBusyExhausted`), 1 Hz heartbeat with `is_alive()`, lane starvation watchdog incrementing `lane_starvation_total`, and graceful shutdown that rejects new B/C/D writes and drains Class A within `SHUTDOWN_CLASS_A_DRAIN_BUDGET_MS` followed by a Class B sub-budget flush. Phase 3 adds the Class B coalescing buffer (`CoalescingBuffer` keyed on `idempotency_key`, last-writer-wins, flushed every 500 ms via unconditional drain-all on each tick — `COALESCE_FLUSH_INTERVAL_MS`/`COALESCE_FLUSH_MAX_MERGES` — bounded at `COALESCE_MAX_KEYS=1024` distinct keys with `WriteRejected{reason="coalescing_map_saturated"}` and `coalesced_rejected_total` increment when full, force-drained on shutdown), per-lane oldest-enqueued tracking that populates `WriteRejected.oldest_queued_ms`, the populated `SHUTDOWN_ADMITTED_OPERATIONS` allowlist (terminal canonical operations such as `run_complete`, `stage_complete`, `agent_complete`, `approval_decision`, `operator_command_complete`, `projection_invalidation_terminal` are admitted during drain; all other Class A writes receive `WriteRejected{reason="shutdown_admission_denied"}`), structured-log fields per proposal §telemetry_metrics_and_thresholds.structured_logs (`write_id_hash`, `queue_wait_ms`, `tx_duration_ms`, `expected_rows`, `actual_rows`), and the evidence file spool module (`db::evidence_spool::write_spool_file` — temp write, SHA-256 checksum, `fsync(file)`, atomic rename, `fsync(parent_dir)` — paired with `verify_spool_file` for reader integrity). The bounded `sweep_evidence_orphans` walk is available via the `storage.reconcile_evidence_orphans` MCP tool, storage write-pressure snapshots have repository/readback support, and storage diagnostics are exposed through GraphQL `storageHealth` plus MCP `storage.health`, `storage.write_pressure`, and `storage.evidence_spool_summary`. Class D telemetry now has a production rollup lifecycle: the daemon starts a background write-pressure producer at the configured `TELEMETRY_FLUSH_CADENCE_MS`, writes compact `dbwriter_telemetry_rollup` snapshots through the telemetry lane, enforces payload/sample caps in the snapshot payload, merges duplicate time-window buckets by adding counters and taking max gauges, and purges `storage_write_pressure_snapshots` to the proposal retention bound of 24 hours or latest 288 windows. The P075 gate now fails closed on malformed write-bypass and operation-registry state, any `temporary_rollout` bypass row, production runtime transaction paths that bypass DbWriter-owned entrypoints, missing Class D rollup producer/retention wiring, or missing duplicate-window telemetry merge proof.
...
- **Single Bounded Gateway**: Non-test runtime writes enter through `DbWriter` or DbWriter-owned registered transaction helpers for multi-row invariants. The source-controlled P075 write-bypass allowlist no longer accepts rollout inventory entries; it is limited to permanent infrastructure scopes (migrations, tests, and startup repair).
...
> **Implemented:** The `evidence_spool_refs` and `storage_write_pressure_snapshots` schemas and metadata repositories are present. The evidence file spool module (`db::evidence_spool::write_spool_file` / `verify_spool_file`: temp write, SHA-256 checksum, `fsync(file)`, atomic no-replace commit, `fsync(parent_dir)`) provides the high-volume "one metadata row per logical object" path and security hardening (canonical layout enforcement, symlink-escape rejection, restrictive POSIX permissions — see below). The bounded orphan-sweep walk (`db::evidence_spool::sweep_evidence_orphans`: walk `evidence/runs/`, parse canonical layout, skip temp files and oversized candidates, stream-hash candidates in bounded chunks, backfill missing `evidence_spool_refs` rows with `producer_operation = "p075_evidence_spool_ref_recovery_sweep"` and `status = recovered_orphan`) is implemented with per-pass `max_files`/`max_bytes` budgets (defaults `SWEEP_DEFAULT_MAX_FILES = 1000`, `SWEEP_DEFAULT_MAX_BYTES = 64 MiB` per proposal §architecture.evidence_spooling.orphan_recovery.bounds), an optional `run_id` scope filter, a `dry_run` mode that computes checksums without inserting metadata, and an `OrphanSweepReport` that includes `bytes_read` and `truncated` so callers can tell when a budget cut a pass short. A candidate larger than the remaining byte budget is skipped without being read and truncates the pass. It is exposed through the `storage.reconcile_evidence_orphans` MCP tool, which takes camelCase `runId`, `dryRun`, and `maxFiles` parameters per proposal §mcp_contract — `runId` is required for non-dry-run calls (binding recovered metadata to a real run; SEC-001) and `artifact_root` is resolved server-side from `CHAINWORKS_META_ROOT` or the `DATABASE_URL` parent rather than accepted from the client. Storage health readback summarizes writer policy, WAL thresholds, evidence-spool counts/bytes, latest write-pressure snapshot freshness, telemetry-rollup units, and kill-switch state, and the GraphQL `storageHealth` query returns the typed `StorageHealth` SDL object (`writer`, `wal`, `projections`, `evidenceSpool`, `killSwitches`, `thresholds`, plus `updatedAt`/`staleAfterMs`/`isStale`) per proposal §graphql_contract.schema_delta — not an opaque JSON blob — with fail-closed defaults that map an absent or unrecognised `dbState` to `DEGRADED`, absent `writer.alive` to `false`, and no live writer heartbeat to stale/degraded readback rather than healthy readback. The daemon constructs one shared `db::writer::DbWriter` and injects its `DbWriterHeartbeat` into the GraphQL schema (via `build_schema_with_storage_writer`) and the MCP server (via `McpServer::new_with_storage_writer`); `db::repos::storage_health::storage_health_with_writer` consumes the injected heartbeat to populate `writer.alive`, `writer.totalQueued`, `writer.lastHeartbeatAt`, `writer.lastDrainAt`, `writer.lanes[*].queuedDepth`/`oldestQueuedAgeMs`, write-lock p50/p95, transaction-duration p95, and `writer.rejectedTotal` from the live snapshot, and only falls back to fail-closed defaults when no heartbeat is supplied (e.g. tests or static helpers). MCP storage diagnostics return a typed error envelope with `error: true`, `errorCode`, `message`, and `tool` fields rather than JSON-RPC -32603 envelopes for domain-level failures: `invalid_input` (e.g. `storage.reconcile_evidence_orphans` without `runId` on a non-dry-run call), `stale` (`storage.health` when `isStale=true`), `unavailable` (storage backend cannot serve the request), `maintenance_disabled` (kill switch or unresolvable artifact root), and `unauthorized` (operator capability boundary). GraphQL `storageHealth` and MCP storage diagnostic tools are operator-only because they expose WAL, queue pressure, orphan counts, and kill-switch state.
...
- **Canonical Layout Enforcement** (P075-SEC-002): `write_spool_file` rejects any `relative_path` that does not start with `evidence/runs/`, mirroring the proposal §architecture.evidence_spooling.layout (`evidence/runs/{run_id}/stages/{stage_id}/agents/{agent_id}/{kind}/...`). Producers cannot write spool files outside this layout.
...
- **Write-Bypass Allowlist**: A checked-in TOML file at `control-plane/crates/db/write-bypass-allowlist.toml` tracks only permanent direct DB write exemptions: migrations, tests, startup repair, and evidence-spool orphan repair. Temporary rollout bypasses are rejected at Phase 8. Every entry requires an owner, reason, scope, retirement criteria, and an expiration phase.
...
- **Gate Enforcement**: The `proposal-075` gate runs fail-closed and fails on temporary rollout bypasses, production runtime transaction paths that bypass DbWriter-owned entrypoints, entries missing retirement data, allowlist entries whose `expires_after_phase` is below the current closeout phase, or observed non-test DbWriter operation names not present in the registry.
