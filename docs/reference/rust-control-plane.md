...
| `engine` | `crates/engine/src/lib.rs` | Orchestrator, command handler, background executor, work queue, recovery service, event bus, mediation settlement, and run-start rollout-contract preflight. The shutdown recovery classifier runs at daemon startup for unresolved signal side effects and identity-ambiguous provider sessions. |
| `graphql-server` | `crates/graphql-server/src/lib.rs` | async-graphql schema (queries, mutations, subscriptions) served over axum. Exposes `Run.p083RolloutContractReadback` and validates `RollbackDispositionJSON` output. |
| `mcp-server` | `crates/mcp-server/src/lib.rs` | MCP JSON-RPC server with tool dispatch, resource reads, stdio and HTTP transports. Enriches `runs.get` rollout readback and registers operator-only command tools for provider-session shutdown, process-absent confirmation, rollback execution, enforcement-mode changes, run retry, and side-effect force-reconcile; those tools validate caller request ids and dispatch through durable command-idempotency handlers. |
...
Queries: `ideas`, `idea`, `runs`, `run`, `stages`, `approvals`, `artifacts`, `storageHealth`, `p080Diagnostics`.
...
**Execution-truth rollout contract readback and operator mutations:**
...
Mutations: `approveApproval`, `rejectApproval`, plus operator lifecycle mutations `providerSessionShutdown`, `p083MarkProviderSessionProcessAbsent`, `p083RollbackExecution`, `p083SetEnforcementMode`, and `runsRetry`. The governed SwiftUI boundary remains read/subscription plus approval-only mutation; execution-truth GraphQL lifecycle mutations are operator-only command surface for explicitly authorized non-UI callers.

GraphQL is the macOS UI read/subscription surface plus the approval-gate
settlement surface. It also exposes the execution-truth operator lifecycle mutations named
...
**P080 stale execution diagnostics readback and phase-scoped repair:** the `p080Diagnostics` query and `p080DiagnosticsUpdates` subscription are registered read-only GraphQL surfaces and require operator-read capability. The query is DB-backed over `p080_readback_heartbeats_v1` when `live_disable` is off and `detection_only` is enabled; it supports filter fields, page-size clamping, and optional budgeted `total_count`. The subscription emits current matching rows as `InitialSnapshotRow` events, then polls the projection and emits `RowUpdated`, `RowRemoved`, `ProjectionRebuilt`, or `AuthorizationLost` events until the client disconnects or authorization/gates fail. MCP `p080.diagnostics.get.v1` and `p080.reconcile.request.v1` are allowed for `Operator` and `ReadOnlyOperator`; `p080.clear_permanent_hold.v1` is Operator-only. The `p080.reconcile.request.v1` handler admits `ReadOnlyOperator` only for `requested_action=diagnose_only` and rejects `repair_if_safe` or `hold` with `unauthorized_missing_capability`. `ArtifactEntity` and `ReportEntity` MCP resources are Operator-only — ReadOnlyOperator carries no default MCP resource capabilities (SEC-P080-001). Per-run row scoping is enforced at both surfaces: restricted GraphQL and MCP callers fail closed unless `filter.run_id` or `target.run_id` is within configured `run_scope`; northbound handlers surface these failures as `unauthorized_missing_capability` while the auth helper distinguishes `auth_scope_required` from `auth_scope_violation` internally. MCP `p080.diagnostics.get.v1` reads existing `p080_readback_heartbeats_v1` rows only when the `detection_only` rollout-control row is enabled; otherwise it fails closed with `rollout_disabled`. When `live_disable` is off and `detection_only` is enabled, the live loop refreshes classifier rows, auto-repairs promoted `acp_startup_stale` and `scheduler_ownership_drift` stale candidates, and still writes diagnose-only readback heartbeats/events for stale candidates whose class or phase is not promoted. `acp_prompt_stale` is delegated to P037 prompt supervision with zero P080 mutations. Operator MCP `repair_if_safe` can requeue `acp_startup_stale` in Phase 2+ and `scheduler_ownership_drift` in Phase 3+ after the matching rollout row is enabled; mutating repairs require durable operator-request dedup and revalidate an ownership-witness predicate hash over readback, work-item, agent/session, side-effect, helper-lease, and executor-watchdog state before mutation. Helper reap, side-effect-adjacent repair, manual hold, and permanent-hold clear remain disabled in the current P080 closeout because P076 is observe-only and the helper/P098 safety contracts are not present in mainline. Operator runbooks for stale-execution repair and live-loop termination live at [`docs/runbooks/p080-stale-execution-repair.md`](../runbooks/p080-stale-execution-repair.md) and [`docs/runbooks/p080-reconciliation-loop-termination.md`](../runbooks/p080-reconciliation-loop-termination.md); the rollout-critical alert rules and overview dashboard live at [`docs/evidence/alerts/p080-alerts.yaml`](../evidence/alerts/p080-alerts.yaml) and [`docs/evidence/dashboards/p080-overview.json`](../evidence/dashboards/p080-overview.json); per-phase rollout/readiness artifacts collect under [`docs/evidence/rollout/p080/`](../evidence/rollout/p080/).
...
- **Compute state** -- creates a `StageExecution` with status `Running`, enqueues `InvokeAgent` work items for each task. If no explicit tasks, the owner agent runs as a single task. Upon completion of an `InvokeAgent` task, the workflow engine's output settlement process is enhanced by P079 (Contract-Aware Output Repair and Provider Fallback): deterministic fixture same-session repair can repair eligible missing/invalid required outputs before marking the stage as `Blocked`, and bounded transcript/provider-envelope recovery can accept transport-attributed current-execution output. Production same-session repair remains fail-closed for advisory-only providers until enforceable sandbox/permission restrictions exist. Controlled provider fallback dispatch is not P079 closeout scope; fallback rows remain readback/idempotency substrate only.
...
3. **`session/prompt`** -- submit the prompt. Stream `session/update` notifications until the terminal response arrives. When output settlement detects missing/invalid required outputs, the engine may first recover transport-attributed provider-envelope output and may dispatch a P079 same-session repair turn over the same session when the adapter has an enforceable permission boundary. Controlled provider fallback dispatch is not part of the current P079 runtime.
...
| `p080_rollout_control_v1` | P080: per-class rollout-control rows seeded default-disabled, including the `live_disable` row and generation counter used for fail-closed enforcement |
| `p080_rollout_control_audit_v1` | P080: additive audit table for rollout-control transitions; no operator rollout mutation surface is enabled in the current slice |
| `p080_reconciliation_events_v1` | P080: typed reconciliation event log; the current slice records diagnose-only and repaired operator/live-loop events |
| `p080_operator_request_dedup_v1` | P080: dedup/idempotency fence for MCP operator requests, fingerprinted by auth/policy/generation/rollout/class |
| `p080_readback_heartbeats_v1` | P080: bounded classifier/readback rows consumed by MCP diagnostics and GraphQL `p080Diagnostics` when `detection_only` is enabled; `p080DiagnosticsUpdates` emits initial snapshot and live polling events from the same projection |
| `p080_helper_leases_v1`, `p080_helper_lease_members_v1` | P080: additive schema for Chainworks-owned helper process lease/membership records used by future orphan-reaping gates |
| `p080_candidate_deferral_v1`, `p080_iteration_cursor_v1`, `p080_recurrence_epoch_v1`, `p080_executor_registration_watchdog_v1` | P080: additive scheduling, deferral, recurrence, and watchdog state for later continuous-reconciliation phases |
...
| `ShutdownProviderSession` | Records provider-cancellation intent and planned shutdown signal rows through command idempotency. |
| `P083RollbackExecution` | Moves enforcement mode to an explicit permissive or disabled target and writes rollback audit/idempotency rows. |
| `P083SetEnforcementMode` | Changes the enforcement mode with transition journal, audit, and command-idempotency rows. |
| `RetryRun` | Re-queues an `AdvanceRun` work item for a stalled or failed run through command idempotency. |
| `ForceReconcileSideEffect` | Force-reconciles a side effect to reconciled status with the operator decision through command idempotency. |
...
4. **SQLite as source of truth.** Both canonical domain state and materialized projections live in the same SQLite database. Execution-truth ownership is defined in [execution-truth-and-recovery.md](execution-truth-and-recovery.md). No separate event store.
