...
- **Symlink-Escape Prevention** (P075-SEC-H001): `artifact_root` is canonicalized via `tokio::fs::canonicalize` before path joining, and the resolved parent directory is verified to start with the canonical root after `create_dir_all`. A symlinked intermediate directory pointing outside the artifact root is rejected before any bytes are written.
