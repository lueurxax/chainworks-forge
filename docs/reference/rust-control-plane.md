...
- **Single Bounded Gateway**: Non-test runtime writes either route through `DbWriter` or use an explicit bounded transaction boundary for multi-row invariants. The source-controlled P075 write-bypass allowlist no longer accepts rollout inventory entries; it is limited to permanent infrastructure scopes (migrations, tests, and startup repair).
...
- **Gate Enforcement**: The `proposal-075` gate runs fail-closed and fails on temporary rollout bypasses, production runtime SQL writes that bypass DbWriter or `begin_immediate_with_retry`, entries missing retirement data, allowlist entries whose `expires_after_phase` is below the current closeout phase, or non-test `WriteOperation.operation_name` literals under `control-plane/crates/db/src` not present in the registry.
