...
- [execution-truth-and-recovery.md](execution-truth-and-recovery.md) — Canonical terminal outcomes, durable execution-truth ownership, command idempotency, shutdown recovery, manual identity holds, atomic transition settlement, cursor-driven resume, stage-owned recovery evidence, approval restore, runtime binding truth, host interruption, **side-effect reconciliation**, **mediation settlement**, and startup recovery progress readback
- [skill-resolution-and-runtime-integration.md](skill-resolution-and-runtime-integration.md) — Default-on mission context, strict frozen Agent Skills, copied-invocation authority, runtime injection, and operator readback
...
- [managed-temporary-artifact-inventory.md](managed-temporary-artifact-inventory.md) — Read-only, dry-run-only managed temporary artifact inventory slice: architecture, DTO schema, ByteCountString representation, path redaction, test root override, and scanner bounds (retained `proposal-089-temp-inventory|p089-temp-inventory` gate aliases)
