...
- local Swift toolchain (Xcode 26.3+) and Python 3 required; no UI host, daemon, or network required
- no Rust toolchain required — this is a Swift presenter + contract doc gate
...
- the gate fails closed if the contract doc is missing a required row or term, any negative fixture is absent or lacks `contract_violation`, the presenter file is missing a required symbol, or the `Proposal085Tests` Swift slice fails
