...
- fail-closed registry enforcement: `write-bypass-allowlist.toml` and `write-operation-registry.toml` must exist, be valid, include retirement metadata, reject all `temporary_rollout` bypass rows, reject production runtime SQL writes that bypass DbWriter or `begin_immediate_with_retry`, and cover observed non-test `WriteOperation.operation_name` literals under `control-plane/crates/db/src`
