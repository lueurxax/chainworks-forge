...
Execution-Truth Ownership and Invariant Model gate. Proves the P083 additive migration set, the new repository modules, and the bounded operational metric labels are present and self-consistent on the current tree. The proposal contract lives in [`../proposals/083-execution-truth-ownership-invariant-model.md`](../proposals/083-execution-truth-ownership-invariant-model.md); cross-cutting implementation references live in [`execution-truth-and-recovery.md`](execution-truth-and-recovery.md) and [`rust-control-plane.md`](rust-control-plane.md).

Scope:

- presence and self-test of the eight additive P083 migrations: `087_p083_001_artifact_lineage_report_kind.sql`, `088_p083_002_command_idempotency_generations.sql`, `089_p083_003_shutdown_receipts_and_signals.sql`, `090_p083_004_cancel_late_output_overflow.sql`, `091_p083_005_enforcement_and_rollback.sql`, `092_p083_006_durable_monotonic_clock.sql`, `093_p083_007_provider_cancellation_intent_and_process_fate.sql`, `094_p083_008_signal_dispatching_state.sql` (the `dispatching` intent_state added in `p083_008` is an implementation-time security hardening that lets the dispatcher atomically claim a planned signal row before issuing the OS signal, preventing concurrent dispatchers or crash-recovery passes from re-issuing the same signal)
- `cargo test -p db --test proposal_083_migrations` — migration shape, column types, indexes, and CHECK constraints for each P083 table (artifact lineage report kind, `command_idempotency` / `command_request_aliases`, shutdown receipts/signals, cancel late-output overflow latches, enforcement and rollback audit rows, durable monotonic clock, provider cancellation intent / process fate)
- `cargo test -p db --lib p083_` — repository slice covering `command_idempotency`, `provider_sessions`, and `shutdown_receipts` modules plus the bounded operational metric labels declared by `db::metrics`
- `cargo test -p engine p083` and `cargo test -p engine shutdown` — engine-level P083 command-handler and shutdown-service slices, including the shutdown recovery classifier
- `cargo check -p db`, `cargo check -p daemon`, `cargo check -p graphql-server`, `cargo check -p mcp-server` — compile sanity for every crate that carries P083 surfaces (db repos/metrics, daemon shutdown wiring, GraphQL `RollbackDispositionJSON` resolver validation, MCP P083 lifecycle tool registration and dispatch)
- Python presence/wiring checks: every required physical P083 migration file exists, the `P083_MIGRATIONS` readback descriptors expose the canonical `20260602_p083_001` through `20260602_p083_007` filenames required by `migration_plan_v1`, `RollbackDispositionJSON::to_value()` calls `validate_rollback_disposition_v1`, the core P083 command handlers use `acquire_tx` for atomic idempotency, and the daemon samples `durable_monotonic_clock_samples` via `monotonic_clock_ms()`

Use when:

- adding or modifying any of the eight P083 migrations, the `command_idempotency`/`provider_sessions`/`shutdown_receipts` repos, the P083 entries in `db::metrics`, the engine shutdown service, the `RollbackDispositionJSON` resolver, or the MCP P083 lifecycle tools
- proving repository truth for the execution-truth ownership and invariant model on the current tree
- verifying the P083 additive migration surface has not regressed before merging adjacent changes

Host policy:

- local Rust toolchain and Python 3 required; no UI host, daemon, or network required
- the gate uses an isolated `CARGO_TARGET_DIR=target/proposal-083-gate` to avoid contending with the default workspace target dir
...
- `p083` is accepted as an alias
- the gate fails closed if any of the eight required P083 migrations is absent, the `proposal_083_migrations` integration test fails, the `p083_` library tests fail, any `cargo check` (`db`, `daemon`, `graphql-server`, `mcp-server`) does not compile, the engine `p083`/`shutdown` slices fail, or any of the Python presence/wiring checks fail
- the gate does not exercise the GraphQL/MCP P083 surfaces directly; those readback surfaces are documented in the GraphQL/MCP references, while executable GraphQL/MCP P083 behavior remains covered only where dedicated surface tests exist
