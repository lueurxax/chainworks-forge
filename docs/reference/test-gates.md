...
- `control-plane/target/parity-control/` is acquired by the gate driver before destructive work; `lease.json`, `current-step.json`, `release-marker.json`, and `reclaim-marker.json` are written via the same-directory temp-file + durable-flush + atomic-rename contract. Before any `target/parity*` directory is created or written, the gate rejects symlinked or out-of-target paths.
- P041 subprocesses run through the gate supervisor with a dedicated process group/session. The supervisor records the active `pgid` in `lease.json` and `current-step.json`, enforces `P041_GATE_DEADLINE_SECONDS` plus `P041_DRAIN_GRACE_SECONDS`, sends process-group termination on timeout/interruption, and writes `timeout-marker.json` with descendant quiescence evidence.
