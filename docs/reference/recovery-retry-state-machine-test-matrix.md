...
| `cancel_pending_approval_preserved` | Cancellation requested while an approval was pending; approval decision not synthesized |
...
| `p082_recovery_matrix_readback` | `p082_recovery_matrix_readback_v1` or null | Inline readback for the rejection context |
...
| `stale_after_ms` | integer or null | Grace window in milliseconds before a session is considered stale |
| `stale_cutoff` | string (ISO-8601) or null | Absolute cutoff timestamp |
...
| `backpressure_scope` | string or null | Scope for capacity backpressure |
