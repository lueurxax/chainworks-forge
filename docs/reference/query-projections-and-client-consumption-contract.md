...
| Agent work continuation status | `continuationStatus(agentExecutionId:)` | `db::repos::agent_work_continuations::{find_active_for_agent_execution,list_for_agent_execution}` | Implemented | Operator-only read of an `AgentExecution`'s active continuation record and full history; renders raw and display status, freshness, and `UNKNOWN` for unrecognised daemon values. Continuation mutations are MCP-only. See [proposal-086-api-contracts.md](proposal-086-api-contracts.md). |
| Agent work continuation candidates | `continuationCandidates(runId:)` | `db::repos::agent_work_continuations::list_candidates_for_run` | Implemented | Operator-only list of eligible continuation candidates for a run, with per-`AgentExecution` eligibility, raw/display status, and disabled reason; restricted to stage-owned `code_writer` rows. See [proposal-086-api-contracts.md](proposal-086-api-contracts.md). |
