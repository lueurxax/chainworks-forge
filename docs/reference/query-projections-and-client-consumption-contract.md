...
- It does **NOT** use GraphQL mutations EXCEPT for the `approveApproval` and `rejectApproval` human-gate paths.
...
| Agent work continuation status | `continuationStatus(agentExecutionId:)` | `db::repos::agent_work_continuations::{find_active_for_agent_execution,list_for_agent_execution}` | Implemented | Operator-only read of an `AgentExecution`'s active continuation record and full history; renders raw and display status, freshness, and `UNKNOWN` for unrecognised daemon values. Continuation mutations are MCP-only. See [agent-work-continuation.md](agent-work-continuation.md). |
| Agent work continuation candidates | `continuationCandidates(runId:)` | `db::repos::agent_work_continuations::list_candidates_for_run` | Implemented | Operator-only list of eligible continuation candidates for a run, with per-`AgentExecution` eligibility, raw/display status, and disabled reason; restricted to stage-owned `code_writer` rows. See [agent-work-continuation.md](agent-work-continuation.md). |
| Execution-truth rollout contract readback | `Run.p083RolloutContractReadback` with `RollbackDispositionJSON` scalar | `crates/graphql-server/src/types/p083.rs` over the terminal `rollout_contract_check` row | Implemented | Render rollout contract status, enforcement mode, hold conditions, and rollback disposition. Disposition values are resolver-validated against `rollback_disposition_v1` before serialization. |
| Identity-ambiguous provider sessions | `p083IdentityHoldSessions(runId:)` | `crates/graphql-server/src/types/p083.rs` over `provider_cancellation_intents` with `intent_state='held'` and `provider_sessions.process_fate='identity_ambiguous'` | Implemented | Operator-only read driving the `ManualProcessIdentityCheckBanner` in run detail, stage detail, and the recovery inbox. Render provider name, cancellation epoch, last-seen PID, process-start identity hash, latest receipt id, reason detail, and bounded live-probe status/detail. Identity-hold resolution actions remain MCP-only; this query performs a read-only process identity probe and refreshes readback without mutating state or issuing shutdown signals. |
| Agent work continuation history | `continuations(runId:)` | `db::repos::agent_work_continuations::list_for_run` | Implemented | Operator-only run-level continuation history for passive readback. The client may display lifecycle status, mode, trigger kind, request/response fingerprints, artifact ids, and reconciliation state, but it must not infer retry/continue authority from this history. Continuation mutations remain MCP-only. |
| Agent work continuation metrics | `continuationMetricsSummary(runId:)` | `db::repos::agent_work_continuations::p086_continuation_metrics_summary_for_run` over `p086_continuation_metric_events` | Implemented | Render durable P086 admission/settlement/recovery counters plus KPI rollups from daemon-owned metric events: useful-progress/no-progress rates, follow-up validation success rate, trigger-specific success rates, average time saved versus fresh retry estimate, provider/session budget totals, and provider-session resurrection attach success/failure totals. Missing rows render as zero-count readback, not as proof that continuation is disabled. |
| Managed temporary artifact inventory | `tempArtifactInventory(input:)` | `crates/graphql-server/src/types/temp_artifact_inventory.rs` over the daemon-installed MCP inventory backend | Implemented | Operator-only read-only advisory inventory readback, `disabled` by default. The resolver delegates to the same mode-check/validation/scan/redaction/mutation-guard path as the MCP lane, so the response is a value-lossless camelCase projection of the canonical `temp_artifact_inventory_v1` DTO; byte counts are `ByteCountString` decimal strings and timestamps are `DateTime` or null. All eight canonical domains are GraphQL enums with `SCREAMING_SNAKE_CASE` items: `InventoryStatus`, `InventoryEnabledState`, `InventoryMode`, `MutationGuardStatus`, `RootKind`, `LifecycleClassification`, `DryRunRecommendation`, and `InventoryErrorCode`. The seven proposal-era domains include `UNKNOWN`; refinement-added `InventoryMode` is currently the closed `DISABLED | HIDDEN_READBACK | OPERATOR_VISIBLE` set. Clients must map unknown cases defensively where their decoder permits it, and `dryRun.recommendationCounts` remains an untyped JSON object. A process without the backend installed returns disabled readback with `disabled_reason_code = "backend_not_wired"`, which is not proof that the capability is unavailable. The client must never scan the local filesystem, infer ownership locally, persist rows, or render raw paths. The checked-in contract fixtures still diverge from parts of the shipped schema and therefore remain an `operator_visible` promotion hold; see [managed-temporary-artifact-inventory.md §4.4](managed-temporary-artifact-inventory.md#44-graphql-projection-specifics-implemented). |
