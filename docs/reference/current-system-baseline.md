...
- durable execution-truth ownership and invariants: additive SQLite surfaces for command idempotency leases, shutdown receipts and signal side-effects, post-cancel late-output overflow latches, enforcement-mode state/audit, rollback audit, durable monotonic clock samples, and provider cancellation intents with `identity_ambiguous` holds; `Run.p083RolloutContractReadback` GraphQL field with `RollbackDispositionJSON` output validation; operator-only execution-truth command tools/mutations for provider-session shutdown, process-absent confirmation, rollback execution, enforcement-mode changes, run retry, and side-effect force-reconcile; bounded operational metric labels under `db::metrics::P083_REQUIRED_METRICS`,
...
- default-on bounded `AgentMissionContextV1` for every fresh Rust-owned provider
  invocation, compiler-owned catalog snapshot format `2`, five strict
  single-file external Agent Skills bundles, exact frozen/durable copy
  validation for task/owner/P017/P058 work, and provider-free canonical proof,
...
- provider-neutral `code_writer` completion freshness and handoff recovery: pre/post prompt worktree fingerprints, current-attempt eligibility, one bounded same-session completion turn, canonical completion receipts and prompt/runtime evidence, bounded default-on partial-write startup recovery, targeted historical recovery with preserved evidence, and GraphQL/MCP/report/Swift readback with forward-compatible public enum handling,
...
| Code-writer completion freshness, receipt recovery, and readback | `control-plane/crates/domain/src/code_writer_completion.rs`, `control-plane/crates/db/src/repos/code_writer_completion_receipts.rs`, `control-plane/crates/engine/src/executor.rs`, `control-plane/crates/engine/src/recovery.rs` | [output-contracts-failure-evidence-and-recovery.md](output-contracts-failure-evidence-and-recovery.md#code-writer-completion-freshness-and-repair-p088-retained-alias) |
...
| Mission context, frozen skill bundles, and copied invocation authority | `control-plane/crates/workflow/src/compiler.rs`, `control-plane/crates/workflow/src/skill_bundle.rs`, `control-plane/crates/engine/src/agent_mission_context.rs`, `control-plane/crates/engine/src/orchestrator.rs` | [skill-resolution-and-runtime-integration.md](skill-resolution-and-runtime-integration.md), [test-gates.md](test-gates.md#agent-context-skills) |
...
| Code-writer completion freshness and repair | [output-contracts-failure-evidence-and-recovery.md](output-contracts-failure-evidence-and-recovery.md#code-writer-completion-freshness-and-repair-p088-retained-alias) |
...
20. code-writer completion freshness classification, bounded same-session output publication, crash-partial receipt recovery, and canonical implementation-completion readback.
