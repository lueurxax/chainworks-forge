...
- durable execution-truth ownership and invariants (Proposal 083): additive SQLite surfaces for command idempotency leases, shutdown receipts and signal side-effects, post-cancel late-output overflow latches, enforcement-mode state/audit, rollback audit, durable monotonic clock samples, and provider cancellation intents with `identity_ambiguous` holds; `Run.p083RolloutContractReadback` GraphQL field with `RollbackDispositionJSON` output validation; operator-only P083 command tools/mutations for provider-session shutdown, process-absent confirmation, rollback execution, enforcement-mode changes, run retry, and side-effect force-reconcile; bounded operational metric labels under `db::metrics::P083_REQUIRED_METRICS`,
