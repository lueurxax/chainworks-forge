...
| DOC-08 | `scripts/test-gate.sh` | 2026-04-10 | High | The repo-owned prerequisite lane still exists as `proposal-029`, not `proposal-030`. | Proposal can point at the wrong or ambiguous proof lane even after its own dependency was renumbered to `P030`. | Proof-lane source of truth. |
...
- Proposal-first blockers:
  - the proof lane still says transfer continuity is preserved even though `3.6a` now explicitly requires Codex re-auth
  - the prerequisite gate still mixes `P030` dependency text with the historical `proposal-029` lane name
- External hold:
...
|---|---|---|---|---|---|---|---|
| NAV-01 | provider settings migration | Proposal + targeted refresh | 2026-04-10 | High | Proposal now explicitly chooses Codex delete-and-reseed with operator re-auth, while Claude/Gemini preserve UUID. | Proof language can drift from the actual migration semantics. | Primary operator migration surface. |
| NAV-02 | settings export/import flow | Baseline + proposal | 2026-04-10 | High | Transfer packages rely on provider-specific secret placeholders derived from UUID; the proposal now drops deleted Codex placeholders intentionally. | Proof contract is wrong if it still promises blanket continuity. | Cross-machine continuity surface. |
| NAV-03 | benchmark/sign-off surfaces | Baseline + targeted refresh | 2026-04-10 | High | MVP boundary/UI/evaluation still freeze the old provider set today, but the proposal now classifies these owners explicitly. | Previous boundary-owner blocker is closed; only the proof lane needs rereview. | Stable boundary surface. |
...
|---|---|---|---|---|---|---|---|---|
| MAP-01 | `Chainworks Forge/Providers/ProviderAdapter.swift` | Secrets | provider secret-key derivation | 2026-04-10 | High | Secrets are keyed by `provider.<uuid>`, not by provider family. | Needed to verify the new Codex re-auth decision against current storage reality. | Migration grounding. |
| MAP-02 | `Chainworks Forge/Support/SettingsTransferService.swift` | Settings transfer | placeholder export/import continuity | 2026-04-10 | High | Exported placeholders are derived from the provider UUID-bound secret key. | Proposal proof wording is wrong if it still promises continuity for deleted Codex rows. | Main proof-contract blocker. |
| MAP-03 | `Chainworks Forge/Providers/ProviderSettingsStore.swift` | Durable settings | default seeding and provider-row identity | 2026-04-10 | High | Seeded providers get fresh UUIDs; proposal now makes that explicit for deleted Codex rows and preserves UUID only for migrated rows. | Prior continuity blocker is closed, but the proof lane must match that split behavior. | Migration owner. |
...
| MAP-05 | `scripts/test-gate.sh` | Proof lane | current repo-owned prerequisite gate naming | 2026-04-10 | High | The repo still exposes `proposal-029` / `PROPOSAL_029_TESTS` for the second-wave ACP gate. | Proposal prerequisite is ambiguous unless it explains this alias or renames it. | Main gate-contract blocker. |
...
|---|---|---|---|---|---|---|---|---|
| DATA-01 | Provider credential continuity | `ProviderAdapter.secretKey(for:)`, `ProviderSettingsStore`, proposal `3.6a` | local persistence + keychain | 2026-04-10 | High | Proposal now explicitly chooses UUID preservation for migrated rows and explicit re-auth for deleted Codex rows. | Proof layer is stale if it still describes the whole migration as preserved continuity. | High proof-contract gap. |
| DATA-02 | Transfer placeholder continuity | `SettingsTransferService`, proposal `3.6a` | exported/imported machine state | 2026-04-10 | High | Placeholder lists are bound to UUID; deleted Codex placeholders are intentionally dropped. | Gate wording becomes inaccurate if it still promises blanket continuity. | High transfer-proof gap. |
...
|---|---|---|---|---|---|---|---|
| INT-01 | keychain + provider row identity | Current repo + proposal | 2026-04-10 | High | Provider identity and secret identity are coupled through the provider UUID, and the proposal now deliberately splits migrated-row continuity from deleted-row re-auth. | Proof wording must follow that split. | Main architecture seam. |
...
| INT-03 | repository gate ownership | Proposal + repo | 2026-04-10 | High | The repo still names the second-wave ACP lane `proposal-029` while the proposal depends on `P030`. | Prerequisite proof remains ambiguous unless the alias is locked or renamed. | Main gate-contract blocker. |
...
| Happy path | Partial | DOC-01, DATA-01, DATA-02 | provider migration and transfer continuity | Migration semantics are now explicit, but the proof lane still over-claims continuity for the whole transfer path. |
...
| Validation error | Partial | DOC-01, DATA-02, MAP-02 | settings transfer import validation | Validation/proof wording is still misaligned with the new Codex re-auth semantics. |
...
| FLAG-02 | durable provider settings migration | provider-platform continuity | contract is explicit, proof wording is partially stale | no special rollback beyond hold | 2026-04-10 | High | Main remaining gap is proof alignment, not migration semantics. |
...
| TEST-02 | durable settings migration proof | persisted provider settings and transfer packages | proposal now explicitly names both local and transfer-path proof | split proof into Claude/Gemini continuity vs Codex re-auth/remediation | 2026-04-10 | High | Follows from the remaining proof-contract gap. |
| TEST-03 | prerequisite gate proof | second-wave ACP dependency lane naming | proposal still mixes `P030` and `proposal-029` | either document the alias or rename the lane | 2026-04-10 | High | Follows from the remaining gate-contract gap. |
...
|---|---|---|---|---|---|---|
| REAL-01 | transfer migration proof | proof lane says “cross-machine continuity preserved” | the same proposal now explicitly deletes Codex rows, drops Codex placeholders, and requires re-auth | 2026-04-10 | High | Proposal text is internally inconsistent until proof wording is narrowed. |
| REAL-02 | prerequisite gate naming | proposal depends on `P030` | the same proposal snippet and repo script still invoke `proposal-029` / `PROPOSAL_029_TESTS` | 2026-04-10 | High | Prerequisite proof remains ambiguous until the alias is explained or renamed. |
| REAL-03 | previous findings | docs/proof/operator wording/`runtimeSessionID` ownership, Codex continuity semantics, and provider-boundary fallout were incomplete | current proposal now includes those sections explicitly | 2026-04-10 | High | Previous stale findings should not be reused. |
...
| State handling | Partial | H table | Migration semantics are explicit, but proof/acceptance wording still trails them. |
| Data / API contract | Partial | DATA-01, DATA-02, REAL-01 | Transfer proof still overstates continuity after the new Codex re-auth decision. |
...
| Testing strategy | Partial | TEST-02, TEST-03 | Proof wording still needs alignment with the new migration and gate semantics. |
| Dependencies / integration points | Partial | DOC-07, REAL-02 | External dependency is explicit, but the prereq lane name is still mixed. |
...
- QUESTION-01: should proof item `2` say “continuity preserved for migrated rows” instead of “cross-machine continuity preserved” to match the Codex re-auth rule?
- QUESTION-02: is `proposal-029` an intentional repo alias for `P030`, or should the gate be renamed in this slice?
- BLOCKER-01: proof wording still over-claims transfer continuity after the new Codex re-auth decision.
- BLOCKER-02: prerequisite gate naming still mixes `P030` and `proposal-029`.
