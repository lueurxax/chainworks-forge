...
- Proposal-first blockers:
  - durable settings migration now has a raw pre-decode seam, but it still does not lock how that seam handles the two different payload shapes: bare `ProviderSettings` and wrapped `ExportableSettingsPackage`
- External hold:
...
|---|---|---|---|---|---|---|---|
| NAV-01 | provider settings migration | Proposal + targeted refresh | 2026-04-10 | High | Proposal now explicitly chooses Codex delete-and-reseed with operator re-auth, while Claude/Gemini preserve UUID. | The remaining risk is no longer semantic drift; it is whether the raw migration contract is schema-aware enough for both payload shapes. | Primary operator migration surface. |
| NAV-02 | settings export/import flow | Baseline + proposal | 2026-04-10 | High | Transfer packages rely on provider-specific secret placeholders derived from UUID; the proposal now drops deleted Codex placeholders intentionally. | Import path still needs explicit wrapper-aware raw migration semantics because `ExportableSettingsPackage` is not shaped like bare `ProviderSettings`. | Cross-machine continuity surface. |
...
|---|---|---|---|---|---|---|---|---|
| DATA-01 | Provider credential continuity | `ProviderAdapter.secretKey(for:)`, `ProviderSettingsStore`, proposal `3.6a` | local persistence + keychain | 2026-04-10 | High | Proposal now explicitly chooses UUID preservation for migrated rows and explicit re-auth for deleted Codex rows. | The remaining ambiguity is not row semantics, but which raw migrator shape owns the local vs wrapped payloads. | High persistence gap. |
| DATA-02 | Transfer placeholder continuity | `SettingsTransferService`, proposal `3.6a` | exported/imported machine state | 2026-04-10 | High | Placeholder lists are bound to UUID and live on the typed transfer package outside `providerSettings`. | Proposal still needs to say where the wrapped transfer-package migration rewrites `secretPlaceholders` before placeholder validation runs. | High transfer-schema gap. |
...
|---|---|---|---|---|---|---|---|
| INT-01 | keychain + provider row identity | Current repo + proposal | 2026-04-10 | High | Provider identity and secret identity are coupled through the provider UUID, and the proposal now deliberately splits migrated-row continuity from deleted-row re-auth. | Those semantics still need a schema-aware raw migration entry point before typed decode. | Main architecture seam. |
...
| Happy path | Partial | DOC-01, DATA-01, DATA-02 | provider migration and transfer continuity | Migration semantics are explicit, but the proposal still does not fully specify how the raw migrator handles both payload schemas. |
...
| Validation error | Partial | DOC-01, DATA-02, MAP-02 | settings transfer import validation | Typed package decode and placeholder validation still need wrapper-aware pre-decode migration semantics before old raw values can be handled. |
...
| FLAG-02 | durable provider settings migration | provider-platform continuity | row-level contract is explicit, schema-aware raw seam is not | no special rollback beyond hold | 2026-04-10 | High | Main remaining gap is migration-owner clarity, not semantics. |
...
| TEST-02 | durable settings migration proof | persisted provider settings and transfer packages | proposal now explicitly names both local and transfer-path proof | add proof that migration runs on raw local JSON and wrapped transfer-package data before typed decode and placeholder validation | 2026-04-10 | High | Follows from the remaining raw-migration gap. |
...
|---|---|---|---|---|---|---|
| REAL-01 | raw settings migration | proposal now says migration happens on raw JSON before decode | current local and transfer owners use two different persisted shapes: bare `ProviderSettings` locally and wrapped `ExportableSettingsPackage` on import | 2026-04-10 | High | Proposal still under-specifies the schema-aware migration seam. |
...
| State handling | Partial | H table | Migration semantics are explicit, but the schema-aware pre-decode owner is still not locked. |
| Data / API contract | Partial | DATA-01, DATA-02, REAL-01 | Local and transfer migration still rely on an implicit wrapper-aware raw-data seam. |
...
| Testing strategy | Partial | TEST-02 | Proof still needs an explicit schema-aware raw pre-decode migration assertion. |
...
- QUESTION-01: should the proposal use one shape-detecting raw migrator, or two separate raw migrators for local settings and transfer packages?
- QUESTION-02: where exactly should `secretPlaceholders` rewrite/drop happen on the wrapped transfer-package path?
- BLOCKER-01: schema-aware raw pre-decode migration ownership is still implicit for both `provider-settings.json` and `chainworks-settings.json`.
