...
- Proposal-first blockers:
  - durable settings migration now has the schema-aware split, but it still does not lock one consistent helper/call-site API after introducing `migrateRawProviderSettings(_:)` and `migrateRawTransferPackage(_:)`
- External hold:
...
|---|---|---|---|---|---|---|---|
| NAV-01 | provider settings migration | Proposal + targeted refresh | 2026-04-10 | High | Proposal now explicitly chooses Codex delete-and-reseed with operator re-auth, while Claude/Gemini preserve UUID. | The remaining risk is no longer schema ownership; it is final API consistency between the named migrators and the call-site text. | Primary operator migration surface. |
| NAV-02 | settings export/import flow | Baseline + proposal | 2026-04-10 | High | Transfer packages rely on provider-specific secret placeholders derived from UUID; the proposal now drops deleted Codex placeholders intentionally. | Import path still needs one unambiguous helper contract because the section defines schema-specific migrators but the call-site text still names a generic one. | Cross-machine continuity surface. |
...
|---|---|---|---|---|---|---|---|---|
| DATA-01 | Provider credential continuity | `ProviderAdapter.secretKey(for:)`, `ProviderSettingsStore`, proposal `3.6a` | local persistence + keychain | 2026-04-10 | High | Proposal now explicitly chooses UUID preservation for migrated rows and explicit re-auth for deleted Codex rows. | The remaining ambiguity is no longer row semantics; it is whether local and transfer paths call schema-specific migrators directly or through a dispatcher. | Residual migration-contract gap. |
| DATA-02 | Transfer placeholder continuity | `SettingsTransferService`, proposal `3.6a` | exported/imported machine state | 2026-04-10 | High | Placeholder lists are bound to UUID and live on the typed transfer package outside `providerSettings`. | Proposal now states the rewrite/drop semantics; the remaining ambiguity is the API name/path used to invoke that wrapped migration. | Residual transfer-contract gap. |
...
|---|---|---|---|---|---|---|---|
| INT-01 | keychain + provider row identity | Current repo + proposal | 2026-04-10 | High | Provider identity and secret identity are coupled through the provider UUID, and the proposal now deliberately splits migrated-row continuity from deleted-row re-auth. | Those semantics are now owned; the remaining seam is consistent helper invocation across local and transfer paths. | Main architecture seam. |
...
| Happy path | Partial | DOC-01, DATA-01, DATA-02 | provider migration and transfer continuity | Migration semantics are explicit, but the call-site/helper API still mixes generic and schema-specific names. |
...
| Validation error | Partial | DOC-01, DATA-02, MAP-02 | settings transfer import validation | Wrapped transfer migration semantics are explicit, but the section still needs one final API contract for how that helper is invoked. |
...
| FLAG-02 | durable provider settings migration | provider-platform continuity | row-level and schema-level contract are explicit, helper/call-site API is not | no special rollback beyond hold | 2026-04-10 | High | Main remaining gap is API consistency, not semantics. |
...
| TEST-02 | durable settings migration proof | persisted provider settings and transfer packages | proposal now explicitly names both local and transfer-path proof | align proof with the final chosen helper/call-site API so tests map cleanly to implementation owners | 2026-04-10 | High | Follows from the remaining migration-API gap. |
...
|---|---|---|---|---|---|---|
| REAL-01 | raw settings migration | proposal now defines two schema-specific raw migrators | the same section still says both call sites invoke `migrateRawJSONIfNeeded(_)` | 2026-04-10 | High | Proposal still under-specifies the final helper/call-site API. |
...
| State handling | Partial | H table | Migration semantics are explicit, but the helper/call-site API is still not locked. |
| Data / API contract | Partial | DATA-01, DATA-02, REAL-01 | Local and transfer migration still rely on an implicit final dispatcher-vs-direct-call choice. |
...
| Testing strategy | Partial | TEST-02 | Proof still needs alignment with the final helper/call-site migration API. |
...
- QUESTION-01: should the final implementation expose the two schema-specific helpers directly at the call sites, or wrap them behind a small dispatcher?
- QUESTION-02: if a dispatcher exists, should the proposal name it explicitly or simply show direct helper mapping at each call site?
- BLOCKER-01: final helper/call-site API remains inconsistent inside `3.6a`.
