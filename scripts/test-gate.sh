...
PY
    ...
    if [[ ! -f "$ROOT_DIR/control-plane/crates/db/migrations/064_p081_audit_log.sql" ]]; then
      die "P081: missing migration 064_p081_audit_log.sql"
    fi
    if [[ ! -f "$ROOT_DIR/control-plane/crates/db/migrations/065_p081_audit_log_checkpoints.sql" ]]; then
      die "P081: missing migration 065_p081_audit_log_checkpoints.sql"
    fi

    log "P081: migration compile check - command_journal caller_class column migration exists (Phase 2)"
    if [[ ! -f "$ROOT_DIR/control-plane/crates/db/migrations/066_p081_caller_class.sql" ]]; then
      die "P081: missing migration 066_p081_caller_class.sql"
    fi
    if [[ ! -f "$ROOT_DIR/control-plane/crates/db/migrations/067_p081_approval_idempotency.sql" ]]; then
      die "P081: missing migration 067_p081_approval_idempotency.sql"
    fi
    if [[ ! -f "$ROOT_DIR/control-plane/crates/db/migrations/068_p081_fix_payload_length_check.sql" ]]; then
      die "P081: missing migration 068_p081_fix_payload_length_check.sql"
    fi
    if [[ ! -f "$ROOT_DIR/control-plane/crates/db/migrations/069_p081_approval_idempotency_request_hash.sql" ]]; then
      die "P081: missing migration 069_p081_approval_idempotency_request_hash.sql"
    fi
    if [[ ! -f "$ROOT_DIR/control-plane/crates/db/migrations/070_p081_mcp_command_idempotency.sql" ]]; then
      die "P081: missing migration 070_p081_mcp_command_idempotency.sql"
    fi
    if [[ ! -f "$ROOT_DIR/control-plane/crates/db/migrations/071_p081_command_journal_idempotency.sql" ]]; then
      die "P081: missing migration 071_p081_command_journal_idempotency.sql"
    fi
