...
PY
  ...
  proposal-080|p080)
    ...
    (
      ...
      if [[ ! -f "crates/db/migrations/079_p080_stale_execution_reconciliation.sql" ]]; then
        echo "FAIL: missing control-plane/crates/db/migrations/079_p080_stale_execution_reconciliation.sql"
        exit 1
      fi
      ...
      if ! grep -q "read_only_operator" "crates/db/migrations/079_p080_stale_execution_reconciliation.sql"; then
        echo "FAIL: migration must use approved principal_class IN ('operator','read_only_operator') (proposal line 513)"
        exit 1
      fi
      if grep -q "'agent'" "crates/db/migrations/079_p080_stale_execution_reconciliation.sql"; then
