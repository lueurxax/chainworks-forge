...
/// Maps goosed server `MessageEvent` SSE payloads into the app's `RuntimeStreamEvent` enum.
///
...
enum GooseStreamEventMapper {
    ...
    /// Map a raw SSE `data:` JSON string from goosed into a `RuntimeStreamEvent`.
    /// Returns `nil` for events that should be silently ignored (e.g., Ping).
    static func map(_ rawJSON: String) -> RuntimeStreamEvent? {
    ...
    private static func mapMessage(_ json: [String: Any]) -> RuntimeStreamEvent {
    ...
    private static func mapFinish(_ json: [String: Any]) -> RuntimeStreamEvent {
