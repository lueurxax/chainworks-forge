...
final class GooseServerManager: ManagedGooseServerControlling {
    ...
    private func makeLaunchPlan() -> GooseManagedServerLaunchPlan? {
        ...
        environment["NODE_OPTIONS"] = Self.managedNodeOptions(base: environment["NODE_OPTIONS"])
    ...
    nonisolated private static func managedNodeOptions(base: String?) -> String {
        let requiredFlag = "--max-old-space-size=4096"
        let existing = (base ?? "").trimmingCharacters(in: .whitespacesAndNewlines)
        guard existing.isEmpty == false else { return requiredFlag }
        if existing.contains(requiredFlag) {
            return existing
        }
