...
final class GooseServerTransport: RuntimeTransportProtocol, @unchecked Sendable {
    // MARK: - Configuration
    ...
    func createSession(request: RuntimeSessionRequest) async throws -> RuntimeSessionResponse {
        ...
        guard let sessionID, !sessionID.isEmpty else {
            throw RuntimeTransportError.sessionCreationFailed(
        ...
        if let resolvedProvider {
            ...
            let transportModel = resolvedModel.map { normalizeModelIdentifierForGoose($0, provider: transportProvider) }
    ...
    /// Normalize model identifiers for goosed compatibility.
    /// ChatGPT Codex rejects case-mismatched names (e.g. "GPT-5.4" must be "gpt-5.4").
    private func normalizeModelIdentifierForGoose(_ model: String, provider: String) -> String {
        switch provider {
        case "chatgpt_codex":
