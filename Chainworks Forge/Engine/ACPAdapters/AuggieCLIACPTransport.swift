...
// MARK: - AuggieCLIACPTransport (Proposal 029 — Full Implementation)
...
nonisolated final class AuggieCLIACPTransport: RuntimeTransportProtocol, @unchecked Sendable {
    ...
    init(executablePath: String = "auggie") {
    ...
    func createSession(request: RuntimeSessionRequest) async throws -> RuntimeSessionResponse {
        ForgeLogger.execution.debug("AuggieCLIACPTransport.createSession called, executablePath=\(executablePath), workingDir=\(request.workingDirectory ?? "nil")")
        let startTime = Date()
        ...
        self.startStderrLogging(for: subprocess, prefix: "AuggieCLIACP")
        ...
        guard initResponse != nil else {
            ...
            throw RuntimeTransportError.sessionCreationFailed(reason: "No response to ACP initialize request from Auggie")
        }
        ...
        // Auggie uses its own model catalog — pass through as-is.
        if let model = request.model {
            sessionParams["model"] = model
        }
        // Map execution policy to Auggie mode.
        // Auggie uses `autonomous` for write-enabled agents, `interactive` for read-only.
        ...
        // No _meta block — Auggie does not use Claude-specific plugin/MCP options.
        ...
        guard let sessionResult = try await readNextResult(from: subprocess),
            ...
            subprocess.terminate()
            throw RuntimeTransportError.sessionCreationFailed(reason: "Invalid session/new response from Auggie ACP")
        }
                ...
                do {
                    ...
                    for try await line in subprocess.readLines() {
                        ...
                        guard let lineData = line.data(using: .utf8),
                            ...
                            ForgeLogger.execution.info("AuggieACP: Non-JSON line from subprocess: \(line.prefix(200))")
                            continue
                        }

                        ForgeLogger.execution.debug("AuggieACP: Received JSON-RPC: method=\(json["method"] ?? "nil"), id=\(json["id"] ?? "nil"), hasResult=\(json["result"] != nil)")
                        ...
                        if let responseID, responseID == promptRequestID {
                            ...
                            if let result = json["result"] as? [String: Any] {
                                ...
                                let message = ACPProtocolSupport.formatJSONRPCError(
                                    error,
                                    fallback: "Unknown Auggie ACP error"
                                )
                        ...
                        if let method = json["method"] as? String {
                            ...
                            if method == "session/update" {
                                ...
                                ForgeLogger.execution.debug("AuggieACP session/update: sessionUpdate=\(sessionUpdate) updateKeys=\(updateKeys)")
                            }
    ...
    private func autoGrantPermission(
        ...
        guard let response = ACPProtocolSupport.permissionSelectionResponse(
            ...
            ForgeLogger.execution.error("Failed to auto-grant Auggie permission for session \(sessionID)")
            return
        }
