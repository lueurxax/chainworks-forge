...
await sessionInfo.execution.closeSession()
        ...
        if result.succeeded {
            ...
            ForgeLogger.session.error("[\(sessionID)] Stream Settled With Failure: \(result.errorMessage ?? "Execution did not produce required outputs"). Duration: \(Int(completedAt.timeIntervalSince(startedAt)))s")
        }
