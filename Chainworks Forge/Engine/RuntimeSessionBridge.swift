...
final class RuntimeSessionBridge: Sendable {
    ...
    private static func buildSystemPrompt(
        ...
        parts.append("- Tool stdout is not an output channel. Only the final assistant message is settled for CHAINWORKS_OUTPUT. Do not call shell `echo`, `printf`, or file-writing commands to return CHAINWORKS_OUTPUT.")
    ...
    private static func buildTaskDirective(
        ...
        if !structuredHints.isEmpty {
            ...
            parts.append("Tool stdout is not an output channel; only the final assistant message is settled.")
            parts.append("Do not call shell `echo`, `printf`, or file-writing commands to return CHAINWORKS_OUTPUT.")
